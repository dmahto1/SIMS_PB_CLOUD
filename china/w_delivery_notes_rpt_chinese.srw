HA$PBExportHeader$w_delivery_notes_rpt_chinese.srw
$PBExportComments$Delivery Report
forward
global type w_delivery_notes_rpt_chinese from w_std_report
end type
end forward
...
integer width = 3790
integer height = 2220
string title = "Delivery Notes"
end type
global w_delivery_notes_rpt_chinese w_delivery_notes_rpt_chinese

on w_delivery_notes_rpt_chinese.create
call super::create
end on

on w_delivery_notes_rpt_chinese.destroy
call super::destroy
...
Datastore   ds_detail,ds_pack,ds_dolist, ds_delivery_notes
integer p
boolean ib_found
...
//Jxlim 07/19/2010 deliver Notes Chinese
ds_delivery_notes = create datastore
ds_delivery_notes.Dataobject = 'd_deltest_prt_chinese'
ds_delivery_notes.settransobject(sqlca)
...
//	ls_sku = ds_pack.getitemstring(i,"sku")
  ...
  //Jxlim 07/19/2010 delivery notes
	// dw_report.setitem(j,"lot_no", ls_lot_no) 
	  ...
	  IF ls_etom <> "" and not isnull(ls_etom) and j=1 THEN
		...
		ELSE
			...
			ls_text[3]="etom_t.Text='$$HEX2$$9853737c$$ENDHEX$$'"
		END IF
	  END IF	
...
//	End If
	  ...
	  //Jxlim 07/19/2010 Delivery notes Chinese report
	   dw_report.setitem(j,"contact_person", w_do.idw_main.getitemstring(1,"contact_person"))
	   dw_report.setitem(j,"tel", w_do.idw_main.getitemstring(1,"tel"))
	   dw_report.setitem(j,"freight_etd", w_do.idw_main.getitemdatetime(1,"Freight_ETD"))
	   dw_report.setitem(j,"freight_eta", w_do.idw_main.getitemdatetime(1,"Freight_ETA"))			
	
	  dw_report.setitem(j,"remark",ls_remark)
	  //ls_allremark = ls_allremark+' '+ls_remark
	 
	 //Jxlim 07/19/2010 Modified code for delivery notes chinese report.
...
//
	...
	//Jxlim 07/19/2010 Chinese report delivery notes
...
//dw_report.object.t_po_number.text = ls_allremark  //Jxlim 07/19/2010 chinese report delivery notes
...
type dw_select from w_std_report`dw_select within w_delivery_notes_rpt_chinese
...
type cb_clear from w_std_report`cb_clear within w_delivery_notes_rpt_chinese
integer x = 3035
integer y = 36
end type

type dw_report from w_std_report`dw_report within w_delivery_notes_rpt_chinese
integer x = 0
integer y = 0
integer width = 3703
integer height = 2000
string dataobject = "d_delivery_notes_prt_chinese"
