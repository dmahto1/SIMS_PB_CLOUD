HA$PBExportHeader$u_nvo_proc_gibson.sru
$PBExportComments$Process GIBSON files
forward
global type u_nvo_proc_gibson from nonvisualobject
end type
end forward

global type u_nvo_proc_gibson from nonvisualobject
end type
global u_nvo_proc_gibson u_nvo_proc_gibson
...
public function integer uf_process_so (string aspath, string asproject);//Process Sales Order files for GIBSON
...
If liFileNo < 0 Then
	lsLogOut = "-       ***Unable to Open File for GIBSON Processing: " + asPath
	FileWrite(giLogFileNo,lsLogOut)
	gu_nvo_process_files.uf_write_log(lsLogOut) /*write to Screen*/
	...
	Choose Case Upper(lsRecType)
		...
		//	ldsDOHeader.SetITem(llNewRow,'wh_code',lswarehouse) /*Default WH for Project */
			...
			If upper(Trim(lsTemp)) = "2180"  OR upper(Trim(lsTemp)) = "MY10"  Then /*Malaysia*/
				ldsDOHeader.SetItem(llNewRow, 'wh_code',"GIBSON-MY") 
			Else /*Singapoore */
				ldsDOHeader.SetItem(llNewRow, 'wh_code',"GIBSON-SG") 
			End If
...
public function integer uf_process_itemmaster (string aspath, string asproject);
//Process Item Master (IM) Transaction for GIBSON (01/2015 - TAM - Cloned from Funai)
...
liFileNo = FileOpen(asPath,LineMode!,Read!,LockReadWrite!)
If liFileNo < 0 Then
	lsLogOut = "-       ***Unable to Open Item Master File for GIBSON Processing: " + asPath
	FileWrite(giLogFileNo,lsLogOut)
	gu_nvo_process_files.uf_write_log(lsLogOut) /*write to Screen*/
	Return -99 /* we wont move to error directory if we can't open the file here*/
End If
...
protected function integer uf_process_po (string aspath, string asproject);//Process PO for GIBSON
...
//Open and read the File In
lsLogOut = '      - Opening File for GIBSON Purchase Order Processing: ' + asPath
FileWrite(giLogFileNo,lsLogOut)
gu_nvo_process_files.uf_write_log(lsLogOut) /*write to Screen*/
...
For llRowPos = 1 to llRowCount
	...
	Choose Case Upper(lsRecType)
		...
		//	idsPOheader.SetItem(llNewRow, 'action_cd', lsTemp) /* defaulting to add above */
			...
			// Warehouse is being set based on the Plant Code - hardcoded for now
			If  upper(Trim(lsTemp)) = "2180" OR upper(Trim(lsTemp)) = "MY10"   Then  /*Malaysia*/
				idsPOheader.SetItem(llNewRow, 'wh_code',"GIBSON-MY") 
			Else /*Singapoore */
				idsPOheader.SetItem(llNewRow, 'wh_code',"GIBSON-SG") 
			End If
...
public function integer uf_process_return_order (string aspath, string asproject);//Process Return Order for Gibson
...
lsLogOut = "- PROCESSING FUNCTION: GIBSON Daily Inventory Snapshot File... " 
FileWrite(gilogFileNo,lsLogOut)
gu_nvo_process_files.uf_write_log(lsLogOut) /*display msg to screen*/
lsLogOut = ""
FileWrite(gilogFileNo,lsLogOut)
...
sql_syntax += " Where Project_ID = 'GIBSON'"
...
IF Len(ERRORS) > 0 THEN
   lsLogOut = "        *** Unable to create datastore for GIBSON Inventory Snapshot ID data.~r~r" + Errors
	FileWrite(gilogFileNo,lsLogOut)
   RETURN - 1
END IF
...
For llRowPos = 1 to llRowCOunt
	...
	If lsSupplier <> lsSupplierSave Then
		...
		If ldsOut.RowCount() > 0 Then
			gu_nvo_process_files.uf_process_flatfile_outbound(ldsOut,'GIBSON')
			ldsOut.Reset()
		End If
		
		//Next File Sequence #...
		ldBatchSeq = gu_nvo_process_files.uf_get_next_seq_no("GIBSON",'EDI_Generic_Outbound','EDI_Batch_Seq_No')
		If ldBatchSeq <= 0 Then
			lsLogOut = "        *** Unable to retrieve the next available sequence number for GIBSON BOH file. Confirmation will not be sent'"
			FileWrite(gilogFileNo,lsLogOut)
			Return -1
		End If
	...
	ldsOut.SetItem(llNewRow,'Project_id', 'GIBSON')
...
lsLogOut = "      Creating GIBSON Daily Balance on Hand-Volume File... " 
FileWrite(gilogFileNo,lsLogOut)
...
sql_syntax += "  Where Content_Summary.Project_id = 'GIBSON'  and "
...
IF Len(ERRORS) > 0 THEN
 	 lsLogOut = "        *** Unable to create datastore for GIBSON  (BOH-Volume Data).~r~r" + Errors
	FileWrite(gilogFileNo,lsLogOut)
 	 RETURN - 1
END IF
...
For llRowPos = 1 to lLRowCount /*Each Inv Record*/
	...
	If ldsBoh.GetITemString(llRowPos,'wh_code') <> lsWarehouseSave Then
		...
		lsFileName = "GIBSON-Daily-BOH-Volume-" + ldsBoh.GetITemString(llRowPos,'wh_code')  + "-" + + String(ldToday,"mm-dd-yyyy") + ".csv"
...
on u_nvo_proc_gibson.create
call super::create
TriggerEvent( this, "constructor" )
end on

on u_nvo_proc_gibson.destroy
