HA$PBExportHeader$u_nvo_proc_garmin.sru
$PBExportComments$Jxlim 05/01/2014 Garmin nvo proc object
forward
global type u_nvo_proc_garmin from nonvisualobject
end type
end forward

global type u_nvo_proc_garmin from nonvisualobject
end type
global u_nvo_proc_garmin u_nvo_proc_garmin
...
public function integer uf_process_dboh (string asproject, string asinifile, string asemail)
end prototypes
...
for llFileRowPos = 1 to llFilerowCount
	...
	Choose Case Upper(lsTemp)
		...
		Case 'PM' /*PO Header*/
			...
			//Order Type	C(1)	Yes	$$HEX1$$1c20$$ENDHEX$$S$$HEX2$$1d200900$$ENDHEX$$Must be valid order type
...
//			End If	
		...
		CASE 'PD' /* detail*/
			...
			//UOM 	C(24)	No	N/A	UOM is EA Jxlim 06/04/2014 			
			lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col24"))
...
public function integer uf_process_delivery_order (string aspath, string asproject);//Jxlim 05/31/2014 Garmin 
//Process Sales Order (DM) Transaction for Baseline Unicode Client

STRING lsTemp, lsProject, lsSku, lsSupplier, lsWarehouse, lsOrderType, lsInvoiceNumber, lsOrderNumber, lsuf1, lsHeaderProject, lsnotetype, lsnotetext,lsConsolNo
BOOLEAN lbError, lbNew, lbDetailError
LONG llCount, llNew, llOwner, llexist, llOrderSeq, llLineSeq, llbatchseq, llLineItemNo, llNewAddressRow,llcontentcount, llnotesequence, llnewnotesrow
INTEGER lirc, liRtnImp, liNewRow, llNewDetailRow
STRING lsLogOut, lsChangeID
INTEGER li_StartCol
INTEGER li_UFIdx
...
For llFileRowPos = 1 to llFilerowCount
	...
	//Record ID	C(2)	Yes	$$HEX1$$1c20$$ENDHEX$$DN$$HEX2$$1d200900$$ENDHEX$$Delivery order Notes identifier

	//Validate Rec Type is DM or DD or DN
	lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col1"))
...
//			ldsSOheader.SetItem(liNewRow,'Request_date',String(Today(),'YYMMDD'))
			...
			//TAM 2016/03 - Added consolidation number to be used for Crossdocking orders - Consolidation Number = Value Left of the "-"
			If Pos(lsInvoiceNumber,'-') >0  Then
				lsConsolNo = Left (lsInvoiceNumber,Pos(lsInvoiceNumber,'-' )-1)
				ldsSOheader.SetItem(liNewRow,'Consolidation_No', lsConsolNo)
			End If
...
//TAM 2016/03 - Added consolidation number to be used for Crossdocking orders
...
Choose Case Upper(Left(asFile,2))
	...
	Case  'PM'  
		...
		liRC = gu_nvo_process_files.uf_process_purchase_order(asProject)  //asProject   //Jxlim 05/02/2014 testing
...
for llFileRowPos = 1 to llFilerowCount
	...
	//Field Name	Type	Req.	Default	Description
	//Record_ID	C(2)	Yes	$$HEX1$$1c20$$ENDHEX$$WH$$HEX2$$1d200900$$ENDHEX$$Work order master identifier
	//Record_ID	C(2)	Yes	$$HEX1$$1c20$$ENDHEX$$WD$$HEX2$$1d200900$$ENDHEX$$Work order detail identifier
	//Record_ID	C(2)	Yes	$$HEX1$$1c20$$ENDHEX$$WC$$HEX2$$1d200900$$ENDHEX$$Work order component identifier
...
public function integer uf_process_dboh (string asproject, string asinifile, string asemail);//Jxlim 05/19/2014 BOH Garmin
...
String			lsFind,	&
				...
				lsNextRunDate,	&
				lsRunFreq, &
				lsFilename,  lsFileNamePath, &
				lsWarehouse, &
				lsLastWarehouse, &
				msg

DEcimal		ldBatchSeq
Integer		liRC, returnCode
...
//	BHYYMDD.dat
	...
	lsFilename = ("BH" + string(today(), "YYYYMMDD") + lsWarehouse + ".txt")
...
//gu_nvo_process_files.uf_process_flatfile_outbound_unicode(ldsOut,lsProject)  // //Jxlim, 06/18/2014 don't need for Garmin; this is to send to flatfileout

// TAM 2011/09  Added ability to email the report

//20-Jun-2014 :Madhu- Added code to store data onto file -START
//lsFileNamePath = ProfileString(asInifile,lsProject,"archivedirectory","") + '\' + lsFileName  + ".txt" //Madhu commented
lsFileNamePath = ProfileString(asInifile,lsProject,"archivedirectory","") + '\' + lsFileName  
returnCode = ldsOut.saveas(lsFileNamePath,TEXT!,true )

msg = 'Garmin Daily Balance On Hand Run Save As Return Code: ' + string(returnCode)
FileWrite(gilogFileNo,msg)
msg = 'Garmin Daily Balance On Hand Run Finished'
FileWrite(gilogFileNo,msg)
msg = '**********************************'
FileWrite(gilogFileNo,msg)
//20-Jun-2014 :Madhu- Added code to store data onto file -END

//setup email string from .ini file
...
//setup email string from activity schedule
gu_nvo_process_files.uf_send_email(lsProject,asEmail, lsProject + " Garmin Daily Balance On Hand Run on " + string(Today(), 'YYYY/MM/DD HH:MM') + ' GMT', "Attached is the BALANCE ON HAND REPORT run on " + string(Today(), ' YYYY/MM/DD HH:MM') + ' GMT', lsFileNamePath)

//
//FileWrite(gilogFileNo,"")
//lsLogOut = '**********************************'
//FileWrite(gilogFileNo,lsLogOut)
//lsLogOut = 'Started Daily Inventory Report'
//FileWrite(gilogFileNo,lsLogOut)
//
//
//// Create our filename and path
////lsFilename +=string(datetime(today(),now()),"MMDDYYYYHHMM") + ".txt"    //+ '.csv'
//lsFileNamePath = ProfileString(asInifile,asproject,"ftpfiledirout","")
//lsFileNamePath += '\' + lsFilename
//// log it
//lsLogOut = 'Confirmation Report Path & Filename: ' + lsFileNamePath
//FileWrite(gilogFileNo,lsLogOut)
//
////setup email string from activity schedule
//gu_nvo_process_files.uf_send_email(lsProject,asEmail, lsProject + " Garmin Daily Balance On Hand Run on " + string(Today(), 'YYYY/MM/DD HH:MM') + ' GMT', "Attached is the BALANCE ON HAND REPORT run on " + string(Today(), ' YYYY/MM/DD HH:MM') + ' GMT', lsFileNamePath)
...
on u_nvo_proc_garmin.create
call super::create
TriggerEvent( this, "constructor" )
end on

on u_nvo_proc_garmin.destroy
