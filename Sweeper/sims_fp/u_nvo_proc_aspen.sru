HA$PBExportHeader$u_nvo_proc_aspen.sru
forward
global type u_nvo_proc_aspen from nonvisualobject
end type
end forward

global type u_nvo_proc_aspen from nonvisualobject
end type
global u_nvo_proc_aspen u_nvo_proc_aspen
...
STRING lsfreight_terms
STRING lsShipToAddr1, lsShipToAddr2, lsShipToAddr3, lsShipToAddr4, lsShipToCity
STRING	lsShipToState, lsShipToZip, lsShipToCountry, lsShipToTel, lsShipToName
...
if liRtnImp < 0 then
	...
	//TAM 2015/09/16			
				...
				//If Freight Terms are Third Party then create a 3rd party address
				If 	Upper(lsFreight_Terms) = 'TPP' or Upper(lsFreight_Terms) = 'THIRDPARTY' Then
						llNewAddressRow = ldsDOAddress.InsertRow(0)
						ldsDOAddress.SetITem(llNewAddressRow,'project_id', lsProject) /*Project ID*/
						ldsDOAddress.SetItem(llNewAddressRow,'edi_batch_seq_no',llbatchseq) /*batch seq No*/
						ldsDOAddress.SetItem(llNewAddressRow,'order_seq_no',llOrderSeq) 
						ldsDOAddress.SetItem(llNewAddressRow,'address_type','3p') /* ThirdParty Address */
					//If we have Bill To Information, Use it for the 3P otherwise use the Ship_To Address
					If 	lbBillToAddress Then
						ldsDOAddress.SetItem(llNewAddressRow,'Name',lsBillToName)
						ldsDOAddress.SetItem(llNewAddressRow,'address_1',lsBillToAddr1)
						ldsDOAddress.SetItem(llNewAddressRow,'address_2',lsBillToAddr2)
						...
						ldsDOAddress.SetItem(llNewAddressRow,'tel',lsBillToTel)
					Else
						ldsDOAddress.SetItem(llNewAddressRow,'Name',lsShipToName)
						ldsDOAddress.SetItem(llNewAddressRow,'address_1',lsShipToAddr1)
						ldsDOAddress.SetItem(llNewAddressRow,'address_2',lsShipToAddr2)
						ldsDOAddress.SetItem(llNewAddressRow,'address_3',lsShipToAddr3)
						ldsDOAddress.SetItem(llNewAddressRow,'address_4',lsShipToAddr4)
						ldsDOAddress.SetItem(llNewAddressRow,'City',lsShipToCity)
						ldsDOAddress.SetItem(llNewAddressRow,'State',lsShipToState)
						ldsDOAddress.SetItem(llNewAddressRow,'Zip',lsShipToZip)
						ldsDOAddress.SetItem(llNewAddressRow,'Country',lsShipToCountry)
						ldsDOAddress.SetItem(llNewAddressRow,'tel',lsShipToTel)
					End If /*BT address exists*/
				End If /*THIRDPARTY Address Needed*/
...
on u_nvo_proc_aspen.create
call super::create
TriggerEvent( this, "constructor" )
end on

on u_nvo_proc_aspen.destroy
