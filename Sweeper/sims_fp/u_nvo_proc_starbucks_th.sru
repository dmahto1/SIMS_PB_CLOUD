HA$PBExportHeader$u_nvo_proc_starbucks_th.sru
forward
...
//
		...
		CASE 'OENTI' /* detail*/
			...
			//[Ravee 9 April] This is a VAT line generated from the SUN system.  Menlo only need to import the required data per IT$$HEX1$$1920$$ENDHEX$$s advice.  This one you can ignore."
			...
			//Item Code	29	15		Valid Item Code or blank for preset or $$HEX1$$1820$$ENDHEX$$-$$HEX2$$18202000$$ENDHEX$$plus any text for nonstocked item.	SKU	SIMS.SKU	this is sent back in the SKU in the RECADV	
			...
			//Purchase Quantity	85	19		+ or $$HEX2$$13202000$$ENDHEX$$with 5 decimal places number	Use Purchase Quantity/100, 000 and load into Quantity field 	"1. Is this the qty we are expected to receive?
...
//			
//			Purchase Quantity	85	19		+ or $$HEX2$$13202000$$ENDHEX$$with 5 decimal places number	NA	1. Is this the qty we are expected to receive?
...
//			Stock Quantity	104	19		+ or $$HEX2$$13202000$$ENDHEX$$with 5 decimal places number	Use Purchase Quantity/100, 000 and load into Quantity field 	What is this used for?	1. Please use this field for receive from supplier
...
//				gu_nvo_process_files.uf_write_log(lsLogOut) /*write to Screen*/
			...
			//Stock Quantity	104	19		+ or $$HEX2$$13202000$$ENDHEX$$with 5 decimal places number	NA	What is this used for?	we will put same purshase quntity	
...
//$$HEX2$$22200900$$ENDHEX$$It is assumed that each of the 170 stores will place 8 excel files in an FTP location, and Menlo Operations staff need to copy the 1360 Excel files into a designated Input folder.
//$$HEX2$$22200900$$ENDHEX$$As weekly files have 1 order date, on some days the demand or order qty will only come from daily files.
//$$HEX2$$22200900$$ENDHEX$$Until all 1360 Excel files are placed in the Input folder, Menlo Operation staff should not use the Excel macro. This can be ensured by checking how many objects are in the status bar. For example, below screenshot shows 10 files.
...
//  string aspath = "C:\118_Weekly_nonCDC_Inv_SM_Bru_Week_3-9_Jun_13.xls"
  ...
  //N, A, F or U.. depends on input filename.. e.g. if filename has $$HEX1$$1c20$$ENDHEX$$Daily_Fresh_Food$$HEX2$$1d202000$$ENDHEX$$or $$HEX1$$1c20$$ENDHEX$$Daily_Ambient$$HEX2$$1d202000$$ENDHEX$$or $$HEX1$$1c20$$ENDHEX$$Weekly_Inv_SM_Bru$$HEX2$$1d202000$$ENDHEX$$or $$HEX1$$1c20$$ENDHEX$$HVS_Daily_Inv_SM_Bru_Week$$HEX2$$1d202000$$ENDHEX$$then $$HEX1$$1c20$$ENDHEX$$N$$HEX1$$1d20$$ENDHEX$$.. If $$HEX1$$1c20$$ENDHEX$$AutoShip$$HEX2$$1d202000$$ENDHEX$$or $$HEX1$$1c20$$ENDHEX$$PromoCampaign$$HEX2$$1d202000$$ENDHEX$$or $$HEX1$$1c20$$ENDHEX$$Urgent$$HEX2$$1d202000$$ENDHEX$$then $$HEX1$$1c20$$ENDHEX$$A$$HEX1$$1d20$$ENDHEX$$.. and so on$$HEX1$$2620$$ENDHEX$$
...
//Weekly files include Weekly_Inv_SM_Bru Weekly_NonCDC_Inv_SM_Bru , AutoShip_SM, AutoShip_Inv and AutoShip_Bru. 
	...
	elseif Pos(Upper(aspath), "DAILY_AMBIENT") > 0 THEN   	//Daily_Ambient $$HEX2$$13202000$$ENDHEX$$The file name is of the format 999_Daily_Ambient_XXXXXXXXXX.xls where 999 is the 3-digit numeric store code and XXXXXXXXXX describes which week of the year. This file is outbound only.
	...
	elseif Pos(Upper(aspath), "WEEKLY_NONCDC_INV_SM_BRU") > 0 THEN   	//Weekly_nonCDC_Inv_SM_Bru $$HEX2$$13202000$$ENDHEX$$The file name is of the format 999_Weekly_nonCDC_Inv_SM_Bru XXXXXXXXXX.xls where 999 is the 3-digit numeric store code and XXXXXXXXXX describes which week of the year. This file is outbound only.
	...
	elseif Pos(Upper(aspath), "WEEKLY_INV_SM_BRU") > 0 THEN   	//Weekly_Inv_SM_Bru $$HEX2$$13202000$$ENDHEX$$The file name is of the format 999_Weekly_Inv_SM_Bru_XXXXXXXXXX.xls where 999 is the 3-digit numeric store code and XXXXXXXXXX describes which week of the year. This file is outbound only.
	...
	elseif Pos(Upper(aspath), "AUTOSHIP_SM") > 0 THEN   	//AutoShip_SM $$HEX2$$13202000$$ENDHEX$$The file name is of the format 999_ AutoShip_SM_XXXXXXXXXX.xls where 999 is the 3-digit numeric store code and XXXXXXXXXX describes which week of the year. This file is outbound only.
	...
	elseif Pos(Upper(aspath), "AUTOSHIP_INV") > 0 THEN   	//AutoShip_Inv $$HEX2$$13202000$$ENDHEX$$The file name is of the format 999_ AutoShip_Inv_XXXXXXXXXX.xls where 999 is the 3-digit numeric store code and XXXXXXXXXX describes which week of the year. This file is outbound only.
		...
		// a. For files containing $$HEX1$$1c20$$ENDHEX$$Weekly_HVS_Daily_Inv_SM_Bru$$HEX2$$1d202000$$ENDHEX$$for worksheet = $$HEX1$$1c20$$ENDHEX$$Weekly_Ambient_Bru$$HEX1$$1d20$$ENDHEX$$, order type = M. This is good.
		// b. For files containing $$HEX1$$1c20$$ENDHEX$$NONCDC_AutoShip$$HEX1$$1d20$$ENDHEX$$, the order type should be W, not A. This is good.
		// But for files containing $$HEX1$$1c20$$ENDHEX$$_CDC_AUTOSHIP$$HEX1$$1d20$$ENDHEX$$, the order type should remain as A. It was changed to W.
		//
		if Pos(Upper(aspath), "_CDC_AUTOSHIP") > 0 then
			lsOrderType = "A"
		else
			// $$HEX1$$1c20$$ENDHEX$$NONCDC_AutoShip$$HEX1$$1d20$$ENDHEX$$
			lsOrderType = "W"
		end if
	...
	elseif Pos(Upper(aspath), "AUTOSHIP_BRU") > 0 THEN   	//$$HEX2$$22200900$$ENDHEX$$AutoShip_Bru $$HEX2$$13202000$$ENDHEX$$The file name is of the format 999_ AutoShip_Bru_XXXXXXXXXX.xls where 999 is the 3-digit numeric store code and XXXXXXXXXX describes which week of the year. This file is outbound only.
	...
	elseif Pos(Upper(aspath), "WEEKLY_HVS_DAILY_INV_SM_BRU") > 0 THEN   	//Weekly_HVS_Daily_Inv_SM_Bru $$HEX2$$13202000$$ENDHEX$$The file name is of the format 999_Weekly_HVS_Daily_Inv_SM_Bru_XXXXXXXXXX.xlsx where 999 is the 3-digit numeric store code and XXXXXXXXXX describes which week of the year. This file is outbound only.
...
//	elseif Pos(Upper(aspath), "CDC_PROMOCAMPAIGN_INV") > 0 THEN   	//999_CDC_PromoCampaign_Inv $$HEX2$$13202000$$ENDHEX$$The file name is of the format 999_016_CDC_PromoCampaign_Inv_XXXXXXXXXX.xlsx where 999 is the 3-digit numeric store code and XXXXXXXXXX describes which week of the year. This file is outbound only.
...
//$$HEX2$$22200900$$ENDHEX$$There are 170 Starbucks Thailand stores in scope, each assigned a fixed 3 digit numeric code such as 005, 123, etc.
//$$HEX2$$22200900$$ENDHEX$$Each store will need to provide 8 types of file namely Daily_Fresh_Food, NonCDC_Weekly_Frozen, Daily_Ambient, Weekly_Inv_SM_Bru, Weekly_NonCDC_Inv_SM_Bru , AutoShip_SM, AutoShip_Inv and AutoShip_Bru.
//$$HEX2$$22200900$$ENDHEX$$The 8 file types will be classified as either daily (7 order dates in the worksheets) or weekly (only 1 order date in the file). Daily files include Daily_Fresh_Food, NonCDC_Weekly_Frozen and Daily_Ambient. NonCDC_Weekly_Frozen will be considered a daily file since there are 7 order dates in the worksheets. Weekly files include Weekly_Inv_SM_Bru Weekly_NonCDC_Inv_SM_Bru , AutoShip_SM, AutoShip_Inv and AutoShip_Bru.
//$$HEX2$$22200900$$ENDHEX$$For all worksheets in all 5 file types, the SKU or item code are in column B, from B10 onwards. The store code will be in cell C5.
...
//				end if
				...
				if trim(ls_current_sku) <> '' AND  ll_Qty > 0 then
					...
					if lb_generate_outbound then  /*Generate Outbound*/		
						...
						//			a.	For files containing $$HEX1$$1c20$$ENDHEX$$Weekly_HVS_Daily_Inv_SM_Bru$$HEX2$$1d202000$$ENDHEX$$for worksheet = $$HEX1$$1c20$$ENDHEX$$Weekly_Ambient_Bru$$HEX1$$1d20$$ENDHEX$$, order type = M.
...
//						lds_load_file.SetItem(li_sku_idx, "line_number", ll_line_no)
						 //Get QTY
						//$$HEX2$$22200900$$ENDHEX$$For daily files, order dates are in cells E8, F8, G8, H8, I8, J8, K8. The order qty are in columns E, F, G, H, I, J, K, from E10 onwards.
						lds_load_file.SetItem(li_sku_idx, "order_date", date(string(lole_worksheet.cells(8,li_exec_col).value)))
			...
			Do 
				...
				if trim(ls_current_sku) <> '' then // AND  ll_Qty > 0 then
					...
					lds_load_file.SetItem(li_sku_idx, "inbound_or_outbound", 'O')
					 ...
					 //
					//  			Outbound 
					//  			For these files, the order type should be M (Starbucks Marketing) for the $$HEX1$$1c20$$ENDHEX$$Weekly_Ambient_Bru$$HEX2$$1d202000$$ENDHEX$$xls tab. The SIMS GI file will not contain orders with order type = M 
...
//I think the key is going to be determining the rules for consolidating the Inbound and Outbound Orders. Since we won$$HEX1$$1920$$ENDHEX$$t be getting order numbers from them, we will need to determine how we determine 
...
//I think that we$$HEX1$$1920$$ENDHEX$$ll end up with one function per file type. We$$HEX1$$1920$$ENDHEX$$ll need to the file as an Excel document, parse specific fields for the store, dates, etc. and then loop through the rest of the rows for the line items. Some of the files just look like a shell of all the possible items and a lot of them don$$HEX1$$1920$$ENDHEX$$t have quantities so we won$$HEX1$$1920$$ENDHEX$$t want to create a line item if there is no qty.
...
//Rich>> it is 10chars $$HEX2$$13202000$$ENDHEX$$YMDHmm999X. ICC only process max 10chars. We want to save on chars so$$HEX1$$2620$$ENDHEX$$
//Y $$HEX2$$13202000$$ENDHEX$$right(yyyy) so for 2013 it is 3
//M - Mid("123456789ABC", Month(OrderDate), 1) $$HEX2$$13202000$$ENDHEX$$month of order date, didn$$HEX1$$1920$$ENDHEX$$t want to use 2 char for 10(oct), 11(nov), dec(12)
//D - Mid("123456789ABCDEFGHIJKLMNOPQRSTUV", Day(OrderDate), 1) 
//H - Mid("0123456789ABCDEFGHIJKLMN", Hour(systemtime + 1, 1) $$HEX2$$13202000$$ENDHEX$$if systertant, just make sure no duplicates per run.
//X $$HEX2$$13202000$$ENDHEX$$N, A, F or U.. depends on input filename.. e.g. if filename has $$HEX1$$1c20$$ENDHEX$$Daily_Fresh_Food$$HEX2$$1d202000$$ENDHEX$$or $$HEX1$$1c20$$ENDHEX$$Daily_Ambient$$HEX2$$1d202000$$ENDHEX$$or $$HEX1$$1c20$$ENDHEX$$Weekly_Inv_SM_Bru$$HEX2$$1d202000$$ENDHEX$$or $$HEX1$$1c20$$ENDHEX$$HVS_Daily_Inv_SM_Bru_Week$$HEX2$$1d202000$$ENDHEX$$then $$HEX1$$1c20$$ENDHEX$$N$$HEX1$$1d20$$ENDHEX$$.. If $$HEX1$$1c20$$ENDHEX$$AutoShip$$HEX2$$1d202000$$ENDHEX$$or $$HEX1$$1c20$$ENDHEX$$PromoCampaign$$HEX2$$1d202000$$ENDHEX$$or $$HEX1$$1c20$$ENDHEX$$Urgent$$HEX2$$1d202000$$ENDHEX$$then $$HEX1$$1c20$$ENDHEX$$A$$HEX1$$1d20$$ENDHEX$$.. and so on$$HEX1$$2620$$ENDHEX$$
//o	Can an Outbound Order contain SKUm time is 1205am, then 0; if system time is 2344, then V
//mm $$HEX2$$13202000$$ENDHEX$$minute of system time. If 12:44, then 44
//999 $$HEX2$$13202000$$ENDHEX$$running seq per run.. not impos with different suppliers or do we need a order per Store/Supplier combination? CP> Rich..
...
//1	Year of order date (last number)	3 for 2013, 4 for 2014$$HEX2$$26202000$$ENDHEX$$A for 2020, B for 2021$$HEX1$$2620$$ENDHEX$$
//2	Month of order date (last number)	1 for Jan, 2 for Feb$$HEX2$$26202000$$ENDHEX$$A for Oct, B for Nov, C for Dec
...
//5		
//6		
//7	3-digit running number for each file	001, 002, 003 $$HEX1$$2620$$ENDHEX$$
//8		
//9		
