HA$PBExportHeader$u_nvo_proc_anki.sru
forward
global type u_nvo_proc_anki from nonvisualobject
end type
end forward

global type u_nvo_proc_anki from nonvisualobject
end type
global u_nvo_proc_anki u_nvo_proc_anki
...
public function integer uf_process_delivery_order (string aspath, string asproject);//Jxlim 08/26/2014  Anki
...
public function integer uf_process_dboh (string asproject, string asinifile);//Jxlim 08/28/2014 Anki  Inventory Snapshot at15 minutes cycle
...
//Jxlim 09/25/2014 Anki required 0 qty to be included on BOH
//ldsboh.Dataobject = 'd_baseline_unicode_dboh'
ldsboh.Dataobject = 'd_anki_dboh'
lirc = ldsboh.SetTransobject(sqlca)

lsLogOut = "- PROCESSING FUNCTION: "+asproject+" Inventory Snapshot!"
FileWrite(gilogFileNo,lsLogOut)
...
For llRowPos = 1 to llRowCOunt
	...
	lsWarehouse = upper(left(ldsboh.GetItemString(llRowPos,'wh_code'),10)) //convert to Upper case
	...
	ldQty = ldsboh.GetItemNumber(llRowPos,'avail_qty')
	...
	ldQty = ldsboh.GetItemNumber(llRowPos,'alloc_qty')
...
//	BHYYMDD.dat
	...
	//lsFilename = ("BH" + string(today(), "YYMMDD") + lsWarehouse + ".dat") //09-Oct-2014 :Madhu- commented
      lsFilename = ("BH" + String(DateTime(Today(),Now()),'yyyymmddhhss')  + lsWarehouse + ".dat") //09-Oct-2014 :Madhu- Added datetime full format

	
	ldsOut.SetItem(llNewRow,'file_name', lsFilename)
	ldsOut.SetItem(llNewRow,'Project_id', lsproject)
	
	if upper(lsLastWarehouse) <> upper(lsWarehouse) then
...
//write to log file
lsLogOut = lsFileName +" Datastore record count is: "+ string(ldsOut.rowcount( ))
...
gu_nvo_process_files.uf_send_email(lsProject,"BOHEMAIL", lsProject + "  Anki Inventory Snapshot Run on " + string(Today(), 'YYYY/MM/DD HH:MM') + ' GMT', "Attached is ANKI Inventory Snapshot run on" + string(Today(), 'YYYY/MM/DD HH:MM') + ' GMT', lsFileNamePath)
...
public function integer uf_load_delivery_order (string aspath, string asproject, ref datastore asgenericimport, ref datastore asdeliverymaster, ref datastore asdeliverydetail, ref datastore asdeliveryaddress, ref datastore asdeliverynotes);//Jxlim 08/26/2014 Anki
...
BOOLEAN lbError, lbNew, lbDetailError
LONG llCount, llNew, llOwner, llexist, llOrderSeq, llLineSeq, llbatchseq, llLineItemNo 
LONG llNewAddressRow, llNoteSequence, llNewNotesRow, ll_priority
INTEGER lirc, liRtnImp, liNewRow, llNewDetailRow
STRING lsLogOut, lsChangeID
INTEGER li_StartCol
INTEGER li_UFIdx
DECIMAL ldQuantity, ldTemp
...
STRING ls_InventoryType, lsNoteType, lsNoteText,ls_Carrier,  ls_transport_mode
STRING lsFromcountry,lsTocountry	//06-Jan-2016 :Madhu - Hard code TRAX values
...
if liRtnImp < 0 then
	...
	gu_nvo_process_files.uf_writeError("***Unable to Open delivery order File for "+asproject+" Processing: " + asPath + " Import Error: " + string(liRtnImp))
	lbError = TRUE
else
	...
	for llFileRowPos = 1 to llFilerowCount
		...
		Choose Case Upper(lsTemp)
			...
			Case 'DM' /* Header */
				...
				//DM003 Project ID	C(10)	Yes	N/A	Project identifier			
				lsTemp = Upper(Trim(ldsImport.GetItemString(llFileRowPos, "col3")))	
				...
				//DM004 Warehouse	C(10)	Yes	N/A	Receiving Warehouse
				lsTemp = Upper(Trim(ldsImport.GetItemString(llFileRowPos, "col4")))		
				...
				//DM005 Delivery Number	C(20)	Yes	N/A	Delivery Order Number			
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col5"))		
				...
				//DM006 Order Date	Date	No	N/A	Order Date			
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col6"))	
				
				If IsNull(lsTemp) OR trim(lsTemp) = '' Then
				Else
					ls_OrderDate = lsTemp   //DM.ord_Date
				End If				
				
				
				//DM007 Delivery Date	Date	No*	*	Date for delivery to Customer
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col7"))	
				
				If IsNull(lsTemp) OR trim(lsTemp) = '' Then
				Else
					ls_DeliveryDate = lsTemp	//DM.Delivery_date
				End If				
				
				//DM008 GI Date	Date	No*	*	Planned goods ship date from warehouse	//this date is supply by ICC to Sims and map to GI and Schedule date			
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col8"))				
				
				If IsNull(lsTemp) OR trim(lsTemp) = '' Then
				Else
					ls_GI_Date = lsTemp		//DM.Schedule_date	
					If len(ls_GI_Date) >= 8 then
						//This is a string (edi outbound (char), added "-" for date recognition. Other wise u_nvo_process_file,uf_process_delivery_order() will set to warehouse time because it didn't think it isdate()
						 ls_GI_Date = left(ls_GI_date,4) + "-" + mid(ls_gi_date,5,2) + "-" +  mid(ls_gi_date,7,2)					
					End If 	
				End If				
				
				
				//DM009 Customer Code	C(20)	Yes	N/A	Customer ID		
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col9"))	
	...
	//				Continue		
	//			Else
					lsCustomerCode = lsTemp  //DM.cust_code
	//			End If		
	...
	//			
				...
				//DM010 Order Type	C(1)	No	N/A	Order Type			
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col10"))	
				
				If IsNull(lsTemp) OR trim(lsTemp) = '' Then
					lsTemp = "S"	
				Else
					lsOrderType = lsTemp			//DM.Ordr_type				
					ldsSOheader.SetItem(liNewRow,'Order_type',lsOrderType) /*Order Type*/									
				End If					
									
				//DM011 Customer Order #	C(20)	No	N/A	Customer Order Number			
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col11"))		
		
				If NOT(IsNull(lsTemp) OR trim(lsTemp) = '') Then
					ldsSOheader.SetItem(liNewRow,'order_no', lsTemp)		//DM.Order_no JXLIM ANKI?  EDI header?
				End If		
										
				//DM012 Carrier	C(20)	No	N/A			
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col12"))	
		
				If NOT(IsNull(lsTemp) OR trim(lsTemp) = '') Then
					ldsSOheader.SetItem(liNewRow,'carrier', lsTemp)		//DM.Carrier
					ls_carrier= lsTemp
				End If					
						
				//DM013 Transport Mode	C10)	No	N/A				
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col13"))  
					
				If NOT(IsNull(lsTemp) OR trim(lsTemp) = '') Then
					ldsSOheader.SetItem(liNewRow,'transport_mode', lsTemp)  //DM.tranport_mode
				Else
					//Concatenate Freight Carrier Identifier (4 AN), Freight Carrier Mode (3 AN) and Freight Carrier Service (3 AN)
					...
					if Len(ls_transport_mode) > 0 then
						ldsSOheader.SetItem(liNewRow,'transport_mode',  ls_transport_mode)
					end if
				End If
	
				//DM014 Ship Via	C(15)	No	N/A	
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col14"))  //DM014 Ship Via
		
				If NOT(IsNull(lsTemp) OR trim(lsTemp) = '') Then
					ldsSOheader.SetItem(liNewRow,'ship_via', lsTemp)		//DM.Ship_via
				End If				
				
				//DM015 Freight Terms	C(20)	No	N/A	Values are:
				//PPD = Prepaid by Shipper
				//PPA = Prepaid by Shipper/Add
				//COL = Collect from Consignee"
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col15"))  
		
				If NOT(IsNull(lsTemp) OR trim(lsTemp) = '') Then
					ldsSOheader.SetItem(liNewRow,'freight_terms', lsTemp)	//DM.Freight_terms
				End If			
				
				//DM016 Agent Info	C(30)	No	N/A				
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col16"))		
		
				If NOT(IsNull(lsTemp) OR trim(lsTemp) = '') Then
					ldsSOheader.SetItem(liNewRow,'agent_info', lsTemp)	//DM.Agent_info
				End If				
				
				
				 //DM017 Ship To Name	C(50)	No	N/A				
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col17"))  
		
				If NOT(IsNull(lsTemp) OR trim(lsTemp) = '') Then
					ldsSOheader.SetItem(liNewRow,'cust_name', lsTemp)	//DM.Cust_name
				End If				
					
				
				//DM018 Ship Address 1	C(60)	No	N/A				
				...
				//DM019 Ship Address 2	C(60)	No	N/A	
				...
				//DM020 Ship Address 3	C(60)	No	N/A	
				...
				//DM021 Ship Address 4	C6)	No	N/A				
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col21"))
		
				If NOT(IsNull(lsTemp) OR trim(lsTemp) = '') Then
	// TAM 2015/11/18 - ****TEMP****  sending a Shipping Instructions in Address 4.  This is temporary to get us through the Holidays in 2015(ICC and ANKI do not have time to make a mapping changed so they are sending a value we need in ADDRESS4
	//				ldsSOheader.SetItem(liNewRow,'address_4', lsTemp)
					ldsSOheader.SetItem(liNewRow,'shipping_instructions_Text', lsTemp)
				End If				
				
				//DM022 Ship City	C(50)	No	N/A				
				...
				//DM024 Ship Postal Code	C(50)	No	N/A				
				...
				//DM025 Ship Country	C(50)	No	N/A				
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col25"))
		
				If NOT(IsNull(lsTemp) OR trim(lsTemp) = '') Then
					ldsSOheader.SetItem(liNewRow,'country', lsTemp)
					lsTocountry =lsTemp //06-Jan-2016 :Madhu Added -Hard cod Trax values
				End If				
				
				//DM026 Ship Tel	C(20)	No	N/A				
				...
				//DM027 Bill To Name	C(50)	No	N/A			
				...
				//DM028 Bill Address 1	C(60)	No	N/A				
				...
				//DM029 Bill Address 2	C(60)	No	N/A				
				...
				//DM030 Bill Address 3	C(60)	No	N/A				
				...
				//DM031 Bill Address 4	C(60)	No	N/A				
				...
				//DM034 Bill Postal Code	C(50)	No	N/A			
				...
				//DM035 Bill Country	C(50)	No	N/A			
				...
				//DM038 Shipping Instructions	C(255)	No	N/A			
	...
	// TAM 2015/11/18 - ****TEMP****  sending a Shipping Instructions in Address 4.  This is temporary to get us through the Holidays in 2015(ICC and ANKI do not have time to make a mapping changed so they are sending a value we need in ADDRESS4
				...
				//DM039 Packlist Notes	C(255)	No	N/A			
				...
				//User Field2	C(10)	No	N/A	User Field  
				//User Field3	C(10)	No	N/A	User Field  //Anki ASN destination
				//User Field4	C(20)	No	N/A	User Field
				//User Field5	C(20)	No	N/A	User Field
				//User Field6	C(20)	No	N/A	User Field  //Client order Type Magento
				//User Field7	C(30)	No	N/A	User Field  //Priority 50=normal
				...
				//DM045 Priority dm.User_field7	C(30)	No	N/A		//ICC pass priority value to Sims on DM045 dm.user_field7, Sims will store this value in dm.priority field and not dm.user_field7
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col45"))
				ll_priority = Long(lsTemp)			
		
				If NOT(IsNull(lsTemp) OR trim(lsTemp) = '') Then
					ldsSOheader.SetItem(liNewRow,'user_field7', '')  //clear put uf7
					ldsSOheader.SetItem(liNewRow,'priority', ll_priority)
				End If			
				
				//DM057 Ship To Contact	C(30)	No	N/A			
				...
				//DM058 Ship To Email		C(50)	No	N/A			
				...
				//DM052 User_Field14			Anki Ecommerce ID -non unique order number that could go longer than 50 lenght
	...
	//TAM 2015/09/16			
				...
				If NOT(IsNull(lsTemp) OR trim(lsTemp) = '') Then
					ldTemp = Long(lsTemp)			
					ldsSOheader.SetItem(liNewRow,'Order_Tax_Amt', ldTemp)
				End If			
				...
				If NOT(IsNull(lsTemp) OR trim(lsTemp) = '') Then
					ldTemp = Long(lsTemp)			
					ldsSOheader.SetItem(liNewRow,'Order_Discount_Amt', ldTemp)
				End If			
				...
				If NOT(IsNull(lsTemp) OR trim(lsTemp) = '') Then
					ldTemp = Long(lsTemp)			
					ldsSOheader.SetItem(liNewRow,'Shipping_Handling_Amt', ldTemp)
				End If			
	
				//END ADDITION OF NAMED FIELDS
				
				//06-Jan-2016 :Madhu Hard code TRAX values, if order is from USA to CA -START
				select Country into :lsFromcountry from Warehouse with(nolock) where wh_code=:lsWarehouse;
				
				IF left(lsFromcountry,2)='US' and lsTocountry='CA' THEN
					ldsSOheader.SetItem(liNewRow,'freight_terms', 'THIRDPARTY')
					ldsSOheader.SetItem(liNewRow,'Trax_Acct_No', 'F9Y624')
					ldsSOheader.SetItem(liNewRow,'Trax_Duty_Terms', 'THIRDPARTYDUTYVAT')
					ldsSOheader.SetItem(liNewRow,'Trax_Duty_Acct_No', '0798AW')
				END IF
				//06-Jan-2016 :Madhu Hard code TRAX values, if order is from USA to CA -END
			...
			//Delivery Order Detail
				...
				//DD003 Project ID	C(10)	Yes	N/A	Project identifier			
				lsTemp = Upper(Trim(ldsImport.GetItemString(llFileRowPos, "col3")))
				...
				//DD004 Delivery Number	C(20)	Yes	N/A	Delivery Order Number (must match header)
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col4"))
				...
				//DD005 Supplier Code	C(20)	Yes	N/A	//Default Supplier to 'ANKI'
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col5"))
	...
	//				Continue
				...
				//DD006 Line Number	N(6,0)	Yes	N/A	Delivery line item number  //this line item  no does not mean anything to Anki and it is not enough lenght, it will not be used by Anki
				//Anki uses user_field2 for Fullfilment line item number
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col6"))
	//			
	//			If IsNull(lsTemp) OR trim(lsTemp) = '' Then
	//				gu_nvo_process_files.uf_writeError("Row: " + string(llFileRowPos) + " - Line Item Number is required. Record will not be processed.")
	...
	//			End If					
				...
				//DD007 SKU	C(50)	Yes	N/A	Material number	
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col7"))
				...
				//DD008 Quantity	N(15,5)	Yes	N/A	Requested delivery quantity
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col8"))
				...
				//DD009 Inventory Type	C(1)	Yes	N/A	Inventory Type	Anki default to 'N'	
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col9"))
				...
				ldsSODetail.SetItem(llNewDetailRow,'Inventory_Type',ls_InventoryType) //default to 'N' 
	...
	//			ldsSODetail.SetItem(llNewDetailRow,'action_cd',lsChangeID) /*Supplier Order*/	
				...
				//DD006 Line Item No
				ldsSODetail.SetItem(llNewDetailRow,'Line_Item_No', llLineSeq)		//DD006 sims line item no is Sims required field. However Anki requier longer lenght, thus this field is not used for Anki	
	
				//DD010 Customer  Line Item Number	C(20)	No	N/A	Anki E-commerce linet item no
				//DD.Line_Item_No or DD.User Line_Item_No  it can be sent by client or Sims based on customer need. 
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col10"))
				
				If NOT(IsNull(lsTemp) OR trim(lsTemp) = '') Then
					ldsSODetail.SetItem(llNewDetailRow, 'User_Line_Item_No', lsTemp)  //Anki E-commerce line item no
				End If		
				
				//DD019 DD.User_field2 Anki fullfillment Line Item No
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col19"))
				If NOT(IsNull(lsTemp) OR trim(lsTemp) = '') Then
					ldsSODetail.SetItem(llNewDetailRow,'user_field2',lsTemp) /*Anki Fullfilment Line Item No*/
				End If
			
				//DD011 Alternate SKU	C(50)	No	N/A	Alternate SKU			
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col11"))
				...
				//DD012 Customer SKU	C(35)	No	N/A	Customer/Alternate SKU			
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col12"))
				...
				//DD013 Lot Number	C(50)	No	N/A	1st User Defined Inventory field			
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col13"))
				...
				//DD014 PO NBR	C(50)	No	N/A	2nd User Defined Inventory field			
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col14"))
				...
				//DD015 PO NBR 2	C(50)	No	N/A	3rd User Defined Inventory Field			
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col15"))
				...
				//DD016 Serial Number	C(50)	No	N/A	Qty must be 1 if present			
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col16"))
				...
				//DD017 Line Item Text	C(255)	No	N/A	Notes / remarks			
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col17"))
		
				If NOT(IsNull(lsTemp) OR trim(lsTemp) = '') Then
					ldsSODetail.SetItem(llNewDetailRow,'line_item_notes', lsTemp)  //DD.line_item_notes
				End If						
				
				//User Field1	C(20)	No	N/A	User Field
				//User Field2	C(20)	No	N/A	User Field	//Anki Fulfillment Order Item ID (Line Number)
				...
				Else
	// TAM 2015/09/24  -  Get Cost from Item master if not sent.
					Select min(std_cost) into :ldTemp
					From Item_MAster
					Where PRoject_ID = :asProject and sku = :lsSku;
					If ldtemp > 0 then 
						ldsSODetail.SetItem(llNewDetailRow,'price', string(ldTemp))
					End If
				End If					
				
				
	// TAM 2015/0916  -  Added missing fields to ANKI  ****TEMP COMMENT OUT UNTIL READY TO MOVE TO PROD
	...
	//			//TAM 2015/05/ Col 32 Unused - GWM Moved UPC from 36 to 32 to sync with structure
				...
				If NOT(IsNull(lsTemp) OR trim(lsTemp) = '') Then
					ldTemp = Long(lsTemp)			
					ldsSODetail.SetItem(llNewDetailRow,'Line_Total_Amt', ldTemp)
				End If	
				...
				If NOT(IsNull(lsTemp) OR trim(lsTemp) = '') Then
					ldTemp = Long(lsTemp)			
					ldsSODetail.SetItem(llNewDetailRow,'Line_Tax_Amt', ldTemp)
				End If	
				...
				If NOT(IsNull(lsTemp) OR trim(lsTemp) = '') Then
					ldTemp = Long(lsTemp)			
					ldsSODetail.SetItem(llNewDetailRow,'Line_Discount_Amt', ldTemp)
				End If	
			...
			Case 'DN' /* Note */ 
				...
				//DN003 Project ID	C(10)	Yes	N/A	Project identifier			
				...
				//DN004 Delivery Number	C(20)	Yes	N/A	Delivery Order Number (must match header)
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col4"))
				...
				//DN005 Line Number	N(6,0)	Yes	N/A	Delivery line item number  /* Anki does not use this field
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col5"))
	...
	//				Continue		
	//			Else
					llLineItemNo = Long(lsTemp)   //DD.Line_Item_No or DD.User Line_Item_No  it can be send by client or Sims based on customer need
	//			End If				
				
				//DN006 Note Type	C(2)	Yes	N/A	Delivery note type...$$HEX1$$1820$$ENDHEX$$WH$$HEX1$$1920$$ENDHEX$$, $$HEX1$$1820$$ENDHEX$$BL$$HEX1$$1920$$ENDHEX$$, $$HEX1$$1820$$ENDHEX$$PK$$HEX1$$1920$$ENDHEX$$, or $$HEX1$$1820$$ENDHEX$$LB$$HEX1$$1920$$ENDHEX$$
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col6"))
				...
				If IsNull(lsTemp) OR trim(lsTemp) = '' Then
					...
					Continue		
				Else
				//Jxlim 09/12/2014 Anki Note_Type is GF (GFT)
				//	IF lsTemp = 'WH' OR lsTemp = 'BL' OR lsTemp = 'PK' OR lsTemp = 'LB'  OR lsTemp = 'MR'  THEN
				IF lsTemp = 'GF'  THEN
						lsNoteType = lsTemp
					ELSE
				...
				//DN007 Note Sequence	N(5,0)	Yes	N/A	Delivery note sequence
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col7"))
				...
				//DN008 Note Text	C(255)	Yes	N/A	Delivery note text.
				lsTemp = Trim(ldsImport.GetItemString(llFileRowPos, "col8"))
...
if liRtnImp < 0 then
	lsLogOut = "-       ***Unable to Open Purchase Order File for "+asproject+" Processing: " + asPath + " Import Error: " + string(liRtnImp)
	gu_nvo_process_files.uf_writeError("Unable to Open Purchase Order File for "+asproject+" Processing: " + asPath + " Import Error: " + string(liRtnImp))
	lbError = TRUE
else
...
on u_nvo_proc_anki.create
call super::create
TriggerEvent( this, "constructor" )
end on

on u_nvo_proc_anki.destroy
