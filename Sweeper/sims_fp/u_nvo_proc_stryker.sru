HA$PBExportHeader$u_nvo_proc_stryker.sru
forward
...
//1.	IN EDI $$HEX2$$13202000$$ENDHEX$$Inbound orders EDI
//a)	This EDI comes into Menlo AFTER Menlo ops scanned the SKUs, lot#/ serial#, expiry and MFG dates and uploaded into Stryker$$HEX1$$1920$$ENDHEX$$s system (called the Barcode Web).
//b)	There are 3 sections in the EDI $$HEX2$$13202000$$ENDHEX$$header, detail and Lot detail.
...
//f)	For inbound, after we confirm the inbound orders, there$$HEX1$$1920$$ENDHEX$$s no need to send back receipt confirmation. From pt #1 above, the scanned data serves as confirmation to Stryker that the receipt is completed.
...
For llRowPos = 1 to llRowCount
	...
	//	K2TRAN	Transaction type	2		29	30	e.g. IN, OT,$$HEX2$$26200900$$ENDHEX$$Ok	"Inbound: First 2 characters of order Nbr
...
//Other Fields
//1. user field 1 $$HEX2$$13202000$$ENDHEX$$this is used as MRP flag. For item that needs to print MRP label, this is $$HEX1$$1c20$$ENDHEX$$Y$$HEX1$$1d20$$ENDHEX$$, else $$HEX1$$1c20$$ENDHEX$$N$$HEX1$$1d20$$ENDHEX$$.						
//2. user field 4 $$HEX2$$13202000$$ENDHEX$$this is used for $$HEX1$$1c20$$ENDHEX$$Imp Lic No.$$HEX1$$1d20$$ENDHEX$$. 						
//3. user field 5 - Vendor Code. 						
//4. user field 6 $$HEX2$$13202000$$ENDHEX$$Vendor Name						
...
//a)	Stryker will generate this EDI file to Menlo AFTER Menlo ops scanned the lot#/ Serial#, expiry and MFG dates and uploaded to Stryker$$HEX1$$1920$$ENDHEX$$s Barcode Web.
//b)	Please advise if this is possible $$HEX2$$e8f02000$$ENDHEX$$The picked inventory$$HEX1$$1920$$ENDHEX$$s lot # from 2d above to be updated into the inventory table.
...
//2.	PK EDI $$HEX2$$13202000$$ENDHEX$$Outbound orders
//a)	Stryker planner will generate this EDI to us, and the outbound order is to be created in SIMS based on this PK file.
//b)	Same as IN $$HEX2$$e8f02000$$ENDHEX$$The field = Warehouse No. will indirectly determine the inventory types, which can be Saleable, Demo or Loaner. Stryker will provide this list soon.
//c)	Since items are not serialized, we need not have serial# printed on the picking list. We$$HEX1$$1920$$ENDHEX$$ll pick by FEFO, followed by FIFO.
//d)	If there are more than 2 inventory with the same expiry and receipt dates but different lot#, Menlo ops may pick any of these inventory. So the actual picked inventory$$HEX1$$1920$$ENDHEX$$s lot# may be different from the picking list. The actual lot# will be scanned and uploaded into Stryker$$HEX1$$1920$$ENDHEX$$s system.
