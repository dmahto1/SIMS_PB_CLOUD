HA$PBExportHeader$u_nvo_proc_philips_sg.sru
forward
global type u_nvo_proc_philips_sg from nonvisualobject
end type
end forward

global type u_nvo_proc_philips_sg from nonvisualobject
end type
global u_nvo_proc_philips_sg u_nvo_proc_philips_sg
...
public function boolean uf_otm_fields_modified (string as_action, datastore ads_current, datastore ads_original, long al_row)
end prototypes
...
For llRowPos = 1 to llRowCount 
	...
	Choose Case Upper(lsRecType)
		...
		//	ldsDOHeader.SetITem(llNewRow,'wh_code',lswarehouse) /*Default WH for Project */
			...
			ldsDOHeader.SetItem(llNewRow,'otm_status','R')
			...
			If upper(Trim(lsTemp)) = "MY10"  OR  upper(Trim(lsTemp)) = "MYQ0"   Then /*Malaysia*/
...
For llfileRowPos = 1 to llFileRowCOunt
	...
	//05/13 - MEA - Added SG10, SG27, SG71 and MY10
	...
	ldsItem.RowsCopy(1, ldsItem.RowCount(), Primary!, ldsItemOrig, 1, Primary!)		// MEA 3/13	OTM addition
...
//		ldsItem.SetItem(1,'lot_controlled_ind','N')   No longer default to 'N' - cawikholm 05/24/11
	...
	If liRC = 1 then
		...
		//MikeA 04/12 OTM Project Added OTM Flag
		If gs_OTM_Flag = 'Y' then 
			...
			if lb_otm_item_turned_on then
				...
				For llPos = 1 to ldsItem.RowCount()
					...
					//Only send for inserts
								
					if ls_action = 'I' then // uf_otm_fields_modified(ls_action,ldsItem,ldsItemOrig,llPos) then
						//	Send to OTM
						...
						//Comment out once everything is working.
...
For llRowPos = 1 to llRowCount
	...
	Choose Case Upper(lsRecType)
		...
		//	idsPOheader.SetItem(llNewRow, 'action_cd', lsTemp) /* defaulting to add above */
			...
			// Warehouse is being set based on the Plant Code - hardcoded for now
			If upper(Trim(lsTemp)) = "MY10"   OR  upper(Trim(lsTemp)) = "MYQ0"  Then  /*Malaysia*/
				idsPOheader.SetItem(llNewRow, 'wh_code',"PHILIPS-MY") 
			Else /*Singapoore */
				idsPOheader.SetItem(llNewRow, 'wh_code',"PHILIPS") 
			End If
...
sql_syntax += " Where Project_ID = 'PHILIPS-SG'"
...
For llRowPos = 1 to llRowCOunt
	...
	If lsSupplier <> lsSupplierSave Then
		...
		If ldsOut.RowCount() > 0 Then
			gu_nvo_process_files.uf_process_flatfile_outbound(ldsOut,'PHILIPS-SG')
			ldsOut.Reset()
		End If
	...
	ldsOut.SetItem(llNewRow,'Project_id', 'PHILIPS-SG')
...
lsLogOut = "      Creating PHILIPS-SG Daily Balance on Hand-Volume File... " 
FileWrite(gilogFileNo,lsLogOut)
...
sql_syntax += "  Where Content_Summary.Project_id = 'PHILIPS-SG'  and "
...
IF Len(ERRORS) > 0 THEN
 	 lsLogOut = "        *** Unable to create datastore for PHILIPS-SG  (BOH-Volume Data).~r~r" + Errors
	FileWrite(gilogFileNo,lsLogOut)
 	 RETURN - 1
END IF
...
For llRowPos = 1 to lLRowCount /*Each Inv Record*/
	...
	If ldsBoh.GetITemString(llRowPos,'wh_code') <> lsWarehouseSave Then
		...
		lsFileName = "PHILIPS-SG-Daily-BOH-Volume-" + ldsBoh.GetITemString(llRowPos,'wh_code')  + "-" + + String(ldToday,"mm-dd-yyyy") + ".csv"
...
public function boolean uf_otm_fields_modified (string as_action, datastore ads_current, datastore ads_original, long al_row);// Checks to determine if required OTM fields have been modified and thus the item needs to be sent to TIBCO/OTM.  
...
on u_nvo_proc_philips_sg.create
call super::create
TriggerEvent( this, "constructor" )
end on

on u_nvo_proc_philips_sg.destroy
