HA$PBExportHeader$u_nvo_proc_tpv.sru
$PBExportComments$Process TPV files
forward
global type u_nvo_proc_tpv from nonvisualobject
end type
end forward

global type u_nvo_proc_tpv from nonvisualobject
end type
global u_nvo_proc_tpv u_nvo_proc_tpv
...
If liFileNo < 0 Then
	lsLogOut = "-       ***Unable to Open File for TPV Processing: " + asPath
	FileWrite(giLogFileNo,lsLogOut)
	gu_nvo_process_files.uf_write_log(lsLogOut) /*write to Screen*/
...
For llRowPos = 1 to llRowCount 
	...
	Choose Case Upper(lsRecType)
		...
		//	ldsDOHeader.SetITem(llNewRow,'wh_code',lswarehouse) /*Default WH for Project */
			...
			If upper(Trim(lsTemp)) = "2180"   Then /*Malaysia*/
				ldsDOHeader.SetItem(llNewRow, 'wh_code',"TPV-MY") 
			Else /*Singapoore */
				ldsDOHeader.SetItem(llNewRow, 'wh_code',"TPV-SG") 
			End If
...
public function integer uf_process_itemmaster (string aspath, string asproject);
//Process Item Master (IM) Transaction for TPV (12/12 - PCONKL - Cloned from Philips-sg)
...
liFileNo = FileOpen(asPath,LineMode!,Read!,LockReadWrite!)
If liFileNo < 0 Then
	lsLogOut = "-       ***Unable to Open Item Master File for TPV Processing: " + asPath
	FileWrite(giLogFileNo,lsLogOut)
	gu_nvo_process_files.uf_write_log(lsLogOut) /*write to Screen*/
	Return -99 /* we wont move to error directory if we can't open the file here*/
End If
...
For llfileRowPos = 1 to llFileRowCOunt
	...
	// 07/13 - PCONKL - Block for all Plant Codes
...
//Open and read the File In
lsLogOut = '      - Opening File for TPV Purchase Order Processing: ' + asPath
FileWrite(giLogFileNo,lsLogOut)
gu_nvo_process_files.uf_write_log(lsLogOut) /*write to Screen*/
...
For llRowPos = 1 to llRowCount
	...
	Choose Case Upper(lsRecType)
		...
		//	idsPOheader.SetItem(llNewRow, 'action_cd', lsTemp) /* defaulting to add above */
			...
			// Warehouse is being set based on the Plant Code - hardcoded for now
			If  upper(Trim(lsTemp)) = "2180"  Then  /*Malaysia*/
				idsPOheader.SetItem(llNewRow, 'wh_code',"TPV-MY") 
			Else /*Singapoore */
				idsPOheader.SetItem(llNewRow, 'wh_code',"TPV-SG") 
			End If
...
lsLogOut = "- PROCESSING FUNCTION: TPV Daily Inventory Snapshot File... " 
FileWrite(gilogFileNo,lsLogOut)
gu_nvo_process_files.uf_write_log(lsLogOut) /*display msg to screen*/
lsLogOut = ""
FileWrite(gilogFileNo,lsLogOut)
...
sql_syntax += " Where Project_ID = 'TPV'"
...
IF Len(ERRORS) > 0 THEN
   lsLogOut = "        *** Unable to create datastore for TPV Inventory Snapshot ID data.~r~r" + Errors
	FileWrite(gilogFileNo,lsLogOut)
   RETURN - 1
END IF
...
For llRowPos = 1 to llRowCOunt
	...
	If lsSupplier <> lsSupplierSave Then
		...
		If ldsOut.RowCount() > 0 Then
			gu_nvo_process_files.uf_process_flatfile_outbound(ldsOut,'TPV')
			ldsOut.Reset()
		End If
		
		//Next File Sequence #...
		ldBatchSeq = gu_nvo_process_files.uf_get_next_seq_no("TPV",'EDI_Generic_Outbound','EDI_Batch_Seq_No')
		If ldBatchSeq <= 0 Then
			lsLogOut = "        *** Unable to retrieve the next available sequence number for TPV BOH file. Confirmation will not be sent'"
			FileWrite(gilogFileNo,lsLogOut)
			Return -1
		End If
	...
	ldsOut.SetItem(llNewRow,'Project_id', 'TPV')
...
lsLogOut = "      Creating TPV Daily Balance on Hand-Volume File... " 
FileWrite(gilogFileNo,lsLogOut)
...
sql_syntax += "  Where Content_Summary.Project_id = 'TPV'  and "
...
IF Len(ERRORS) > 0 THEN
 	 lsLogOut = "        *** Unable to create datastore for TPV  (BOH-Volume Data).~r~r" + Errors
	FileWrite(gilogFileNo,lsLogOut)
 	 RETURN - 1
END IF
...
For llRowPos = 1 to lLRowCount /*Each Inv Record*/
	...
	If ldsBoh.GetITemString(llRowPos,'wh_code') <> lsWarehouseSave Then
		...
		lsFileName = "TPV-Daily-BOH-Volume-" + ldsBoh.GetITemString(llRowPos,'wh_code')  + "-" + + String(ldToday,"mm-dd-yyyy") + ".csv"
...
on u_nvo_proc_tpv.create
call super::create
TriggerEvent( this, "constructor" )
end on

on u_nvo_proc_tpv.destroy
