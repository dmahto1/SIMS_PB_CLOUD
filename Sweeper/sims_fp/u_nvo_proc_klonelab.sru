HA$PBExportHeader$u_nvo_proc_klonelab.sru
forward
...
//$$HEX2$$a7f00900$$ENDHEX$$Based on Project_id, Code_Type = $$HEX1$$1c20$$ENDHEX$$traxPackLocation$$HEX2$$1d202000$$ENDHEX$$and Code_Id = new value from trax_acct_no. If a value exists in Code_Descript, copy it to $$HEX1$$1c20$$ENDHEX$$trax_pack_location$$HEX2$$1d202000$$ENDHEX$$(field being added to d_baseline_Unicode_shp_header)
//$$HEX2$$a7f00900$$ENDHEX$$Only move the account number from UF11 to trax_acct_no if no entry is found in the lookup table. This will indicate 3rd party billing using the default Pack Location.
//$$HEX2$$a7f00900$$ENDHEX$$If no entry is found and we are setting trax_acct_no, we also want to set freight_terms to $$HEX1$$1c20$$ENDHEX$$THIRDPARTY$$HEX1$$1d20$$ENDHEX$$
//$$HEX2$$a7f00900$$ENDHEX$$If we are setting the Freight Terms to $$HEX1$$1c20$$ENDHEX$$THIRDPARTY$$HEX1$$1d20$$ENDHEX$$, we also want to create a Trax 3rd Party Address record if the Bill To Address record has been created.
//$$HEX2$$22200900$$ENDHEX$$The baseline logic creates a Delivery_address record with address_type = $$HEX1$$1820$$ENDHEX$$BT$$HEX2$$19202000$$ENDHEX$$if the Bill to fields are present. In the new custom NVO, We want to update the Address_Type to $$HEX1$$1820$$ENDHEX$$3P$$HEX2$$19202000$$ENDHEX$$if we created a $$HEX1$$1820$$ENDHEX$$BT$$HEX2$$19202000$$ENDHEX$$record.
...
//		 ls_date = mid(ls_date,7,2)  + "/" + mid(ls_date,5,2) + "/" +  left(ls_date,4)
	...
	CASE 'TP'	
		...
		//The baseline logic creates a Delivery_address record with address_type = $$HEX1$$1820$$ENDHEX$$BT$$HEX2$$19202000$$ENDHEX$$if the Bill to fields are present. In the new custom NVO, We want to update the Address_Type to $$HEX1$$1820$$ENDHEX$$3P$$HEX2$$19202000$$ENDHEX$$if we created a $$HEX1$$1820$$ENDHEX$$BT$$HEX2$$19202000$$ENDHEX$$record.
