HA$PBExportHeader$u_nvo_edi_confirmations_honda_new.sru
$PBExportComments$Process outbound edi confirmation transactions for Honda Thailand
forward
global type u_nvo_edi_confirmations_honda_new from nonvisualobject
end type
end forward

global type u_nvo_edi_confirmations_honda_new from nonvisualobject
end type
global u_nvo_edi_confirmations_honda_new u_nvo_edi_confirmations_honda_new
...
Long			llNewRow,i,llRowCount,lAdjustID,llTransID
...
Datastore ldsAdjustment,ldsAdjustmentbatch
...
//SARUN2015NOV24 : (Consolidation of Adjustment in one File) DataStore for All batch record for adjustment having status 'N' 
If Not isvalid(ldsAdjustmentbatch) Then
	ldsAdjustmentbatch = Create Datastore
	ldsAdjustmentbatch.Dataobject = 'd_adjustment_batch_honda'
	ldsAdjustmentbatch.SetTransObject(SQLCA)
End If

llRowCount = ldsAdjustmentBatch.Retrieve()
...
//SARUN2015NOV24 : (Consolidation of Adjustment in one File) Inserting all record to genric file for creating single file
For i = 1 to llRowCount 
	lAdjustID = Long(ldsAdjustmentBatch.GetitemString(i,'trans_order_id'))
	llTransID = ldsAdjustmentBatch.GetitemnUMBER(i,'trans_id')
																	 
	//Retreive the adjustment record
	If ldsAdjustment.Retrieve(asProject, lAdjustID) <= 0 Then
	...
	If (lsWarehouse = 'EXP-HONDA' ) Then
		...
		Else
			...
			//Project	C(10)	Yes	$$HEX1$$1c20$$ENDHEX$$HONDA     $$HEX2$$1d200900$$ENDHEX$$Project Identifier
...
String		lsFind, lsOutString,	lsMessage, lsLogOut, lsFileName,  lsSku, lsSuppCode,lsfilenamedate, lsLineItemNo,lsModel,lsType,lsColor, lsSerial, lsPoNo2
...
//	lsOutString += String(idsROMain.GetITemDateTime(1,'complete_date'),'hhmmss') 
	...
	//TAM 01/31/2015 Order date is now stored in detail UF5.  Assume the formatting is correct in the field(No formatting required)
//	lsOutString += String(idsROMain.GetITemDateTime(1,'ord_date'),'yyyymmdd') //+ lsDelimitChar
	lsTemp = Left(idsRoDetail.GetItemString(llDetailFind,'User_Field5'),8)
...
//TAM 01/22/2015 -  Use detail_UF4 instead of line no.	
//	// Line Number C(3)
//	lsOutString += lsLineItemNo
	lsTemp = Left(idsRoDetail.GetItemString(llDetailFind,'User_Field4'),3)
	If IsNull(lsTemp) then 
		lsTemp = '   '
	Else
	...
	//TAM 2015/01/26  If Serial Number = '-' then send blanks
	lsTemp =idsRoPutaway.GetITemString(llRowPos,'Serial_No')
	If lsTemp = '-' then
		lsOutString += '         '
		lsOutString += '       '
	Else
	...
	//TAM 2015/01/26  If Po_No2 = '-' then send blanks
...
//	lsOutString += String(idsROMain.GetITemDateTime(1,'complete_date'),'yyyymmdd') 
	...
	// Location
	//TAM 02/11/2015 Location is now stored in UF1.  
//lsOutString += 'DOM  '
	lsTemp = Left(idsROMain.GetItemString(1,'User_Field1'),5)
	If IsNull(lsTemp) then 
		lsTemp = '     '
	Else
...
String 	lsCaseTrunc, lsUf4, lsUf5, lsUf6, lsUf7, lsUf8, lsUf9, lsfilenametype, lsLineItemN, lsPoNoTrunc,lsFileNamePath, lsModel, lsType, lsColor, lsTemp, lsSerial, lsPoNo2
...
//	idsDoPick.Dataobject = 'd_do_Picking_detail_baseline' 
	idsDoPick.Dataobject = 'd_do_Picking_detail_honda' 
...
For llRowPos = 1 to llRowCOunt
	...
	//Delivery Date	Date	Yes	N/A	Delivery completion date
	// TAM 2015/02 changed from complete date to order date
//	lsOutString = String(idsDOMain.GetITemDateTime(1,'complete_date'),'yyyymmdd') 
	lsOutString = String(idsDOMain.GetITemDateTime(1,'ord_date'),'yyyymmdd')

	//Delivery Time	Time	Yes	N/A	Delivery completion time
//	lsOutString += String(idsDOMain.GetITemDateTime(1,'complete_date'),'hhmmss')
	lsOutString += String(idsDOMain.GetITemDateTime(1,'ord_date'),'hhmmss') 
...
//	lsOutString +=  string(idsDoDetail.GetItemNumber(llDetailFind,'Line_item_no'),'0000' )
	lsTemp = Left(idsDoDetail.GetItemString(llDetailFind,'User_Field2'),4)
	If IsNull(lsTemp) then 
		lsTemp = '    '
	Else
...
//TAM 2015/01/23 If SerialNo = '-' Then Engine Head and Engine No are '' Blank
//TAM 2015/02/ SerialNo is now a concatenation of SerialNo and PONO2 separated by a ':'.  Added logic to substring the 2         
	lsTemp = idsdoPick.GetITemString(llRowPos,'Serial_No')

	If TRIM(lsSerial ) = '-' then //Trim field of blanks
		lsOutString +=  '         '
		lsOutString +=  '       '
	Else
...
//TAM 2015/01/23 If PoNo2 = '-' Then Frame Head and Frame No are '' Blank
	lsTemp = idsdoPick.GetITemString(llRowPos,'po_no2')
	
	If Trim(lsPoNo2) = '-' then //TAM - Added trim blanks
		lsOutString +=  '          '
		lsOutString +=  '       '
	Else
	...
	// TAM 2015/02 changed from complete date to request date
//	lsOutString += String(idsDOMain.GetITemDateTime(1,'complete_date'),'yyyymmdd')
	lsOutString += String(idsDOMain.GetITemDateTime(1,'request_date'),'yyyymmdd')

	//Truck No	C(7)	No		N/A	Detail_UF1
	// TAM 2015/02 changed to HeaderUF2
	lsTemp = Left(idsDoMain.GetItemString(1,'User_Field2'),7)
	If IsNull(lsTemp) then 
		lstemp = '  '
	Else
		lsTemp =  lsTemp + Space(7 - len(lsTemp))
...
//	lsOutString = String(idsDOMain.GetITemDateTime(1,'complete_date'),'yyyymmdd') //+ lsDelimitChar
	lsOutString = String(idsDOMain.GetITemDateTime(1,'ord_date'),'yyyymmdd') //+ lsDelimitChar

	//Delivery Time	Time	Yes	N/A	Delivery completion time
//	lsOutString += String(idsDOMain.GetITemDateTime(1,'complete_date'),'hhmmss') //+ lsDelimitChar
	lsOutString += String(idsDOMain.GetITemDateTime(1,'ord_date'),'hhmmss') //+ lsDelimitChar
...
on u_nvo_edi_confirmations_honda_new.create
call super::create
TriggerEvent( this, "constructor" )
end on

on u_nvo_edi_confirmations_honda_new.destroy
