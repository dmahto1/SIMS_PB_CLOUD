HA$PBExportHeader$u_nvo_edi_confirmations_gibson.sru
$PBExportComments$Process outbound edi confirmation transactions for GIBSON
forward
global type u_nvo_edi_confirmations_gibson from nonvisualobject
end type
end forward

global type u_nvo_edi_confirmations_gibson from nonvisualobject
end type
global u_nvo_edi_confirmations_gibson u_nvo_edi_confirmations_gibson
...
public function integer uf_gi (string asproject, string asdono);//Prepare a Goods Issue Transaction for GIBSON for the order that was just confirmed
...
If ldBatchSeq <= 0 Then
	lsLogOut = "        *** Unable to retrieve the next available sequence number~rfor Goods Issue Confirmation.~r~rConfirmation will not be sent to GIBSON!'"
	FileWrite(gilogFileNo,lsLogOut)
	Return -1
End If
...
For llRowPos = 1 to llRowCOunt
	...
	idsOut.SetItem(llNewRow,'Project_id', 'GIBSON')
...
gu_nvo_process_files.uf_process_flatfile_outbound(idsOut,'GIBSON')
...
public function integer uf_gr (string asproject, string asrono);//Prepare a Goods Receipt Transaction for GIBSON for the order that was just confirmed
...
ldBatchSeq = gu_nvo_process_files.uf_get_next_seq_no(asproject,'EDI_Generic_Outbound','EDI_Batch_Seq_No')
If ldBatchSeq <= 0 Then
	lsLogOut = "        *** Unable to retrieve the next available sequence number~rfor Goods Receipt Confirmation.~r~rConfirmation will not be sent to GIBSON!'"
	FileWrite(gilogFileNo,lsLogOut)
	Return -1
End If
...
public function integer uf_rt (string asproject, string asrono);//Prepare a Goods Receipt Transaction for GIBSON for the RETURN order that was just confirmed
...
on u_nvo_edi_confirmations_gibson.create
call super::create
TriggerEvent( this, "constructor" )
end on

on u_nvo_edi_confirmations_gibson.destroy
