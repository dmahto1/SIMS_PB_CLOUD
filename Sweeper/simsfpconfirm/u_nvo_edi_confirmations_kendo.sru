HA$PBExportHeader$u_nvo_edi_confirmations_kendo.sru
$PBExportComments$Process outbound edi confirmation transactions for kendo
forward
global type u_nvo_edi_confirmations_kendo from nonvisualobject
end type
end forward

global type u_nvo_edi_confirmations_kendo from nonvisualobject
end type
global u_nvo_edi_confirmations_kendo u_nvo_edi_confirmations_kendo
...
If ldBatchSeq <= 0 Then
	lsLogOut = "        *** Unable to retrieve the next available sequence number~rfor Goods Receipt Confirmation.~r~rConfirmation will not be sent to Kendo!'"
	FileWrite(gilogFileNo,lsLogOut)
	Return -1
End If
...
//	Item Group (from Item Master)
	...
	Else
		lsOutString += "S" + lsDelimitChar /*default if not present - manual orders*/
	End If	
...
Long			llNewRow, llOldQty, llNewQty, llRowCount,	llAdjustID, llOwnerID, llOrigOwnerID, llTranDetailIDNO
				
String		lsOutString, lsMessage,	lsSKU, lsOldInvType,	lsNewInvType, lsFileName,		&
				lsReason, 	lsTranType, lsSupplier, lsRONO, lsOrder,  lstemp, lsRefType, lsRefLineNum, lsRefID
...
// If reason is TRANSFER, this adjustment was created from a stock transfer, retrieve the transfer_detail record by ID_NO (in the Ref_No field)
if lsReason = "TRANSFER" Then
	
	llTranDetailIDNO = Long(ldsAdjustment.GetITemString(1,"ref_no"))
	If llTranDetailIDNO > 0 Then
		
		Select User_Field1, USer_Field2, User_Field3
		Into :lsRefType, :lsRefLineNum, :lsRefID
		From Transfer_Detail
		Where ID_NO = :llTranDetailIDNO;
...
If lsReason = "TRANSFER" Then
	lsOutString += "T" + lsDelimitChar /* T=Transfer */
Else
	lsOutString += "C" + lsDelimitChar /* C = Cycle Count * */
End If
	
//Date	Date	Yes	N/A	Transaction date 
lsOutString += String(today(),'yyyymmdd') + lsDelimitChar
...
// (OLD) Expiration Date	Date	No	N/A	
//lsOutString += String( ldsAdjustment.GetITemDateTime(1,'Old_Expiration_Date'),'yyyymmdd') + lsDelimitChar

lsTemp =String( ldsAdjustment.GetITemDateTime(1,'Old_Expiration_Date'),'yyyymmdd')
If IsNull(lsTemp) or lsTemp = ""  Then
	lsTemp =String( ldsAdjustment.GetITemDateTime(1,'Expiration_Date'),'yyyymmdd')
End If
...
// New Expiration Date 
lsOutString += String( ldsAdjustment.GetITemDateTime(1,'Expiration_Date'),'yyyymmdd')  + lsDelimitChar
		
//Ref Type (From Transfer)
If IsNull(lsRefType) then lsRefType = ''
lsOutString += lsRefType + lsDelimitChar

//Ref Line Num (From Transfer)
If IsNull(lsRefLineNum) then lsRefLineNum = ''
lsOutString += lsRefLineNum + lsDelimitChar

//Ref ID (From Transfer) - LAST COLUMN
If IsNull(lsRefID) then lsRefID = ''
lsOutString += lsRefID  //+ lsDelimitChar
...
on u_nvo_edi_confirmations_kendo.create
call super::create
TriggerEvent( this, "constructor" )
end on

on u_nvo_edi_confirmations_kendo.destroy
