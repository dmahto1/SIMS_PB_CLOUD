HA$PBExportHeader$u_nvo_edi_confirmations_philipscls.sru
...
idsOut.SetItem(llNewRow,'Project_id', 'PHILIPSCLS')
idsOut.SetItem(llNewRow,'edi_batch_seq_no', Long(ldBatchSeq))
idsOut.SetItem(llNewRow,'line_seq_no', llNewRow)
idsOut.SetItem(llNewRow,'batch_data', lsOutString)
...
If idsOut.RowCount() > 0 Then
	gu_nvo_process_files.uf_process_flatfile_outbound(idsOut,'PHILIPSCLS')
End If
...
lsLogOut = "        Creating SDBH For DONO: " + asdono
FileWrite(gilogFileNo,lsLogOut)
...
For ll_row = 1 to ll_detail_count
	...
	idsOut.SetItem(llNewRow,'line_seq_no', llNewRow)
	idsOut.SetItem(llNewRow,'batch_data', lsOutString)
	lsFileName = 'SDBH' + String(ldBatchSeq,'00000000') + '.dat'
	idsOut.SetItem(llNewRow,'file_name', lsFileName)
Next
