HA$PBExportHeader$u_nvo_edi_confirmations_tpv.sru
$PBExportComments$Process outbound edi confirmation transactions for TPV
forward
global type u_nvo_edi_confirmations_tpv from nonvisualobject
end type
end forward

global type u_nvo_edi_confirmations_tpv from nonvisualobject
end type
global u_nvo_edi_confirmations_tpv u_nvo_edi_confirmations_tpv
...
public function integer uf_gi (string asproject, string asdono);//Prepare a Goods Issue Transaction for TPV for the order that was just confirmed
...
//18-Jul-2013 :Madhu - Added condition to don't generate GI, if order_type ='I' and  write to Log
If idsDOMain.GetITemNumber(1,'edi_batch_seq_no') = 0 or isNull(idsDOMain.GetITemNumber(1,'edi_batch_seq_no'))  or idsDOMain.GetITemString(1,'Ord_Type') = 'I'  Then 
	lsLogOut = "      - Order not received electronically or Ord_Type=I. Not creating GI." 
     FileWrite(gilogFileNo,lsLogOut)
	Return 0
End IF
...
If ldBatchSeq <= 0 Then
	lsLogOut = "        *** Unable to retrieve the next available sequence number~rfor Goods Issue Confirmation.~r~rConfirmation will not be sent to TPV!'"
	FileWrite(gilogFileNo,lsLogOut)
	Return -1
End If
...
For llRowPos = 1 to llRowCOunt
	...
	idsOut.SetItem(llNewRow,'Project_id', 'TPV')
...
gu_nvo_process_files.uf_process_flatfile_outbound(idsOut,'TPV')
...
public function integer uf_gr (string asproject, string asrono);//Prepare a Goods Receipt Transaction for TPV for the order that was just confirmed
...
ldBatchSeq = gu_nvo_process_files.uf_get_next_seq_no(asproject,'EDI_Generic_Outbound','EDI_Batch_Seq_No')
If ldBatchSeq <= 0 Then
	lsLogOut = "        *** Unable to retrieve the next available sequence number~rfor Goods Receipt Confirmation.~r~rConfirmation will not be sent to TPV!'"
	FileWrite(gilogFileNo,lsLogOut)
	Return -1
End If
...
public function integer uf_rt (string asproject, string asrono);//Prepare a Goods Receipt Transaction for TPV for the RETURN order that was just confirmed
...
on u_nvo_edi_confirmations_tpv.create
call super::create
TriggerEvent( this, "constructor" )
end on

on u_nvo_edi_confirmations_tpv.destroy
