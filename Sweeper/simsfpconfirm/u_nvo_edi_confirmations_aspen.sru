HA$PBExportHeader$u_nvo_edi_confirmations_aspen.sru
$PBExportComments$Process outbound edi confirmation transactions for Aspen
forward
global type u_nvo_edi_confirmations_aspen from nonvisualobject
end type
end forward

global type u_nvo_edi_confirmations_aspen from nonvisualobject
end type
global u_nvo_edi_confirmations_aspen u_nvo_edi_confirmations_aspen
...
//	lsCOO
	...
	//Supp_Code	C(50	Yes	N/A	Supp_Code
...
//TAM 2014/10/28 -  Added Line Item No to the Find to get the correct detail record.  (May have the same SKU /Supplier for multiple lines
//	llFindRow = idsRODetail.Find("sku='"+lsSku+"' and supp_code = '"+lsSuppCode+"' ", 1, idsRODetail.RowCount())
	...
	If llFindRow > 0 AND idsRODetail.GetItemString(llFindRow,'Currency_Code') <> '' Then
		lsOutString += String(idsRODetail.GetItemString(llFindRow,'Currency_Code')) + lsDelimitChar
	Else
		lsOutString += lsDelimitChar
	End If				
	...
	If llFindRow > 0 AND idsRODetail.GetItemString(llFindRow,'Supplier_Order_Number') <> '' Then
		lsOutString += String(idsRODetail.GetItemString(llFindRow,'Supplier_Order_Number')) + lsDelimitChar
	Else
		lsOutString += lsDelimitChar
	End If				
	...
	If llFindRow > 0 AND idsRODetail.GetItemString(llFindRow,'Cust_PO_Nbr') <> '' Then
		lsOutString += String(idsRODetail.GetItemString(llFindRow,'Cust_PO_Nbr')) + lsDelimitChar
	Else
		lsOutString += lsDelimitChar
	End If				
	...
	If llFindRow > 0 AND idsRODetail.GetItemString(llFindRow,'Line_Container_Nbr') <> '' Then
		lsOutString += String(idsRODetail.GetItemString(llFindRow,'Line_Container_Nbr')) + lsDelimitChar
	Else
		lsOutString += lsDelimitChar
	End If				
	...
	If llFindRow > 0 AND idsRODetail.GetItemString(llFindRow,'Vendor_Line_Nbr') <> '' Then
		lsOutString += String(idsRODetail.GetItemString(llFindRow,'Vendor_Line_Nbr')) + lsDelimitChar
	Else
		lsOutString += lsDelimitChar
	End If				
	...
	If llFindRow > 0 AND idsRODetail.GetItemString(llFindRow,'Client_Line_Nbr') <> '' Then
		lsOutString += String(idsRODetail.GetItemString(llFindRow,'Client_Line_Nbr'))  // No End Delimiter
	End If				
...
//	End If	
	...
	Else 
		lsOutString +=lsDelimitChar //TAM 2015/09/23 !!!!!!Added new fields so uncommented this line
	End If		

	//TAM 2015/09/22 Added New Named Fields

	//Master Department Code  - GI067
	...
	//Detail SSCC_No - GI078 - Not Used
		lsOutString +=lsDelimitChar 

	//Master Account_Nbr  - GI079
	...
	//Master Client_Cust_PO_Nbr - GI081
	...
	//Master Client_Cust_SO_Nbr - GI082
	...
	//Master Equipment_Nbr - GI086
	If idsDoMain.GetItemNumber(1,'Equipment_Nbr') <> 0 Then
	...
	//Master FOB_Bill_Duty_Acct - GI088
	...
	//Master FOB_Bill_Duty_Party - GI089
	...
	//Master FOB_Bill_Freight_Party - GI090
	...
	//Master FOB_Bill_Freight_To_Acct - GI091
	...
	//Detail client_cust_line_no - GI097
	...
	//Detail Client_Cust_Invoice - GI102
	...
	//Detail User_Line_Item_No - GI109
	...
	//Detail customer_sku - GI111
	If idsdoDetail.GetItemString(llDetailFind,'Customer_Sku') <> '' Then
		lsOutString += String(idsDoDetail.GetItemString(llDetailFind,'Customer_Sku')) 
	Else
		lsOutString +=lsDelimitChar  
	End If		

	//Master Cust_Order_No - GI112
	If idsDoMain.GetItemString(1,'Cust_Order_No') <> '' Then
		lsOutString += String(idsDoMain.GetItemString(1,'Cust_Order_No')) //+ lsDelimitChar //LAST COLUMN DOESN'T HAVE A DELIMITER
	Else
//		lsOutString +=lsDelimitChar  //LAST COLUMN DOESN'T HAVE A DELIMITER
	End If		
...
//TAM - Added additional Named Fields
...
on u_nvo_edi_confirmations_aspen.create
call super::create
TriggerEvent( this, "constructor" )
end on

on u_nvo_edi_confirmations_aspen.destroy
