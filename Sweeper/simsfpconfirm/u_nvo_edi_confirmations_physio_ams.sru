HA$PBExportHeader$u_nvo_edi_confirmations_physio_ams.sru
$PBExportComments$Process outbound edi confirmation transactions for AMS-MUSER
forward
global type u_nvo_edi_confirmations_physio_ams from nonvisualobject
end type
end forward

global type u_nvo_edi_confirmations_physio_ams from nonvisualobject
end type
global u_nvo_edi_confirmations_physio_ams u_nvo_edi_confirmations_physio_ams
...
public function integer uf_gi (string asproject, string asdono);//Jxlim 08/01/2012 CR12 interface for PHYSIO AMS
//Prepare a Goods Issue Transaction for PHYSIO AMS for the order that was just confirmed
...
public function integer uf_rs (string asproject, string asdono, long altransid);//Jxlim 08/03/2012 Added RS interface for PHYSIO AMS
//Prepare a Ready to Ship Transaction for PHYSIO AMS for the order that was just set to Ready to Ship
...
//Convert Carrier to PHYSIO AMS Carrier (SCAC)
lsCarrier = idsDoMain.GetItemString(1,'carrier')
		
Select scac_code into :lsSCAC
From Carrier_Master
Where project_id = "PHYSIO AMS" and carrier_code = :lscarrier;
...
//                  This may be short term until PHYSIO AMS can accept multiple LN's per Line ITem.
...
ldBatchSeq = gu_nvo_process_files.uf_get_next_seq_no(asproject,'EDI_Generic_Outbound','EDI_Batch_Seq_No')
If ldBatchSeq <= 0 Then
	lsLogOut = "        *** Unable to retrieve the next available sequence number~rfor Goods Issue Confirmation.~r~rConfirmation will not be sent to PHYSIO AMS!'"
	FileWrite(gilogFileNo,lsLogOut)
	Return -1
End If
...
For llRowPos = 1 to llRowCOunt
	...
	idsOut.SetItem(llNewRow,'Project_id', 'PHYSIO AMS')
	idsOut.SetItem(llNewRow,'edi_batch_seq_no', Long(ldBatchSeq))
	idsOut.SetItem(llNewRow,'line_seq_no', llNewRow)
	idsOut.SetItem(llNewRow,'batch_data', lsOutString)
...
//Write the Outbound File - no need to save and re-retrieve - just use the currently loaded DW
gu_nvo_process_files.uf_process_flatfile_outbound(idsOut,'PHYSIO AMS')

Return 0
end function

on u_nvo_edi_confirmations_physio_ams.create
call super::create
TriggerEvent( this, "constructor" )
end on

on u_nvo_edi_confirmations_physio_ams.destroy
