HA$PBExportHeader$w_workorder.srw
...
String	is_WONO, isColumn, isorigSearchSql
Window	iw_Window
...
For llRowPos = 1 to llRowCount /*For each Detail Record*/
	...
	For llRowQty = 1 to ldReqQty /*QTY 1 in Putaway Row For each Detail QTY*/
...
long i, ldOwnerId,   ldAvailQty, ll_row,ll_count
...
//TAM 07/2014 If 'A' Then UF 4 is component qty.  If "D" then UF4 is Component Putaway Location
	If idw_main.GetItemString(1,'ord_type') = 'A'  Then	
		If isNull(idw_main.GetITemString(1,'user_field4')) or idw_main.GetITemString(1,'User_Field4') ='' Then
			...
			If not isNumber(idw_main.GetITemString(1,'user_field4')) Then
	...
	If idw_main.GetItemString(1,"ord_type") = 'A' Then
		...
		If lsComponentSupplier < '' Then
			Messagebox(is_title,'Component Item is not found in the Item Master.')
			Return -1
		End If
		...
		If ll_count = 1 Then
			Messagebox(is_title,'Component Item is already in the Item Master.')
			Return -1
		End If
...
//TAM 07/2014 Check that UF4(Component Putaway Location) is a valid location
		...
		If ll_count <> 1 Then
			Messagebox(is_title,'Component Putaway Location Not a valid location.')
			Return -1
		End If
	End If
	...
	for i = 1 to ll_count
		...
		*/
		If idw_main.GetItemString(1,"ord_type") = 'A'Then
			...
			idw_detail.SetITem(ll_row,'wo_no',idw_main.GetITemString(1,'wo_no'))
			idw_detail.SetItem(ll_row,'line_item_no',ll_row)
			idw_detail.SetITem(ll_row,'sku',idw_main.GetITemString(1,'user_field3'))
//			idw_detail.SetITem(ll_row,'supp_code',idw_main.GetITemString(1,'user_field4'))
			...
			idw_detail.SetITem(ll_row,'Req_Qty',lds_detail_whs.GetItemNumber(i,'Avail_Qty')* dec(idw_main.GetITemString(1,'user_field4')))
...
//		llFindRow = idw_pick.Find("line_item_no = " + String(idw_pick.GetItemNumber(llFindRow,"line_item_no")),1,idw_pick.RowCount())
		...
		idw_putaway.setitem(llNewRow,'wo_no', idw_main.GetITemString(1,'wo_no'))
		idw_putaway.SetItem(llNewRow,"sku", idw_main.GetITemString(1,'User_field3'))	
		idw_putaway.SetItem(llNewRow,"sku_parent", idw_pick.GetItemString(llRowPos,"sku_parent"))	
...
//		idw_putaway.SetItem(llNewRow,"inventory_type", idw_pick.GetItemString(llFindRow,"inventory_type"))	
		...
		ldChildQty = idw_pick.GetItemNumber(llRowPos,"quantity") * dec(idw_Main.GetItemString(1,"User_Field4"))
...
For llRowPos = 1 to llRowCount /*For each Picking Record*/
	...
	w_main.SetMicrohelp('generating Putaway for Picking Line: ' + String(llRowPos) + ' of ' + String(llRowCount))
		...
		If idw_pick.GetItemString(llRowPos,"Component_Ind") = '*'Then
			...
			idw_putaway.SetItem(llNewRow,"component_no",0) 
			idw_putaway.SetItem(llNewRow,"l_code",  idw_main.GetItemString(1,"user_field4"))	//Tam 07/2014 Use UF4(ComponentPutaway Location)  from Main to set component putaway location)
		Else
			idw_putaway.SetItem(llNewRow,"component_ind",  'Y') //Default Parent to Yes
			idw_putaway.setitem(llNewRow,'Container_Id',idw_pick.GetItemString(llRowPos,"Container_Id"))
			idw_putaway.SetItem(llNewRow,"component_no", idw_pick.GetItemNumber(llRowPos,"component_no")) 
...
// TAM 2010/10/13 Added container Id
	...
	If llFindRow > 0 Then /*update qty on existing record*/
		//Jxlim 01/04/2011 not write the serial numbers to Content if the Serialized_ind = $$HEX1$$1820$$ENDHEX$$B$$HEX1$$1920$$ENDHEX$$. 	
	...
	Else /* create a new content record*/
		...
		idw_putaway_Content.setitem(ll_new,'component_no',ll_Component_No)
	...
	// TAM - 2014/04 - Added Workorder Types Kit Change Add and Delete.  Get the Putaway warehouse from Userfield1 on the Putaway screen
		...
		idw_putaway_Content.setitem(ll_new,'ro_no',idw_putaway.getitemstring(i,'wo_no'))
	...
	////                             via a work order and it$$HEX1$$1920$$ENDHEX$$s serial tracking indicator is $$HEX1$$1820$$ENDHEX$$B$$HEX1$$1920$$ENDHEX$$. Pandora and Bluecoat for now...
...
//*				idw_putaway.SetItem(llComprow,"Expiration_date",(idw_pick.GetItemDateTime(llFindPickRow,'Expiration_date')))
...
//TAM 10/13/2010 For NYCSP we are seting the Parent Quantity to 1 so we use the BOM quantity for the Children
				If gs_project = 'NYCSP' Then
					...
					ldqty = ldqty - lu_ds.GetItemNumber(llRowPos,"child_qty")
				Else
					idw_putaway.SetItem(llCompRow,"quantity",(idw_pick.GetItemNumber(llFindPickRow,'quantity')))
					ldqty = ldqty - idw_pick.GetItemNumber(llFindPickRow,'quantity')
				End If
			...
			//Set Child Sku Values
				...
				Into :lsSerial, :lsCompInd, :lsLot, :lsPO, :lsPO2, :lsExpire
				From Item_master
				Where project_id = :gs_project and sku = :lsChildSku and supp_code = :lsChildSupplier
				Using SQLCA;
				...
				idw_putaway.SetItem(llCompRow,"expiration_controlled_ind",lsExpire)
...
this.Control[iCurrent+1]=this.cb_create_detail
this.Control[iCurrent+2]=this.cb_1
this.Control[iCurrent+3]=this.cb_void
this.Control[iCurrent+4]=this.cb_confirm
this.Control[iCurrent+5]=this.st_1
this.Control[iCurrent+6]=this.dw_report
this.Control[iCurrent+7]=this.sle_order
...
If llRow > 0 Then
	...
	If This.GetITemstring(llrow,'user_Field2') = '' or isnull(This.GetITemstring(llrow,'user_Field2')) Then
		...
		For ll_component_row = 1 to this.RowCount()
