HA$PBExportHeader$w_adjust_create.srw
...
If Upper(gs_Project) = 'LOGITECH' or Upper(gs_Project) = 'MAQUET' or Upper(gs_Project) = 'PHXBRANDS' or Upper ( gs_Project ) = "PANDORA" or Upper ( gs_Project ) = "BLUECOAT" or Upper ( gs_Project ) = 'PHILIPSCLS' Then
		...
		//				MessageBox("Send 947", lsPrevSerial + " swap with " + lsSerial) 
				...
				IF dw_Serial.getItemStatus(llRowPos,"po_no",Primary!) = DataModified! Then
					...
					IF rb_adjust_other.checked = true AND (upper(ls_cc_class_Code) ='A') AND (Upper(trim(lspo)) = 'RESEARCH' AND Upper(trim(lsoldpo)) <> 'RESEARCH' )  THEN
						...
						lsMsg += 'Inventory will be moved to Project Code: RESEARCH.$$HEX2$$a0002000$$ENDHEX$$Deletion of Inventory and Serial number must be processed in a separate Stock Adjustment session after Google has approved in IMS.' + "~r"
	...
	IF upper(gs_project) ='PANDORA' THEN
		...
		iF f_retrieve_parm("PANDORA","FLAG","RECONCILE OPTION") = "Y" Then
			...
			ELSEIF (rb_serial_reconcile.checked  OR rb_adjust_qty.checked) and ll_qty = dw_Serial.RowCount() THEN
				//prompt message
				//ADD is not allowed when Inventory Quantity and Serial Numbers are in sync.$$HEX2$$a0002000$$ENDHEX$$If you need to add a inventory please do this in a Cycle Count

				MessageBox("Adjust", "ADD is not allowed when Inventory Quantity and Serial Numbers are in sync.$$HEX2$$a0002000$$ENDHEX$$If you need to add a inventory please do this in a Cycle Count.",Stopsign!)
	...
	If dw_serial.getItemString(i,'c_select') = 'Y' Then /* checked for Delete*/
		...
		IF upper(gs_project) ='PANDORA' THEN
			...
			ELSEIF rb_adjust_qty.checked and ll_serial_count = ll_content_qty THEN
				...
				//MIKEA - Do not allow a down count for a Class A item unless the project (PO_NO) = $$HEX1$$1820$$ENDHEX$$RESEARCH$$HEX1$$1920$$ENDHEX$$
				...
				IF upper(ls_cc_class_Code) ='A' and trim(upper(ls_po_no)) <> 'RESEARCH' then
					...
					Messagebox("Adjust", "A down count for a Class A item is not allowed unless the project (PO_NO) = $$HEX1$$1820$$ENDHEX$$RESEARCH$$HEX2$$1920a000$$ENDHEX$$",Stopsign!)
					Return
...
event itemchanged;call super::itemchanged;String	lsCOO 
	...
	Case 'CONTAINER_ID' /* 01/03 - PCONKL - must be numeric so we increment if necessary */
		...
		//		This does NOT simulate a unique column constraint, only unique primary key of 15 columns.
			...
			// 04/27/2010 ujh  END change
				...
				If Not isnumber(data) and data <> '-' Then
