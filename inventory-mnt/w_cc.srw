...
public subroutine wf_sort ();// wf_sort()
...
// TAM 2017/06 PEVS-637 Lock Down Pandora Fields
		...
		If upper(gs_project) ='NYCSP' AND idw_main.GetItemString(1, "ord_type") = "L"	Then 
...
For index = 1 to max 
	...
	if Upper( sku ) = 'EMPTY' then continue
...
If idw_main.GetItemString(1, "ord_type") = "L" or idw_main.GetItemString(1, "ord_type") = "B" Then
...
f_method_trace_special( gs_project, this.ClassName() , 'Process Load sys inv, count is: '+String(llRowCount) ,isCCOrder, '','',isCCOrder) //18-Jun-2014 :Madhu- Added Method Trace calls.
	...
	For ll_ctr = 1 to llRowCount
			...
			If ld_qty > 0 OR ld_component_qty > 0  Then
				...
				ls_component_ind = ads.object.component_ind [ll_ctr]
...
IF upper(gs_project) <> 'NYCSP'  Then
	idw_si.SetFilter("component_ind = 'N'")
ELSE
	idw_si.SetFilter("component_ind = 'Y'")
END IF
idw_si.Filter()
...
String	lsFindStr, lsOrdStatus, ls_sku, ls_supp_code, ls_wh_code
long 	llSerialized, ll_row, ll_wareshoue_row
//lsMobileStatus = idw_main.GetItemString(1, "mobile_status_Ind")
lsOrdStatus = idw_main.GetItemString(1, "Ord_status")
...
If upper(gs_project) ='PANDORA'  Then
	...
	if lsOrdStatus <> 'V' and  idw_si.rowcount() > 0 Then
		...
		If llSerialized > 0 Then
			...
			If ib_Foot_Print_SKU and lsOrdStatus ='3' Then
...
ids_CC_UpCountZero_Result.rowcount( )
...
//				ls_ro_no					= idw_si.object.ro_no[ li_idx ]
				...
				if sqlca.sqlcode < 0 then
					...
					Execute Immediate "ROLLBACK" using SQLCA;
					Close (w_update_status)
					Messagebox ("DB Error", ls_SQLError )
					Return -1
				end if
...
If ll_result = 1 Then ll_result = idw_si.Update(False, False)
If ll_result = 1 Then ll_result = idw_result1.Update(False, False)
If ll_result = 1 Then ll_result = idw_result2.Update(False, False)
If ll_result = 1 Then ll_result = idw_result3.Update(False, False)
If ll_result = 1 Then ll_result = idw_mobile.Update(False, False) /* 05/14 - PCONKL*/
If ll_result = 1 Then ll_result = idw_serial_numbers.Update(False, False) /* 2017/11 - TAM*/
If ll_result = 1 Then ll_result = idw_cc_container.Update( False, False) //24-APR-2018 :Madhu S18502 - FootPrint Cycle Count
If idw_main.RowCount() = 0 and ll_result = 1 Then ll_result  = idw_main.Update(False, False)
...
If ll_result = 1 Then
   ...
   Else
		ls_SQLError = SQLCA.SQLErrText
		Execute Immediate "ROLLBACK" using SQLCA;
		MessageBox(is_title, ls_SQLError)
		SetMicroHelp("Save failed!")
...
If idw_main.GetItemString(1, "ord_type") = "L" Then /* By Location */
...
If gs_project = 'PANDORA' Then
	...
	IF (idw_si.Find("(Not IsNull(difference)) AND difference <> 0", 1, idw_si.RowCount()) > 0  and  (ls_ord_status ='2' OR ls_ord_status ='1' )) THEN
		MessageBox(is_title, "Discrepancy found!. Please proceed upto 3rd Count and confirm CC Order", Stopsign!)	
		Return -1
	END IF
...
// TAM - 2017/11 - 3PL CC 
	...
	setSortOrder( 'l_code,sku, line_item_no' )
...
event itemchanged;long ll_rtn,ll_row,ll_owner_id, ll_count
