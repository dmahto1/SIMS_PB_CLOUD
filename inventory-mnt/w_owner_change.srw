HA$PBExportHeader$w_owner_change.srw
...
boolean ib_SerialDeleted //dts - 2014-05-05 (deleted serials weren't being saved).
...
string is_ToLocListWarehouse, is_ToLocListPONO, is_ToLocListSKU, is_ToLocListSerialized
...
int ilCurrPickRow
long ilNbrContainers, ilTotAvailQty
...
//			TODO !!! Project Check here? (12/15/15)
	...
	//					// LTK 20110223	SOC enhancement, commented out block below
	//
	//					/*some material is staying so in the same location
	//					  so check content for the location to see if we're moving all of the material... */
	//					select sum(avail_qty) 	into :ldAvailQty
	//					from content
	//					where project_id = 'PANDORA'
	//					and wh_code = :ls_swhcode and l_code = :ls_slcode
	//					and owner_id <> :llNewOwner
	//					//and (owner_id <> :llNewOwner or Inventory_Type <> :ls_new_inventory_type) and Avail_Qty > 0 
	//					//group by owner_id, sku, inventory_type
	//					Using SQLCA;
	//
	//					//If ld_quantity = ldAvailQty and ls_dlcode = ls_slcode Then
	//					If ld_quantity = ldAvailQty Then
	//					//If true Then //TEMPO!!! - Skipping this validation by Ian's direction to process two H to P SOC's....
	//						//Moving all of the stock in the location to the new owner
	//					Else
	//						//not all stock is moving to new owner
	//	//only throw the message if some of the material is staying in the same location.
	//	//					lbFullQty = False
	//						messagebox(is_title, "Owner change must change all of the material in a location (" + ls_slcode +"), or move to a different location!", StopSign!)
	//						Return -1
	//					End If
...
// TAM - 2017/01/10 Need to check the serial_ind of the detail row.  What was happening is if any row on the detail is serialized then ib_serialized is set on.  It was then doing the location serial validation for non serialized rows.
...
//							If ld_other_owner_qty > 0 Then
					...
					lbSameLoc = False
				end if //if ls_sLCode <> lsNextLoc
				
			
	//			if llOwner <> llNewOwner or ls_itype <> ls_new_inventory_type then
	//				lbFullQty = False
	//				i = idw_detail.rowcount() + 1
	//			end if
	//		else //moving to a new location
	//			//lbFullQty = False
	//			//i = idw_detail.rowcount() + 1
	//		end if
	//		ls_slcode_hold = ls_slcode
		next
					
	//messagebox("TEMPO!", lsTemp)
	//	if lbFullQty then
	//		//select owner_id, inventory_type, sum(avail_qty) 		into :liOwner, :lsInvType, :ldAvailQty
	//		select sum(avail_qty) 	into :ldAvailQty
	//		from content
	//		where project_id = 'pandora'
	...
	//			//not all stock is moving to new owner
	//			lbFullQty = False
	//			messagebox(is_title, "Owner change must change all of the material in a location, or move to a different location!", StopSign!)
	//			Return -1
	//		End If
	//	end if
	end if
...
// dts 2015-12-15 - Commenting out entire block of 'if g.ibSNchainofcustody...' as validation prompts were already commented out so this wasn't actually doing anything
//			if g.ibSNchainofcustody then
//			//if gs_Project = 'PANDORA' OR gs_project = 'BLUECOAT' then
//				//Only do for location OR owner_change
//				llOwner = idw_detail.getitemNumber(i,'Owner_id')
//				ls_itype = Upper(idw_detail.getitemstring(i,'inventory_type'))	
//				llNewOwner = idw_detail.getitemNumber(i,'new_owner_id')
//				ls_new_inventory_type = Upper(idw_detail.getitemstring(i,'new_inventory_type'))
...
//					/*changing owner but not changing location
//						- check that location is either empty or contains only material of like Owner and Inventory Type 
//						! - or if the owner change will change all of the inventory! */
//					ls_sku = Upper(idw_detail.getitemstring(i,'sku'))
...
//		
//					//TimA 06/21/11  This was moved above this For loop
//					 //lds_d_owner_change_check_location = CREATE datastore;
//					 //lds_d_owner_change_check_location.dataobject = "d_owner_change_check_location"
//					 //lds_d_owner_change_check_location.SetTransObject(SQLCA)
//	
//					//TimA 06/21/11
//					 //Added warehouse code in the search                                                
//					 lds_d_owner_change_check_location.Retrieve(gs_project, ls_swhcode,ls_dlcode) 
//	
//					
//	//				MessageBox ("rowcount", lds_d_owner_change_check_location.RowCount())
//				
//					//Kinda redundant but through
//					//Needed to do to handle -A - B locations that had to be created.
//					
//					llNewOwner = idw_detail.getitemNumber(i,'new_owner_id')
//					llOwner = idw_detail.getitemNumber(i,'owner_id')		
//					
//					integer li_find
//					for j = 1 to idw_detail.RowCount()
//						if ls_dlcode = idw_detail.getitemString(j,'d_location') then
//							ls_sku = Upper(idw_detail.getitemstring(i,'sku'))
//							ld_quantity = idw_detail.getitemnumber(i,'quantity')
//							
//							li_find = lds_d_owner_change_check_location.Find("sku = '"+ls_sku+"' and owner_id = " + string(llOwner) + " and inventory_type = '" + ls_itype+ "'", 1,lds_d_owner_change_check_location.RowCount() )
//							if li_find > 0 then
//								ld_avail = lds_d_owner_change_check_location.GetItemNumber(li_find, "sum_avail_qty")
//								lds_d_owner_change_check_location.SetItem(li_find, "sum_avail_qty", (ld_avail - ld_quantity))
//							end if
//						end if
//					next
//	
//					//dts - 09/16/10 - changed 'avail_qty' to 'sum_avail_qty' in find ....
//					li_find = lds_d_owner_change_check_location.find ("sum_avail_qty > 0 ", 1, lds_d_owner_change_check_location.RowCount()) 
//	
//					if li_find > 0 then
//	//					// KRZ - Check status of location
...
//	//					
//	//					IF li_Count > 0 THEN
//	//						messagebox(is_title, "The 'TO Location' ("+ls_dlcode+") has material of a different Owner or Inventory Type!",StopSign!)
//	//						Return -1
//	//					END IF
//	
//					end if
...
//	//					Where project_id = 'PANDORA'
//	//					and wh_code = :ls_dwhcode and l_code = :ls_dlcode
//	//					and (owner_id <> :llNewOwner or Inventory_Type <> :ls_itype) and Avail_Qty > 0 
//	//					Using SQLCA;
//	//					
//	//				
...
//	//					End If
//	//					
//				END IF //IF (ls_lcode = ls_dlcode) AND (llOwner <> llNewOwner or ls_pono <> ls_new_pono )
//			end if //if g.ibSNchainofcustody 
...
//		if wf_check_full_pallet_container() = -1 then return -1
	...
	// LTK 20110110
	// The following function call is no longer needed, checked production database to verify.
	//
	//
	////Need to see if this order processed in a previous version
	//
	//IF wf_check_process() <> 1 THEN
	//	
	...
	//	ls_sku = Upper(idw_detail.getitemstring(i,'sku'))
	//	lsSupplier = Upper(idw_detail.getitemstring(i,'supp_code'))
	////	lsCoo = Upper(idw_detail.getitemstring(i,'Country_of_origin'))
	//	llOwner = idw_detail.getitemNumber(i,'Owner_id')
	//	llNewOwner = idw_detail.getitemNumber(i,'new_owner_id')
	//	ls_lcode = Upper(idw_detail.getitemstring(i,'s_location'))
	//	ls_dlcode = Upper(idw_detail.getitemstring(i,'d_location'))
	...
	//	ls_new_pono = Upper(idw_detail.getitemstring(i,'new_po_no'))
	//	ls_itype = Upper(idw_detail.getitemstring(i,'inventory_type'))
	////	ls_container_id = Upper(idw_detail.getitemstring(i,'container_id')) 	//GAP 11/02 added 
	////	ldt_expiration_date = idw_detail.getitemdatetime(i,'expiration_date') //GAP 11/02 added here 
	//	ll_line_item_no =  idw_detail.getitemnumber(i,'line_item_no')
	...
	//
	////
	////	IF llOwner = llNewOwner AND &
	////		ls_pono = ls_new_pono AND &
	////		ls_lcode = ls_dlcode AND &
	////		ls_new_inventory_type = ls_itype THEN
	////		
	//////		MessageBox ("Error", "No Changes")
	////		
	////		Continue
	////		
	////	END IF
	...
	/*
		...
		// Old code block being replaced...
	
	//
	//
	//	//ls_container_id
	//
	//	li_rowcount = idw_content.retrieve(gs_project, ls_warehouse, ls_sku,lsSupplier,llOwner, ls_lcode, &
	//										ls_pono, ls_itype )										  
	//	
	//
	//
	//	//A row is create for each row in content in transfer detail. There will never be a case where there is a less of avail_qty for a transfer_detail row.
	//	
	...
	//			j ++
	//			
	//			lb_record_in_db = false
	//			
	//			//See if a content record already exists.
	//			
	//			lsCoo = idw_content.GetItemString(j, "Country_of_origin")
	//			ls_sno = idw_content.GetItemString(j, "Serial_No")
	//			ls_lno = idw_content.GetItemString(j,'lot_no')
	//			ls_ro =  idw_content.GetItemString(j,'ro_no')
	//			ls_pono2 =  idw_content.GetItemString(j,'po_no2')
	//			ldt_expiration_date = idw_content.GetItemDateTime(j,'expiration_date')
	//			ls_container_id = idw_content.GetItemString(j,'container_id')	
	//			
	//	
	//			SELECT avail_qty INTO :ld_content_qty
	//				FROM content 
	//				WHERE Project_ID = :gs_project AND
	//						SKU = :ls_sku AND
	//						Supp_Code = :lsSupplier AND
	//						Owner_ID  = :llNewOwner AND
	//						Country_of_Origin = :lsCoo AND
	//						WH_Code  = :ls_warehouse AND
	//						L_Code = :ls_dlcode AND
	//						Inventory_Type = :ls_new_inventory_type AND
	//						Serial_No = :ls_sno AND
	//						Lot_No = :ls_lno AND 
	//						RO_No = :ls_ro AND 
	//						PO_No = :ls_new_pono AND 
	//						PO_No2 = :ls_pono2 AND
	...
	//				
	//				IF NOT lb_record_in_db THEN
	//				
	//					idw_content.SetItem( j, "po_no", ls_new_pono)
	//					idw_content.SetItem( j, "owner_id", llNewOwner)
	//					idw_content.SetItem( j, "l_code", ls_dlcode)
	//					idw_content.SetItem( j, "inventory_type", ls_new_inventory_type)
	//		
	//				
	//				ELSE
	//					
	//					ld_content_qty = ld_content_qty + ld_avail
	//					
	...
	//				//Adjust the required.
	//
	//				ld_req = ld_req - ld_avail
	//			
	//			ELSEIF ld_avail > ld_req  THEN 
	//				
	//	//
	//	
	//		
	...
	//				IF NOT lb_record_in_db THEN
	//		
	//					idw_content.RowsCopy( j, j, Primary!, idw_content, (idw_content.RowCount() + 1), Primary!)
	//	
	//					//Subtract what is required.
	//	
	//					idw_content.SetItem( j, "avail_qty", (ld_avail - ld_req))
	//		
	//					//Create new line for change
	//		
	//					idw_content.SetItem( idw_content.RowCount(), "avail_qty", ld_req)
	//	
	//					idw_content.SetItem( idw_content.RowCount(), "po_no", ls_new_pono)
	//					idw_content.SetItem( idw_content.RowCount(), "owner_id", llNewOwner)
	//					idw_content.SetItem( idw_content.RowCount(), "l_code", ls_dlcode)
	//					idw_content.SetItem( idw_content.RowCount(), "inventory_type", ls_new_inventory_type)
	//	
	//				ELSE
	//					
	//					//Subtract what is required.
	...
	//					//Update it.
	//					
	//					ld_content_qty = ld_content_qty + ld_req					
	//					
	//					
	//					UPDATE content
	//						SET avail_qty = :ld_content_qty
	...
	////		//Remove any 0 Rows.
	////		
	////		for j = idw_content.RowCount() TO 1 Step -1
	////			if idw_content.GetItemNumber( j, "avail_qty") = 0 then
	////				idw_content.DeleteRow(j)
	////			end if
	////		next
	//		
	//		if idw_content.Update() <> 1 then
	//			Execute Immediate "ROLLBACK" using SQLCA;
	//// TAM 2010/09/10 moved message after rollback
	//			Messagebox ("Error", "Error Saving Content")
	//			RETURN -1
	//		
	//		end if
	//		
	...
	//		Execute Immediate "ROLLBACK" using SQLCA;
	//// TAM 2010/09/10 moved message after rollback
	//		Messagebox ("Error", "Not Enough Inventory - Please reselect from location. 2")
	//		RETURN -1
	//		
	//	end if
	//	
	
		// End of old code block being replaced
...
// TAM - 2018/04 - DE3780 - Add rollback
	...
	/*
		lsFind = "Upper(sku) = '" + ls_sku + "' and Upper(l_code) = '" + ls_dlcode + &
			...
			"' and upper(country_of_origin) = '" + lscoo + "' and Upper(inventory_type) = '" + ls_itype + "' and owner_id = " + string(llowner)  + & 
			" and upper(supp_code) = '" + lsSupplier + "'"
	*/		
		lsFind = "Upper(sku) = '" + ls_sku + "' and Upper(l_code) = '" + ls_dlcode + &
			"' and Upper(serial_no) = '" + ls_sno + "' and Upper(lot_no) = '" + ls_lno + "' and Upper(po_no) = '" + ls_new_pono +  "' and Upper(po_no2) = '" + ls_pono2 +  & 
			"' and Upper(Container_ID) = '" + ls_container_id + "' and String(expiration_date,'mm/dd/yyyy hh:mm') = '" +  String(ldt_expiration_date,'mm/dd/yyyy hh:mm')   + & 
	...
	/*		
			lsFind = "Upper(sku) = '" + ls_sku + "' and Upper(l_code) = '" + ls_dlcode + &
				...
				"' and Upper(inventory_type) = '" + ls_itype + "' and owner_id = " + string(llOwner) + & 
				" and upper(supp_code) = '" + lsSupplier + "'"
	...
	*/
	...
	//			idw_content_append.setitem(ll_currow,'po_no', ls_pono)
				...
				idw_content_append.setitem(ll_currow,'po_no2', ls_pono2)
				idw_content_append.setitem(ll_currow,'container_ID', ls_container_ID)  //GAP 11/02 added here 
				idw_content_append.setitem(ll_currow,'expiration_date', ldt_expiration_date)		//GAP 11/02 added here 	
	...
	//idw_transfer_detail_content_append.SaveAs("c:\idw_transfer_detail_content_append"  + idw_main.Object.to_no[1] + " .csv", CSV!, false)
		...
		values						(:gs_project,:ls_sku,:lsSupplier,:llOwner,:llNewOwner,:lsCoo,:lsCoo,:ls_dwhcode,:ls_dlcode,:ls_itype,:ls_new_inventory_type, &
										:ls_sno,:ls_lno, :ls_new_pono, :ls_pono,:ls_pono2,:ls_pono2,
										:ls_container_ID, :ldt_expiration_date, :ls_ro, :ll_quantity, :ll_quantity,:ls_Ref,:ls_reason,:gs_userid,:ldtToday,'O',
										:ls_lno) 
...
// TAM 2019/05 - S33409 - Populate Serial History Table
				SET Owner_Id = :ll_new_owner_id, Owner_CD = :ls_c_new_owner_name, po_no = :ls_new_pono, l_code = :ls_new_l_code, update_date = :ldtToday, update_user = :gs_userid,
	...
	//TAM 2010/08/19 Save serial records into new records into table
	
	ls_warehouse = getWarehouse()
	llOwner = idw_detail.getitemNumber(1,'Owner_id')
	llNewOwner = idw_detail.getitemNumber(1,'new_owner_id')
	ls_c_new_owner_name =  Upper(idw_detail.getitemstring(1,'c_new_owner_name'))
	...
	If llOwner <> llNewOwner Then
	
		j = 0
	
		Do while  j < tab_main.tabpage_alt_serial_capture.dw_alt_serial_parent.RowCount()
			j ++
	
		//Get existing serial inventory record for the serial number.  
			ls_sku = tab_main.tabpage_alt_serial_capture.dw_alt_serial_parent.GetItemString(j,'sku_parent')
			ls_serial_no =tab_main.tabpage_alt_serial_capture.dw_alt_serial_parent.GetItemString(j,'serial_no_parent')
			SELECT Wh_Code, Owner_Id  
			INTO :ls_sn_warehouse, :ll_sn_owner_id  
			FROM Serial_Number_Inventory  
			WHERE ( Project_Id = :gs_project ) AND ( SKU = :ls_sku ) AND (Serial_No = :ls_serial_no ) using sqlca   ;
	...
	// TAM 2010/09/10 moved message after rollback
			...
			//Write to table
			UPDATE Serial_Number_Inventory  
			SET Owner_Id = :llnewowner, Owner_CD = :ls_c_new_owner_name  
			WHERE ( Project_Id = :gs_project ) AND ( SKU = :ls_sku ) AND (Serial_No = :ls_serial_no ) using sqlca   ;
...
FOR ll_idx  = 1 to idw_detail.RowCount()
	...
	If lbFootprint and lsGPNConversionFlag = 'Y' Then
		...
		If llFootprintSerialNumbersPresent > 0 and not lbDashedFootprint Then
			...
			MessageBox ("Missing Serial Numbers Alert!", lsMsg4 )
			tab_main.SelectTab(2)
	...
	If lbFootprint  then
		...
		IF Not lbAllocated Then
			...
			ElseIf llEmptyContainers > 0 and llAllocContainers > 0 and Not ibMixedContainerization Then
				...
				idw_detail.SetItemStatus(ll_idx, "color_code", Primary!, NotModified! )	
			ElseIf ibMixedContainerization and ll_Qty <> ll_count and lsMixedType <> 'N' and ll_count > 0 and lsLocation <> "*" and lsMsg5 = "" Then	//MCL needs split
				...
				lsMsg4 += "Footprint GPN " + ls_sku + " must maintain full pallet and containers.~n~r~n~r" + &
					...
					"Once identified, the serial number(s) allocated will be linked to this order for stock owner change."
...
If lsMsg1 > "" or lsMsg2 > "" or lsMsg3 > "" or lsMsg4 > "" Then
	tab_main.SelectTab(2)
	tab_main.tabpage_detail.SetFocus()
	idw_detail.SetColumn("container_id")
	idw_detail.SetFocus()
...
long i,ll_totalrows, ll_no,ll_rtn, ll_detail_rows, ll_detail_rowcount
String ls_prefix, ls_order,ls_Message,ls_status, lsOrderNbr,lsSyntax
...
call super::create
iCurrent=UpperBound(this.Control)
this.Control[iCurrent+1]=this.cb_set_new
this.Control[iCurrent+2]=this.st_2
this.Control[iCurrent+3]=this.sle_orderno
this.Control[iCurrent+4]=this.cb_confirm
this.Control[iCurrent+5]=this.cb_void
...
event modified;String ls_order, ls_wh_code, ls_code
Integer	lirc, llCount, ll_Serialized_Indacator, li_row
...
this.Control[]={this.dw_transfer_detail_content_append,&
...
//	MessageBox("Debug - Client", "ds.rowcount() = " + String(lds_from_locations.rowCount()) + &
//		"  From Locations rows modified =  " + String(lds_from_locations.Object.co_change_count[1]))
...
string lsContainer, lsLocation, lsMsg
boolean lbFootprint
integer liRC
