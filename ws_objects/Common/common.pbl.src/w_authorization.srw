...
Choose case isAuthorize_Type 
	...
	Case 'F10'
		...
		Else
			...
			str_EC.nReturn	= -1
		End if
		//Dhirendra S53682  Google - SIMS - TMS F10 User Update -start
		Case 'TMS'
		IF len(isAuthorize_User) > 0 THEN
			str_EC.Authorize_user = isAuthorize_user
			str_EC.Authorize_date = idtAuthorize_date
		...
		// Dhirendra S53682  Google - SIMS - TMS F10 User Update -End
	Case Else
		str_EC.Authorize_license = ''
		str_EC.Authorize_user = ''
		str_EC.nReturn	= -1
...
//	End If
	...
	Else
		...
		boolean TMS_AUTHORIZIED////S53682  Google - SIMS - TMS F10 User Update
		...
		using SQLCA; 
		//Dhirendra S53682  Google - SIMS - TMS F10 User Update -start
		IF ll_count = 0 and gs_project = 'PANDORA' then 
		select count(*) into :ll_count from Lookup_Table with(nolock) 
		where Code_Type='TMS_AUTHORIZE' and Code_Id=:sle_authorized_user.text
		and User_Updateable_Ind ='Y'
		using SQLCA; 
		if ll_count > 0 then
			TMS_AUTHORIZIED = true
		end if 
	end if 
		...
		If ((ll_useraccess =0 or ll_useraccess =-1) and (isAuthorize_Type ='F10'))   THEN
			bAuthenticated = TRUE
		ElseIf ((ll_useraccess =1 or ll_useraccess =2) and (isAuthorize_Type ='F10') and (ll_count > 0)) and TMS_AUTHORIZIED = false THEN
			...
			bAuthenticated = TRUE
			ElseIf ((ll_useraccess =1 or ll_useraccess =2) and (isAuthorize_Type ='F10') and (ll_count > 0)) and TMS_AUTHORIZIED = true THEN
			f_method_trace_special( gs_project, this.ClassName() + ' - clicked', 'Following User# '+sle_authorized_user.text + ' has already had F10 access',gs_system_no, ' ',' ',gs_system_no) //07-Oct-2015  :Madhu added
			bAuthenticated = TRUE
			isAuthorize_Type = 'TMS'
		ElseIf ((ll_useraccess =1 or ll_useraccess =2) and (isAuthorize_Type ='F10') and (ll_count=0)) and TMS_AUTHORIZIED = false THEN
