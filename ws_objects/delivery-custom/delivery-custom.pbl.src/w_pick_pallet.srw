$PBExportHeader$w_pick_pallet.srw
...
u_ds_ancestor idsSerialsEntered
...
Int iiPrevRow, iiCurrRow, iiCurrPickRow, iiRC2, iiPos, iiRC, iiCnt, iiSplit, iiMultiRono, iiPC, iiPC2, iiPD, IIpD2
...
public function string getrono (string asdono, integer aslineitemno)
end prototypes

event ue_container_adjust();Integer i, j, k
String lsSqlSyntax, lsWhere, lsSql, lsSerialRoNo
...
If  isContinue = "Y" Then
	...
	Else
		...
		//GailM 5/12/2020 - S45954 F22932 I2621 Google DA Kitting move to Spoke Warehouse - Putaway
		//No longer applicable.  Inbound will merge new ROs into what is already there.  Only one GPN content entry will be in a DA Kitting location.
		If ibPharmacyProcessing Then
//			j = wf_retrieve_pick_detail(iiCurrPickRow)
//			iiPD = idsPickDetail.RowCount()
//			iiPC = idw_pick.RowCount()
//			
//			isRoNo = getRoNo(isDoNo, idw_pick.GetItemNumber(iiCurrPickRow, "line_item_no") )
//			idsSerialValidate.Reset()
...
//		
//			k = idsSerialValidate.SetSqlSelect(lsSql)
//			If k <> 1 Then
...
//			End If
//			
//			ilSerialRows = idsSerialValidate.Retrieve()
//
//			For i = 1 to ids2.rowcount()		//Loop thru serial numbers pick and check for reallocation
//				isSerialNo = ids2.GetItemString(i, 'Serial_No')
//				isFind = "serial_no = '" + isSerialNo + "' "
//				ilFound = idsSerialValidate.Find( isFind, 1, idsSerialValidate.rowcount() )
//				If ilFound > 0 Then
//					lsSerialRoNo = idsSerialValidate.GetItemString(ilFound, 'ro_no')
//					If lsSerialRoNo <> isRoNo Then
//						isFind = "ro_no = '" + lsSerialRoNo + "' "
//						iiPD2 = idsPickDetail.Find(isFind, 1, idsPickDetail.RowCount() )
//						If iiPD2 > 0 Then			//Update quantity ++
//							j = idw_pick.SetItem( iiPD2, 'quantity', idw_pick.GetItemNumber(iiPD2, 'quantity') + 1 )				//Add to new
//							k = idsPickDetail.SetItem( iiPD2, 'quantity', idsPickDetail.GetItemNumber( iiPD2, 'quantity') + 1 )
//							j = idw_pick.SetItem( iiPC, 'quantity',  idw_pick.GetItemNumber( iiPC, 'quantity') - 1 )				//decrement orig
//							k = idsPickDetail.SetItem( iiPD, 'quantity', idsPickDetail.GetItemNumber( iiPD, 'quantity') - 1 )
//						Else
//							iiPC2 = idw_pick.RowsCopy(1, 1, Primary!, idw_pick, idw_pick.rowcount()+1, Primary!)
//							iiPC2 = idw_pick.RowCount()	//New row added at end of data
//							j = idw_pick.SetItem(iiPC2, 'quantity', 1)
//							j = idw_pick.SetItem(iiPC, 'quantity', idw_pick.getitemnumber(iiPC,"quantity") - 1)
//							
//							iiPD2 = idsPickDetail.RowsCopy( iiPD, iiPD, Primary!, idsPickDetail, idsPickDetail.rowcount()+1, Primary!)
//							iiPD2 = idsPickDetail.RowCount()
//							k = idsPickDetail.SetItem(iiPD2, 'ro_no', lsSerialRoNo)
//							k = idsPickDetail.SetItem(iiPD2, 'quantity', 1 )
//							k = idsPickDetail.SetItem(iiPD, 'quantity', idsPickDetail.getitemnumber(iiPD,'quantity') - 1)
//							
//							j = idw_pick.SetItemStatus ( iiPC2, 'quantity', Primary!, DataModified!)
//							k = idsPickDetail.SetItemStatus ( iiPD2, 'quantity', Primary!, DataModified!)
//							k = idsPickDetail.SetItemStatus ( iiPD2, 'ro_no', Primary!, DataModified!)
//						End If
//						isReallocateInd = 'Y'
//					End If
//				End If
...
//				If ilFound > 0  Then					//Delete pick row if quantity is zero
//					iiRC = idw_pick.DeleteRow(ilFound)
//					isReallocateInd = 'Y'
//				End If
//			Next
//			For i = idsPickDetail.rowcount() to 1 Step -1
//				isFind = "quantity <= 0" 
//				ilFound = idsPickDetail.Find( isFind, 1, idsPickDetail.rowcount() )
//				If ilFound > 0  Then					//Delete pick row if quantity is zero
//					iiRC = idsPickDetail.DeleteRow(ilFound)
//					isReallocateInd = 'Y'
//				End If
//			Next
//			If isReallocateInd = 'Y' Then
//				Execute Immediate "Begin Transaction" using SQLCA;
//				iiRC = idw_pick.update()
//				If iiRC = 1 Then
//					iiRC = idsPickDetail.update()
//					If iiRC = 1 Then
//						Execute Immediate "Commit" Using SQLCA;
//					Else
//						Execute Immediate "Rollback" using SQLCA;
//						isMessage =  "Could not save Picking reallocated data for DA Kitting.~r~r" + idsContentSummary.isDBErrTExt
//					End If
//				Else
//					Execute Immediate "Rollback" using SQLCA;
//					isMessage =  "Could not save Picking Detail reallocated data for DA Kitting.~r~r" + idsContentSummary.isDBErrTExt						
//				End If
//			End If
		End If
	End If
...
End If
If ilContentSummaryRows > 1 Then
			...
			If ilContentSummaryRows = 1 and ilContentRows = 1 Then
				...
				For iiSelPos = 1 to ids1.RowCount()		//Those staying in stock will have a new container/carton
					...
					If ilFound > 0 Then
						...
						If iiPos > 0 Then		//This serial no stays and has a new container id
					...
					End If	
				Next
				For iiSelPos = 1 to ids2.RowCount()		//Those remaining with the order with same container ID
					isSerialNo = ids2.GetItemString( iiSelPos, 'Serial_No')
					isFind = "Serial_No = '" + isSerialNo + "' "
					ilFound = idsSerial.Find(isFind, 1, idsSerial.RowCount())
					If ilFound > 0 Then				//This serial no stays in container and is linked to the order with "L" serial flag
						idsSerial.SetItem( ilFound, 'serial_flag', 'L' )
						idsSerial.SetItem( ilFound, 'do_no', isDoNo )
						idsSerial.SetItem( ilFound, 'update_user', gs_userid )
						idsSerial.SetItem( ilFound, 'update_date', idtToday )
						idsSerial.SetItem( ilFound, 'Transaction_Type', 'UPDATE SERIAL FLAG' )
						idsSerial.SetItem( ilFound, 'Transaction_Id', isDoNo)
						idsSerial.SetItem( ilFound, 'Adjustment_Type', 'DYNAMIC BREAK' )
					End If
				Next
				If ilSerialRows < ilOrigQty Then		//The pallet splits.  Two adjustments and two content records
					...
					If iiRC > 0 Then
						...
						isCompletionMessage += "New ContainerID " + isNewContainer + " qty: " + string(ilSerialRows) + "~n~r"
...
If ibSNChanged or ibSerialChanged Then
	...
	Else
		lsPoNo = idw_pick.GetItemString(1,"Po_No")		//Get original project
...
liRtn = idsPickDetail.retrieve(lsDoNo, lsSKU, lsSupplier, llOwnerCD, lsCOO, lsLocation, lsInventoryType, lsSerialNo, lsLotNo, lsPoNo, lsPoNo2, llLineItemNo, lsContainerID, ldtExpDate, llCompNo, lsZoneID)
...
/* GailM 5/11/2020 - DE15589 Google - remove coding to simulate bolding (HEX symbols) ******/
If lbContinue and Not lbMixedContainerization Then
	lsMsg =  "Do you want to remove the existing Pallet ID "+isPoNo2+" against Container ID "+ls_container_id+&
				" and "+string(llSplitParticipants) + " other participant container(s), for this shipment? ~r~n~r~nClick Yes to remove or No to retain it."
...
public function string getrono (string asdono, integer aslineitemno);//GailM 5/12/2020 - S45954 F22932 I2621 Google DA Kitting move to Spoke Warehouse - Putaway
String lsRoNo

Select ro_no Into :lsRoNo
From Delivery_Picking_Detail with (nolock)
Where do_no = :asdono And line_item_no = :aslineitemno
Using SQLCA;
...
Else
	...
	If ibPharmacyProcessing Then
		lsFilter = "" 
	ElseIf isPoNo2 = gsFootPrintBlankInd or isColorCode = "5" Then
	...
	If (isColorCode = "9" or isColorCode = "" or isColorCode = "0") and idw_pick.rowcount() > 0 Then
		ilDetailReqQty = idw_pick.GetItemNumber(iiCurrPickRow,"quantity")
...
event add_2dbarcode_sn();//GailM 2/20/2020 Google Footprints GPN Conversion Process adding serial numbers by 2dBarcode
Long llRow, llDel, llSNRowCount, llFound, llDw1RowCount
Integer liRtn
String lsFind, lsMsg

llSNRowCount = idsSerialsEntered.RowCount()
llDw1RowCount = dw_1.RowCount()

//For llRow = 1 to llRowCount
//	isSerialNo = idsSerialsEntered.GetItemString(llRow, "serial_no")
//	
//Next

If dw_1.rowcount() > 0 Then
	If llSNRowCount > llDw1RowCount Then
		lsMsg = "There are more serial numbers scanned than required.~r~nThis row requires " + String(llDw1RowCount) + " serial numbers."
		lsMsg += "~r~n~r~nPlease investigate and retry."
		MessageBox("Serial Number Warning", lsMsg)
		Return
	ElseIf llSNRowCount < llDw1RowCount Then
		lsMsg   = "WARNING:~r~n~r~n"
		lsMsg += "The number of serial numbers scanned (" + String(llSNRowCount) + ") is less than required for this row (" + String(llDw1RowCount) + ").~r~n~r~n"
		lsMsg += "Do you wish to load these SNs now?"
		If 	MessageBox("Serial Error", lsMsg, Question!, YesNo!) = 2 Then
			Return
		End If
	End If
	
	For llRow = 1 to llSNRowCount
		isSerialNo = idsSerialsEntered.GetItemString(llRow, "serial_no")
		lsFind = "serial_no = '" + isSerialNo + "' "
		llFound = dw_2.Find(lsFind, 1, dw_2.rowcount())
		If llFound > 0 Then
			messagebox("Serial Number Error","Serial Number " + isSerialNo + " has already been scanned.")
		Else
			//Does the serial number already exists in serial number inventory table?
			ipSerParms.String_Arg[1] = isSerialNo
			ipSerParms = f_serial_number_exists(ipSerParms)
			
			If ipSerParms.Long_Arg[1] = 0 Then
				dw_1.SetItem(1, "serial_no", isSerialNo)
				dw_1.RowsMove(1, 1, Primary!,  dw_2, dw_2.rowcount() + 1, Primary!)
				st_msg2.text = "Target: " + String(ilDetailReqQty) + "  Assigned: " + String(dw_2.rowcount())
				ibSNChanged = TRUE
			Else
				lsMsg = "The requested serial number already exists in serial number inventory table.~r~n"
	...
	Next
Else
	messagebox("Add serial numbers by 2dBarcode","There are no other serial numbers to add.")
End If
...
event modified;Long llRow, llDel, llRowCount, llSNRowCount, llFound, llDw1RowCount
Integer liRtn,liPos
String lsFind, lsMsg, lsSNMsg
isScanned = sle_1.text

liPos =  Pos(isScanned,",")
idsSerialsEntered = Create u_ds_ancestor
idsSerialsEntered.DataObject = 'd_pick_pallet'
idsSerialsEntered.SetTransObject(SQLCA)

idsSerialsEntered = f_parse_string_to_datastore(isScanned, ",")
llSNRowCount = idsSerialsEntered.RowCount()
llDw1RowCount = dw_1.RowCount()

If isColorCode = "9" Then			//Entering serial numbers
	If  liPos > 0 Then
		This.TriggerEvent("add_2dbarcode_sn")
	Else
	...
	For llRow = 1 to llSNRowCount
		...
		Else
			...
			Else
				...
				If llFound > 0 Then
					dw_1.RowsMove(llFound, llFound, Primary!,  dw_2, dw_2.rowcount() + 1, Primary!)
					st_msg2.text = "Target: " + String(ilDetailReqQty) + "  Assigned: " + String(dw_2.rowcount())
					ibSNChanged = TRUE
				Else
					lsSNMsg += "Serial number " + isSerialNo + " is not on the list of scanable SNs.~r~n"
				End If
			End If
		End If
	Next
...
If lsSNMsg > '' Then
	lsSNMsg += "~r~n~r~nPlease investigate and retry."
	MessageBox("Serial Number Error", lsSNMsg)
End If
...
Int liRtn, liRow
String lsPalletId, ls_old_pono2
	...
	If llDw1Rows > 0 Or llDw2Rows > 0 Then
		...
		ls_old_pono2 = dw_1.getItemString( 1, 'po_no2', primary!, true)
		If ls_old_pono2 = lsPalletId Then 
			This.Text = ""
			This.SetFocus()
			Return
		ElseIf len(lsPalletId) < 5 and lsPalletId <> 'NA' Then
			Messagebox("Change Pallet","Pallet Id must be over 4 characters except the value NA.  Please re-enter.")
			This.Text = ""
			This.SetFocus()
			Return 
		End If
...
Long llDw1Rows, llDw2Rows, llDw1Row, llDw2Row
Int liRtn, liRow
String lsContainerId, ls_old_containerId
...
//Else
	...
	If llDw1Rows > 0 Or llDw2Rows > 0 Then
		//GailM 8/11/2020 S48701 F24564 Google - Prevent N/A on put away and stock adjustment and stock adjustment
		//Force container_id to 5 or more characters and do not allow anything less that 5 to be anything but NA
		ls_old_containerId = dw_1.getItemString( 1, 'carton_id', primary!, true)
		If ls_old_containerId = lsContainerId Then 
			This.Text = ""
			This.SetFocus()
			Return
		ElseIf len(lsContainerId) < 5 and lsContainerId <> 'NA' Then
			Messagebox("Change ContainerID","Container Id must be over 4 characters except the value NA.  Please re-enter.")
			This.Text = ""
			This.SetFocus()
			Return 
		End If
