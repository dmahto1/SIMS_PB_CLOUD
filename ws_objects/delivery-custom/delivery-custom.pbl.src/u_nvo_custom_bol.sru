$PBExportHeader$u_nvo_custom_bol.sru
...
//SARUN2015JUNE29 :1st row should be displayed as “ 1 Packing List” (1 is by default value and no hyphen is required		
...
//SARUN2015JUNE29 : 2nd row should be displayed as “ XX White wood Pallets” (XX refers to the count). If no White wood pallets are available, display it as 0 White wood Pallets
...
//SARUN2015JUNE29 :		3rd row should be displayed as “ XX CHEP Pallets” (XX refers to the count). If no White wood pallets are available, display it as 0 CHEP Pallets
		if lsChepMsg > '' then
//			li_rtn_item = adw_bol.insertrow(0)
			adw_bol.RowsCopy (1, 1, Primary!, adw_bol, li_rtn_item +1, Primary!)
...
//02-Feb-2020 :MikeA - Added New BOL for 'DOHMH-DAYT'

string ls_c_name,ls_c_add1,ls_c_add2,ls_c_add3,ls_c_add4,ls_c_city,ls_c_state,ls_c_zip

if Upper(w_do.idw_main.GetITemString(1,'wh_Code')) = 'DOHMH-DAYT'  then

	//(Additional requirement; not noted on the BOL image): When outbound orders contain warehouse code DOHMH-DAYT, print this BOL. No other BOL should print when using this warehouse code and this BOL should not print for any other customer or warehouse.

	
	 adw_bol.object.p_1.X = 2245

	//1. Under the BILL OF LADING header, change the current City of NY EM address to:

     adw_bol.object.t_address_1.text = 'New York City'
     adw_bol.object.t_address_2.text = 'Department of Health and Mental Hygiene'
     adw_bol.object.t_address_3.text = '42-09 28th Street, 6th Floor, CN-22E'
     adw_bol.object.t_address_4.text = 'Long Island City, NY 11101-4132'


	// 2. Remove the blue NYC logo. Do not replace with anything else.	

	adw_bol.object.p_blue_logo.visible = false
	adw_bol.object.p_1.X = 2245  //Move XPO Logo over.

	//3. Change the Warehouse Address to:

           //XPO Logistics, Inc. c/o DOHMH
	      //2349 US Rt. 130, Suite B
           //Dayton, NJ, 08810
			 
		//Completed in Warehouse table. 	 
	
	//4.  Remove all Shipper Notes. Do not replace with anything else.

//MIKEA - 05/2020 - DE15361 - NYCSP-Defect - Remove Text from DOHMH BOL Shipper Notes (r20.2)
//   	adw_bol.object.t_shipping_instructions.visible = false
//	adw_bol.object.shipping_instructions.visible = false
	
	//Make black bar wider. 
//	adw_bol.object.t_42.width = 1957

	
	//5. In the Carrier access liability$$HEX2$$26202000$$ENDHEX$$section at the bottom of the BOL, replace $$HEX1$$1c20$$ENDHEX$$City of New York$$HEX2$$1d202000$$ENDHEX$$with $$HEX1$$1c20$$ENDHEX$$Department of Health and Mental Hygiene$$HEX2$$1d202000$$ENDHEX$$in two places.
	
	//MikeA - S45616 - I2913 - KNY-NYCSP - Update BOL Shipper’s Certification
	
	adw_bol.object.t_carrier_acess_liability_1.text = 'Carrier accepts liability for goods except as noted above at time of pickup. Payment of freight is by the Department of Health and Mental Hygiene in accordance with pre-established contract and protocols with XPO'
	adw_bol.object.t_carrier_acess_liability_2.text = 'Logistics and subordinate contracs between XPO Logistics and the designated carrier. The Department of Health and Mental Hygiene will not assume liability for disagreements between XPO Logistics and '
	adw_bol.object.t_carrier_acess_liability_3.text = 'contracted carrier.'
...
String ls_custcode,ls_custname, ls_carton_type, lsSyntax, lsErrors
...
string	lsCustName, lsPrefix	//GailM 4/1/2020 lsCustName replaced by lsPrefix
boolean lbTHD, lbWAY
...
//MikeA - 02/05/2020 - DE13961 - Don't set Qty to 1 for THD. All 16 lines. 
...
//GailM 4/1/2020 S44399 Bosch takes on Wayfair as they do HomeDepot
select Left(Cust_Name,4) into :lsPrefix from Delivery_Master where Delivery_Master.DO_No= :as_dono using SQLCA;

if lsPrefix = 'THD-' Then lbTHD = True			//HomeDepot
If lsPrefix = 'WAY-' Then lbWAY = True		//Wayfair
...
// TAM 2014/11/5 Added Supplemetal Report if more than one page
	//Suplemental page
	lds_bol_suplm = Create datastore
	If Not lbTHD and Not lbWAY then
...
adw_bol.SetTransObject(SQLCA)
lsSyntax = adw_bol.Describe("Datawindow.Table.Select")
...
For ll_page=1 to liNbrPages //29-Aug-2015 :Madhu- Loop through each BOL to Print
	...
	//Header Info
		...
		// TAM 2018/07/20 - S21706 
		// GailM 4/1/2020 S44399  - Bosch takes on Wayfair as they do HomeDepot	Add Wayfair .. this may change
		If lbTHD or lbWAY then
			adw_bol.SetItem(ll_page,"user_field1", lds_bol_detail.GetitemString(1,"user_field1"  )) //PO Number  from DMA CO/N9
...
//		End If
			...
			For i = ll_count+1 to 	ll_detailrowcount
				If i = ll_count+1 Then
						...
						If lbTHD or lbWAY then
							...
							//ld_total_thd_qty ++
							ld_total_thd_qty += adw_bol.GetItemDecimal(ll_page,"pack_qty1")
						End If
				...
				ElseIf i = ll_count+2 Then
						...
						If lbTHD or lbWAY then
							...
							//ld_total_thd_qty ++
							ld_total_thd_qty += adw_bol.GetItemDecimal(ll_page,"pack_qty2")
						End If
				...
				ElseIf i = ll_count+3 Then
						...
						If lbTHD or lbWAY then
							...
							//ld_total_thd_qty ++
							ld_total_thd_qty += adw_bol.GetItemDecimal(ll_page,"pack_qty3")
						End If
				...
				ElseIf i = ll_count+4 Then
						...
						If lbTHD or lbWAY then
							...
							//ld_total_thd_qty ++
							ld_total_thd_qty += adw_bol.GetItemDecimal(ll_page,"pack_qty4")
						End If
				...
				ElseIf i = ll_count+5 Then	
						...
						If lbTHD or lbWAY then
							...
							//ld_total_thd_qty ++
							ld_total_thd_qty += adw_bol.GetItemDecimal(ll_page,"pack_qty5")
				...
				ElseIf i = ll_count+6 Then	
						...
						If lbTHD or lbWAY then
							...
							//ld_total_thd_qty ++
							ld_total_thd_qty += adw_bol.GetItemDecimal(ll_page,"pack_qty6")
				...
				ElseIf i = ll_count+7 Then
						...
						If lbTHD or lbWAY then
							...
							//ld_total_thd_qty ++
							ld_total_thd_qty += adw_bol.GetItemDecimal(ll_page,"pack_qty7")
				...
				ElseIf i = ll_count+8 Then
						...
						If lbTHD or lbWAY then
							...
							//ld_total_thd_qty ++
							ld_total_thd_qty += adw_bol.GetItemDecimal(ll_page,"pack_qty8")
				...
				ElseIf i = ll_count+9 Then
						...
						If lbTHD or lbWAY then
							...
							//ld_total_thd_qty ++
							ld_total_thd_qty += adw_bol.GetItemDecimal(ll_page,"pack_qty9")
				...
				ElseIf i = ll_count+10 Then
						...
						If lbTHD or lbWAY then
							...
							//ld_total_thd_qty ++
							ld_total_thd_qty += adw_bol.GetItemDecimal(ll_page,"pack_qty10")
				...
				ElseIf i = ll_count+11 Then	
						...
						If lbTHD or lbWAY then
							...
							//ld_total_thd_qty ++
							ld_total_thd_qty += adw_bol.GetItemDecimal(ll_page,"pack_qty11")
				...
				ElseIf i = ll_count+12 Then	
						...
						If lbTHD or lbWAY then
							...
							//ld_total_thd_qty ++
							ld_total_thd_qty += adw_bol.GetItemDecimal(ll_page,"pack_qty12")
				...
				ElseIf i = ll_count+13 Then
						...
						If lbTHD or lbWAY then
							...
							//ld_total_thd_qty ++
							ld_total_thd_qty += adw_bol.GetItemDecimal(ll_page,"pack_qty13")
				...
				ElseIf i = ll_count+14 Then
						...
						If lbTHD or lbWAY then
							...
							//ld_total_thd_qty ++
							ld_total_thd_qty += adw_bol.GetItemDecimal(ll_page,"pack_qty14")
				...
				ElseIf i = ll_count+15 Then	
						...
						If lbTHD or lbWAY then
							...
							//ld_total_thd_qty ++
							ld_total_thd_qty += adw_bol.GetItemDecimal(ll_page,"pack_qty15")
				...
				ElseIf i = ll_count+16 Then	
						...
						If lbTHD or lbWAY then
							...
							//ld_total_thd_qty ++
							ld_total_thd_qty += adw_bol.GetItemDecimal(ll_page,"pack_qty16")
			...
			//Total Qty and total Weight by Order number
			// GailM 4/1/2020 S44399  - Bosch takes on Wayfair as they do HomeDepot	Add Wayfair 
			If lbTHD or lbWAY then
