...
//	through the messagerouter event.
//
//	Sequence:
//	If application is MDI:
//	1) Active MDI sheet messagerouter event.
//	2) MDI Frame Window messagerouter event.
//
//	Application is SWI:
//	1) ParentWindow messagerouter event.
//
//////////////////////////////////////////////////////////////////////////////
//
//	Revision History
...
if IsValid (lw_frame) then
	...
	lw_sheet = lw_frame.GetActiveSheet()
	if IsValid (lw_sheet) then
		li_rc = lw_sheet.dynamic event messagerouter (as_message)
	end if

	if li_rc <> 1 then
		// Try sending the message to the frame
		li_rc = lw_frame.dynamic event messagerouter (as_message)
	end if
else
	// Try sending the message to the parentwindow
	if IsValid (am_source.parentwindow) then
		li_rc = am_source.parentwindow.dynamic event messagerouter (as_message)
	end if
end if
...
//	of_MessageBox ('menu_msgnotreceived', gnv_app.inv_debug.ics_pfc, &
