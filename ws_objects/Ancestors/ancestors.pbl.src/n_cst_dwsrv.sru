...
//				ll_rc = idw_Requestor.Event retrievedddw(ls_colname)
...
ELSE
	...
	If ll_rc > 0 Then
//		ll_rc = idw_requestor.event populatedddw (as_dddwname, ldwc_obj)
		if ll_rc < 0 then return -1
	End If
END IF 
...
FOR ll_cnt=1 TO ll_columncount
	...
	ELSE
		...
		ll_rc = idw_Requestor.GetChild (ls_colname, ldwc_obj) 
		If ll_rc > 0 Then
//			ll_rc = idw_requestor.event populatedddw (ls_colname, ldwc_obj)
			if ll_rc < 0 then return -1
