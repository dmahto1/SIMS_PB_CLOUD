$PBExportHeader$w_maintenance_user.srw
$PBExportComments$-
forward
...
// 07/20 - PCONKL - S48292 - Only Super Duper and Provisioning (3) can update users now

//If gs_role = '0' or gs_role = '-1' Then
If gs_role = '3' or gs_role = '-1' Then
...
//07/20 - PCONKL - S48292 - Only provisioning and super duper can create users
If gs_role = '-1' or gs_role = '3' Then
Else
	Messagebox('User Maintenance','Only the Provisioning Team can create new users',Stopsign!)
	return
End If
...
If gs_role = '-1' or gs_role = '3' Then
Else
	Messagebox('User Maintenance','Only the provisioning team can modify user profiles.',Stopsign!)
	Return -1
End If
...
IF NOT IsNull(ls_code) Or ls_code <> '' THEN
	...
	IF ib_edit THEN								    // Edit Mode
		IF ll_rows > 0 THEN
			...
			// 02/08 - PCONKL - Added Super Duper User
			// 07/2020 - PCONKL - SS48292 - Added provisioning role (3)
			if gs_role = '-1' or gs_role = '3'  Then
			...
			// 07/2020 - aleart user they won't be able to save if not provisioning team
			if gs_role = '-1' or gs_role = '3'  Then
			Else
				Messagebox('User Maintenance','Only the Provisioning team can change User Profiles')
			End If
			
			// 08/00 PCONKL - Only Super can access Users from other projects
			//02/08 - PCONKL - Added Super Duper User
			// 07/2020 - PCONKL - SS48292 - Added provisioning role (3) and remove Super
			//If gs_role <> '0' Then
...
// 08/20 - PCONKL - If not Pandora, hide Commodity fields
If gs_project <> "PANDORA" Then
	this.modify("commodity_authorized_user_t.visible=false commodity_authorized_user.visible=false t_1.visible=False")
End If
end event
...
if not isNull(ls_string) and ls_string <> '' then
	Choose case ls_string
		...
		Case 'Operator'
			llAccessLevel = 2
		Case 'Provisioning' /* 08/20 - PCONKL - S48292*/
			llAccessLevel = 3
	End Choose
...
long textcolor = 33554432
boolean vscrollbar = true
string item[] = {"Super","Admin","Operator","Provisioning"}
borderstyle borderstyle = stylelowered!
end type
...
//07/20 - PCONKL - S48292 - Only Provisioning team (3) can make these changes

If gs_Project = 'PANDORA' Then
	dwItemStatus l_status
	//If gs_role <> '0' and gs_role <> '-1' Then //Super and Super Dupers are excluded
	If gs_role <> '3' and gs_role <> '-1' Then //Provisioning and Super Dupers are excluded
		llRow = Row
...
For llRowPos = 1 to llRowCount
	...
	// 08/20 - PCONKL - S48292 - Allow provisioning (3) to access all projects
	
	If gs_role <> "-1" and gs_role <> "3"  then /*Not super duper or provisioning (3)*/
