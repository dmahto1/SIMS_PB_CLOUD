$PBExportHeader$w_outbound_confirmation.srw
$PBExportComments$OutBound Order Report (GAP - 8/02)
forward
global type w_outbound_confirmation from w_std_report
end type
type uo_order_status_search from uo_multi_select_search within w_outbound_confirmation
end type
end forward
...
integer width = 3566
integer height = 2100
string title = "Outbound Confirmation Report"
event ue_load_philips_notes ( )
uo_order_status_search uo_order_status_search
end type
global w_outbound_confirmation w_outbound_confirmation
...
on w_outbound_confirmation.create
int iCurrent
call super::create
...
on w_outbound_confirmation.destroy
call super::destroy
...
//hdc 12/10/2012  This report doesn't work because of the inner join to delivery_packing causing lines to be repeated
// The join has been removed from the data window, so the tracking_number field must be fetched and updated manually
//  do_no has been included on the select for the retrieval  

String lsDO_NO, lsTracking_No, lsSKU, lsOldSKU, lsOldDoNo
long llRowNum, llLine_No, llOld_Line
Datastore ldsSTID

ldsSTID = create datastore  //we're going to use sku, detail line item and do_no to get a list of tracking ids
ldsSTID.dataobject = 'd_shipper_tracking_id'
ldsSTID.SetTransObject(SQLCA)
lsOldSKU = ""
llOld_Line = -1
lsOldDoNo = ""

for llRowNum = 1 to dw_report.RowCount() 
	lsDO_NO = dw_report.GetItemString(llRowNum, 'Do_No')		
	lsSKU = dw_report.GetItemString(llRowNum, 'delivery_detail_sku')
	llLine_No = dw_report.GetItemNumber(llRowNum, 'delivery_detail_line_item_no')
	if dw_report.GetItemString(llRowNum, 'delivery_serial_detail_serial_no') <> "" then
		dw_report.SetItem(llRowNum, 'ship_qty', 1)
	end if
	if lsSKU=lsOldSKU and llOld_Line = llLine_No and lsDO_NO = lsOldDoNo then  //only refetch when the line number/sku changes
		dw_report.SetItem(llRowNum, 'tracking_number', lsTracking_No)  //when it hasn't changed repeat the current shipper tracking id value
		continue
	end if
	ldsSTID.retrieve(lsDO_NO, llLine_No, lsSKU)
	lsTracking_No = ""  //re-init the string that will hold the concatenated tracking numbers
	for i = 1 to ldsSTID.RowCount()
		//only add tracking numbers that aren't empty and haven't been added to the string already
		if "" = ldsSTID.GetItemString(i, 'shipper_tracking_id') or Match(lsTracking_No, ldsSTID.GetItemString(i, 'shipper_tracking_id')) then
			continue
		end if
		if lsTracking_No <> "" then  //if this isn't the first pass precede the addition with a comma
			if len(lsTracking_No) > 35 then
				lsTracking_No += "+"
				continue
			end if
			lsTracking_No += ", " + ldsSTID.GetItemString(i, 'shipper_tracking_id')
		else	//first pass, no comma
			lsTracking_No += ldsSTID.GetItemString(i, 'shipper_tracking_id')
		end if
	next
	dw_report.SetItem(llRowNum, 'tracking_number', lsTracking_No)  //update the row's tracking ID
	lsOldSKU = lsSKU  //we're now on a new line item/sku so update the variables that tell us what's current
	llOld_Line = llLine_No
next 
...
// 05/09 - PCONKL - Different format for Philips, 12/12 - added TPV, 6/13 - added FUNAI, TAM 2015/03 - Added Gibson
//3-FEB-2019 :Madhu S28945 Added PHILIPSCLS
//Dinesh - 11/19/2020- S51443- PHILIPS-DA - Mimic PHILIPSCLS logic in Reports
If gs_project = 'PHILIPS-SG' OR gs_project = 'PHILIPS-TH' or gs_project='PHILIPSCLS' or gs_project = 'TPV' or gs_project = 'FUNAI' or gs_project = 'GIBSON'  or gs_project = 'PHILIPS-DA' Then
...
type dw_select from w_std_report`dw_select within w_outbound_confirmation
...
type cb_clear from w_std_report`cb_clear within w_outbound_confirmation
integer taborder = 20
end type

type dw_report from w_std_report`dw_report within w_outbound_confirmation
integer x = 5
integer y = 460
integer width = 3429
integer height = 1400
integer taborder = 30
string dataobject = "d_outbound_confirmation"
