$PBExportHeader$w_outbound_order.srw
$PBExportComments$OutBound Order Report (GAP - 8/02)
forward
global type w_outbound_order from w_std_report
end type
type uo_order_status_search from uo_multi_select_search within w_outbound_order
end type
end forward
...
global w_outbound_order w_outbound_order
...
//05-Feb-2014 :Madhu- commented below condition for baseline
...
// 
	...
	//05-Feb-2014 :Madhu- make it for baseline -START
	IF (is_warehouse_code <> "") and NOT isnull(is_warehouse_code) THEN
		ls_Where = ls_Where + " and delivery_master.WH_Code = '" + is_warehouse_code + "'" 
	END IF
	//05-Feb-2014 :Madhu- make it for baseline -END
...
//JXlim 07/22/2010 end of changed.	
	...
	Else
		...
		dw_select.Setfocus()
	End If 
//END IF //05-Feb-2014 :Madhu- commented condition for baseline

// 05/09 - PCONKL - For Philips, we want to retrieve the Delivery Notes, 12/12 - added TPV - 6/13 - MEA - added FUNAI, TAM 2015/03 - Added Gibson
//3-FEB-2019 :Madhu S28945 Added PHILIPSCLS
//Dinesh - 11/19/2020- S51443- PHILIPS-DA - Mimic PHILIPSCLS logic in Reports
If gs_project = 'PHILIPS-SG' or gs_project = 'PHILIPSCLS' or gs_project = 'TPV'  or gs_project = 'FUNAI'  or gs_project = 'GIBSON' or gs_project = 'PHILIPS-DA' Then
...
// MEA - 8/13 - Different format for STBTH
//PFC - 11/13 - different format for Ariens
If gs_project = 'STBTH' Then
	dw_report.dataobject = "d_starbuck_th_outbound_order"
	dw_report.SetTransObject(SQLCA)
Elseif gs_project = 'ARIENS' Then
	dw_report.dataobject = "d_ariens_outbound_orders"
	dw_report.SetTransObject(SQLCA)	
	//Added Outbound report for PE-THA  31-03 nxjain
Elseif gs_project = 'PE-THA' Then
	dw_report.dataobject = "d_outbound_order_petha"
	dw_report.SetTransObject(SQLCA)
	//Added Outbound report for PE-THA  31-03 nxjain
Elseif gs_project = 'ANKI' Then
	dw_report.dataobject = "d_outbound_order_anki"
	dw_report.SetTransObject(SQLCA)
//TAM 09/2016 Adding Stock Movement report for Pandora.
ElseIf gs_project = 'PANDORA' Then
	dw_report.dataobject = 'd_pandora_outbound_order'
	dw_report.SetTransObject(SQLCA)
//TAM 08/2018 S21781 Adding Outbound report for Coty.
ElseIf gs_project = 'COTY' Then
	dw_report.dataobject = 'd_coty_outbound_order'
	dw_report.SetTransObject(SQLCA)
End If
...
type dw_select from w_std_report`dw_select within w_outbound_order
...
dw_select.GetChild("wh_name", ldwc_warehouse)
ldwc_warehouse.SetTransObject(sqlca)
...
type cb_clear from w_std_report`cb_clear within w_outbound_order
integer taborder = 20
end type

type dw_report from w_std_report`dw_report within w_outbound_order
integer x = 5
integer y = 460
integer width = 3429
integer height = 1404
integer taborder = 30
string dataobject = "d_outbound_order"
