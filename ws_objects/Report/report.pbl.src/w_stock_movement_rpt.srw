$PBExportHeader$w_stock_movement_rpt.srw
forward
global type w_stock_movement_rpt from w_std_report
end type
end forward
...
global w_stock_movement_rpt w_stock_movement_rpt
...
on w_stock_movement_rpt.create
call super::create
end on

on w_stock_movement_rpt.destroy
call super::destroy
if IsValid(MenuID) then destroy(MenuID)
end on

event ue_retrieve;String ls_whcode, ls_sku, ls_ord_type,ls_filter,ls_supp,ls_order_no
...
ls_order_no= dw_select.getitemstring(1,"order_no") //24-Nov-2014 Madhu- Added search by Order No.
...
ll_cnt = dw_report.Retrieve(gs_project, ls_whcode,ls_sku, ls_supp) /* 05/01 PCONKL - including sku/supplier in retriveal arg instead of filtering*/
...
//24-Nov-2014 :Madhu- Added "Search by Order No" for PANDORA - START
IF not isnull(ls_order_no) and ls_order_no <> ''  THEN
	ls_filter = "orderno = '"+ls_order_no+"'"
	dw_report.setfilter( ls_filter)
	dw_report.filter( )
END IF
//24-Nov-2014 :Madhu- Added "Search by Order No" for PANDORA - END
...
event open;call super::open;//TAM W&S 2011/03/19  Added W&S
If left(gs_project,3) = 'WS-' Then
	dw_report.dataobject = 'd_stock_movement_rpt_ws'
	dw_report.SetTransObject(SQLCA)
End If

//MEA PHILIP-SG 2011/10  Added PHILIP-SG, 12/12 - PCONKL - added TPV - 6/13 - added FUNAI, TAM 2015/03 - Added Gibson
...
If gs_project = 'PHILIPS-SG' or gs_project ='PHILIPSCLS' or gs_project = 'TPV' or gs_project = 'FUNAI'  or gs_project = 'GIBSON' or gs_project ='PHILIPS-DA' Then
	dw_report.dataobject = 'd_stock_movement_rpt_philips_sg'
	dw_report.SetTransObject(SQLCA)
End If	
	
	
//MEA NIKE-SG AND NIKE-MY 2012/01  Added NIKE
If gs_project = 'NIKE-SG' OR gs_project = 'NIKE-MY'  Then
	dw_report.dataobject = 'd_nike_stock_movement_rpt'
	dw_report.SetTransObject(SQLCA)
End If	
...
if gs_project = 'KLONELAB' then
	dw_report.dataobject = "d_stock_movement_rpt_klonelab"
	dw_report.SetTransObject(SQLCA)
...
If gs_project = 'PANDORA' Then
	dw_report.dataobject = 'd_stock_movement_rpt_pandora'
	dw_report.SetTransObject(SQLCA)
End If

end event

type dw_select from w_std_report`dw_select within w_stock_movement_rpt
integer x = 18
integer width = 3131
...
type cb_clear from w_std_report`cb_clear within w_stock_movement_rpt
integer x = 3154
integer y = 12
integer width = 270
end type
...
type dw_report from w_std_report`dw_report within w_stock_movement_rpt
integer y = 188
integer width = 3401
integer height = 1704
string dataobject = "d_stock_movement_rpt"
