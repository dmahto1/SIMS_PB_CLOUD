$PBExportHeader$w_boxno_add_delete.srw
forward
global type w_boxno_add_delete from window
end type
type st_oneswap from statictext within w_boxno_add_delete
end type
type st_total_picked from statictext within w_boxno_add_delete
end type
type st_9 from statictext within w_boxno_add_delete
end type
type st_picked from statictext within w_boxno_add_delete
end type
type st_8 from statictext within w_boxno_add_delete
end type
type cb_cancel from commandbutton within w_boxno_add_delete
end type
type cb_ok from commandbutton within w_boxno_add_delete
end type
type dw_1 from u_dw_ancestor within w_boxno_add_delete
end type
type sle_qty_recd from statictext within w_boxno_add_delete
end type
type st_7 from statictext within w_boxno_add_delete
end type
type st_difference from statictext within w_boxno_add_delete
end type
type sle_owner from singlelineedit within w_boxno_add_delete
end type
type st_owner from statictext within w_boxno_add_delete
end type
type sle_pono from singlelineedit within w_boxno_add_delete
end type
type st_3 from statictext within w_boxno_add_delete
end type
type st_supplier from statictext within w_boxno_add_delete
end type
type st_6 from statictext within w_boxno_add_delete
end type
type st_5 from statictext within w_boxno_add_delete
end type
type sle_sku from singlelineedit within w_boxno_add_delete
end type
type st_4 from statictext within w_boxno_add_delete
end type
type cb_clear from commandbutton within w_boxno_add_delete
end type
type cb_delete from commandbutton within w_boxno_add_delete
end type
type st_2 from statictext within w_boxno_add_delete
end type
type st_totrows from statictext within w_boxno_add_delete
end type
type st_1 from statictext within w_boxno_add_delete
end type
type sle_boxno from singlelineedit within w_boxno_add_delete
end type
end forward
...
global w_boxno_add_delete w_boxno_add_delete
...
ids_detailWindow = istrparms.datawindow_arg[1]
...
If il_SingleBoxIdSwap = 0 then
	...
	ll_rowCount =ids_detailWindow.RowCount() 
	ids_detailWindow.SetFilter("sku = '" + is_Sku + "'")
Else
	This.st_oneSwap.visible = True
	ll_rowCount = 1
	ids_detailWindow.SetFilter("sku = '" + is_Sku + "'" + " and Line_Item_no = " + String(il_LineItemNo) + " and container_id = '" + is_ContainerID + "'")

End if

//ids_detailWindow.SetFilter("sku = '" + is_Sku + "'")
ids_detailWindow.Filter()

IF ids_detailWindow.RowCount() > 0 THEN
	FOR ll_row = 1 TO ids_detailWindow.RowCount() 
		If ids_detailWindow.GetItemString(ll_row,'user_field3') = "N" Then
			ll_SkuCount = ll_SkuCount +1
			ll_Qty = ll_Qty + ids_detailWindow.GetItemNumber(ll_row,'quantity')
		End If
	Next
End if

ids_detailWindow.SetFilter("")
ids_detailWindow.Filter()
...
ids_detailWindow.ScrollToRow( llPickRow )
...
w_boxno_add_delete.triggerevent(Close!)
end event
...
String lsSku
Long llRowCount, i
llRowCount = ids_detailWindow.Rowcount( )

For i=1 to llRowCount
	lsSku = ids_detailWindow.GetitemString( i, 'Sku')
	If lsSku <> '' then
	Exit
	End if
Next
...
llFound = ids_detailWindow.Find(lsFind,1,ids_detailWindow.RowCount())
If llFound > 0 then
	ids_detailWindow.scrolltorow( llFound )
	ls_Sku = ids_detailWindow.GetitemString(llFound, 'SKU')
	ll_LineNo = ids_detailWindow.GetitemNumber(llFound, 'Line_Item_No')
	ll_CurrentQty= ids_detailWindow.GetitemNumber(llFound, 'quantity')
	ll_Current = 1
End if
...
type tab_main from w_std_master_detail`tab_main within w_boxno_add_delete
