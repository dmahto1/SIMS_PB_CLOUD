$PBExportHeader$w_owner_change_recommend.srw
...
event clicked;// 03/20 - DE15365  - The total and index need to be decimal and long instead of integer
integer li_idx
...
ldTotal = istrparms.decimal_arg[1]
...
for li_idx = 1 to dw_current.RowCount()
	...
	if ldTotal > 0 then
		...
		if ldTotal >= ld_avail_qty then
			
			dw_current.SetItem( li_idx, "c_putaway_amt", ld_avail_qty)

			ldTotal = ldTotal - ld_avail_qty
			
		else
		
			dw_current.SetItem( li_idx, "c_putaway_amt", ldTotal)
