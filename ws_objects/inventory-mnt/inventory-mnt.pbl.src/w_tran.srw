...
If gs_project = 'PANDORA' Then
	...
	llRowCount = idw_content.RowCount()
	For llRowPos = 1 to llRowCount
		lsWhCode = idw_content.GetItemString(llRowPos,'wh_code')
		lsLCode = idw_content.GetItemString(llRowPos,'l_code')
		lsLocType = f_get_loc_type(lsWhCode, lsLCode)
		If lsLocType = "Z" Then
			lsSKU = idw_content.GetItemString(llRowPos,'sku') 
			lsSupplier = idw_content.GetItemString(llRowPos,'supp_code') 
			ibFootprintInd = f_is_sku_foot_print(lsSKU, lsSupplier)		//pono2 and containerId have already been changed to NA/NA
			lsRoNo = idw_content.GetItemString(llRowPos,'ro_no') 
			lsCOO = idw_content.GetItemString(llRowPos,'country_of_origin') 
			lsPoNo = idw_content.GetItemString(llRowPos,'po_no') 
			lsPoNo2 = idw_content.GetItemString(llRowPos,'po_no2') 
			lsCntrId = idw_content.GetItemString(llRowPos,'container_id') 
			ldOwnerId = idw_content.GetItemNumber(llRowPos,'owner_id') 
...
//	End If
	...
	for j = ll_totalrows to 1 Step -1
		//Unlink serial number inventory records from detail rows if sku is serial tracked
...
//Dinesh - S51444- 11/23/2020- Philips-da
if (Upper(Trim( gs_project )) = 'PHILIPS-SG' or Upper(Trim( gs_project )) = 'PHILIPSCLS' or Upper(Trim( gs_project )) = 'PHILIPS-DA') and Upper(Trim( ls_whcode )) = 'PHILIPS-MY' and Upper( idw_main.Object.Ord_Type[ 1 ] ) = 'W' then
	lb_rework = TRUE
end if
...
for i = 1 to idw_detail.rowcount()
	...
	ls_lcode = Upper(idw_detail.getitemstring(i,'s_location',Primary!,True))
	ls_dlcode = Upper(idw_detail.getitemstring(i,'d_location',Primary!,True))
	ls_LocType = f_get_loc_type(isWarehouse, ls_dlcode)		//S45954		Used for DA kitting process
	ls_sno = Upper(idw_detail.getitemstring(i,'serial_no',Primary!,True))
	ls_lno = Upper(idw_detail.getitemstring(i,'lot_no',Primary!,True))
	ls_pono = Upper(idw_detail.getitemstring(i,'po_no',Primary!,True))
	...
	If trim(ls_dlcode) > '' then
		If ls_LocType = 'Z' Then
			If  f_is_sku_foot_print(ls_sku, lsSupplier) Then
	...
	//TimA Commend out 08/06/14
		...
		If ll_currow <= 0 Then
			...
			//idw_content.setitem(ll_currow,'inventory_type',ls_itype)
			idw_content.setitem(ll_currow,'inventory_type',ls_new_itype)		
			idw_content.setitem(ll_currow,'serial_no',ls_sno)
			idw_content.setitem(ll_currow,'lot_no', ls_lno)
			idw_content.setitem(ll_currow,'po_no', ls_pono)
			
			If ls_LocType = 'Z' and f_is_sku_foot_print(ls_sku, lsSupplier) Then			//GailM 5/15/2020 S45954	GailM 10/8/2020 DE18065 add footprint check
				idw_content.setitem(ll_currow,'container_ID',gsFootPrintBlankInd) 
				idw_content.setitem(ll_currow,'po_no2',gsFootPrintBlankInd)	
			Else
				idw_content.setitem(ll_currow,'container_ID', ls_container_ID) 
				idw_content.setitem(ll_currow,'po_no2', ls_pono2)	
			End If
			idw_content.setitem(ll_currow,'expiration_date', ldt_expiration_date)
...
dcon = idw_dcontent.GetItemDateTime(j,'last_cycle_count')
		...
		if ls_new_itype <> ls_itype then
			...
			If ls_LocType = 'Z' Then			//GailM 5/15/2020 S45954 If destination location is DA Kitting set pono2 to NA
				lds_adjustment_insert.Object.PO_No2[ ll_row ] = gsFootPrintBlankInd
			Else
...
If (g.ibSNChainOfCustody  and lsEnforceLocUpdate = 'Y' and ll_rtn = 1) or (ll_rtn = 1 and lb_require_serial_capture) Then	// LTK 20160105  Pandora #1002 added the Pandora single project rule check
	...
	For llRowPOs = 1 to llRowCount
		...
		If ls_LocType = 'Z' Then			//GailM 5/15/2020 S45954 DA Kitting
			ls_pono2 = gsFootPrintBlankInd
			ls_container_id = gsFootPrintBlankInd
		Else
			ls_pono2 = ldstransferSerial.GetITemString(llRowPos,'po_no2')		
			ls_container_id = ldstransferSerial.GetITemString(llRowPos,'container_id')
		End If
// TAM 2019/05 - S33409 - Populate Serial History Table
		Update Serial_Number_Inventory 
		Set l_code = :lsLoc, po_no2 = :ls_pono2, carton_id = :ls_container_id,
			Transaction_Type = 'TRANSFER', Transaction_ID = :ls_tono, Adjustment_Type = 'LOCATION'
	...
	If ls_LocType = 'Z' Then		//GailM 5/15/2020 S45954 DA Kitting
		MessageBox(is_title,"Destination " + lsLoc + " is a DA Kitting location.  Pallet and carton Id have been changed to " + gsFootPrintBlankInd + ".")
	End If
...
long llFootprintSerialNumbersPresent, llNbrSerialLinked
...
FOR ll_idx  = 1 to idw_detail.RowCount()
	...
	If lbFootprint and lsGPNConversionFlag = 'Y' Then
		...
		If llFootprintSerialNumbersPresent <> 0 Then
			...
			If ll_Qty < llFootprintSerialNumbersPresent Then		//Quantity required is less that difference between Content and Serial Number Inventory
				lsMsg4 += "Detail row " + string(ll_idx) + " requires " + string(ll_Qty) + " serial numbers.  Missing serial numbers"
				lsMsg4 += " for this WH/GPN/Loc/Pallet/Container: " + string(ABS(llFootprintSerialNumbersPresent)) + "~n~r~n~r"
			Else
				lsMsg4 += "~n~rDetail row " + string(ll_idx) + " has " + string(Abs(llFootprintSerialNumbersPresent)) + " missing serial numbers for this WH/GPN/Loc/Pallet/Container.~n~r~n~r" 
			End If
	...
	End If
		...
		If ls_pono_2 = gsFootPrintBlankInd or ls_container_id = gsFootPrintBlankInd Then 
			...
			ElseIf  (ls_pono_2 = gsFootPrintBlankInd and ls_container_id = gsFootPrintBlankInd) OR (ls_pono_2 = '-' and ls_container_id = '-') Then
	...
	If lbFootprint then		//not loose serials
		...
		IF Not lbAllocated Then
			...
			ElseIf ibMixedContainerization and ll_Qty <> ll_count and ll_count > 0 Then	//MCL needs split
				...
				lsMsg4 += "Footprint GPN " + ls_sku + " must maintain full pallet and containers.~n~r~n~r" + &
					...
					"Once identified, the serial number(s) allocated will be linked to this order for transfer.~n~r~n~r"
...
If lsMsg4 > "" Then		//Mixed containerization needs split (8)	
	lsMsg4 += "Double-click pick list ContainerId column to enter serial numbers. " 
...
if gs_Project = 'PANDORA'  and ll_cnt >  0 Then	
	...
	ls_dLocation = idw_detail.GetItemString(1, 'd_location')
