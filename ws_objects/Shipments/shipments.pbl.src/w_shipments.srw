$PBExportHeader$w_shipments.srw
...
Choose Case ls_status
	...
	*/		
	...
	//When a shipment is first dropped (Shipment_Ord_status = ‘N’ or ‘P’ ) to SIMS we need the following fields locked from edit 	
...
if Upper(gs_project) = 'CHINASIMS' then
	...
	tab_main.tabpage_main.tab_locations.tabpage_origin.text = "发货方"
	tab_main.tabpage_main.tab_locations.tabpage_dest.text = "收货方"
...
If	ib_UpdateOrderRemoveStatus = True Then
	If Upper(gs_project) = 'PANDORA'  THEN	
		//Jxlim BRD #437 If user clicks Yes, do the following: Change Delivery_Master.OTM_Status = ‘N”, Non OTM and  Set Delivery_Master.Consolidation_no = Null
		lsordstatus = 'N'
...
If This.Trigger Event ue_save() = 0 Then
	...
	//If the user clicks Yes, do the following Change Delivery_Master.OTM_Status = ‘N”, Non OTM  Set Delivery_Master.Consolidation_no = Null
Else
...
//				End if
				...
				//Then if Customer.User_Field1 = ‘ENT’ proceed with the order confirmation process. 	
				Else
					Select		Count(dm.do_no) Into : ll_cnt
					From   		Delivery_Master DM, Customer C
					Where 		DM.project_id = 'PANDORA' 
	...
	If THis.GetITemString(llRowPos,'c_delete_ind') = 'Y' Then
		...
		If idw_Main.GetITemString(1,'ord_type') <> 'I' Then
			...
			If This.GetITemstring(llRowPos,'rodo_no') <> lsDoNoHold Then /*we may have more than 1 row per dono*/
				...
				//If the user clicks Yes, do the following: Change Delivery_Master.OTM_Status = ‘N”, Non OTM and  Set Delivery_Master.Consolidation_no = Null
//				Update Delivery_Master
