$PBExportHeader$w_geistlich_part_label_print.srw
forward
global type w_geistlich_part_label_print from w_main_ancestor
end type
type cb_print from commandbutton within w_geistlich_part_label_print
end type
type cb_selectall from commandbutton within w_geistlich_part_label_print
end type
type cb_clear from commandbutton within w_geistlich_part_label_print
end type
type cbx_show_comp from checkbox within w_geistlich_part_label_print
end type
type cbx_2d_barcode from checkbox within w_geistlich_part_label_print
end type
type dw_label from u_dw_ancestor within w_geistlich_part_label_print
end type
type cb_retrieve from commandbutton within w_geistlich_part_label_print
end type
end forward
...
integer width = 4393
integer height = 1940
string title = "Geistlich Part Labels"
string menuname = ""
event ue_print ( )
...
global w_geistlich_part_label_print w_geistlich_part_label_print
...
////IF f_retrieve_parm("GEISTLICH", "FOOTPRINT", "FOOTPRINT_LABELS" ,"USER_UPDATEABLE_IND") = 'N'  THEN
...
Long	llQty,	&
		llRowCount,	&
		llRowPos, &
		ll_rtn, llPrintJob, llFindRow, ll_ownerid,llqtycount
...
//Dinesh - 09132021- S61867- Geistlich kitting labels
...
If llRowCount > 0 Then
	For llRowPos = 1 to llRowCount /*each detail row */
		llQTY = dw_label.GetITemnumber(llRowPos,'quantity')
     	IF llQTY >= 1 then
			for llqtycount = 1 to llQTY 
				lsFormat = "geistlich_part_label_common.txt"
				lsLabel 		= lu_Labels.uf_read_label_Format(lsFormat)
				lsCurrentLabel			= lsLabel
			IF lsLabel 		= "" Then Return	
			lsPrintText = 'Geistlich Part Labels ' ///Dinesh - 09092021- S61867- Geistlich kitting label
		...
		IF Isnull(ls_alt_sku) THEN 
			...
			// Begin - Dinesh - 09/14/2021- S61867- Geistlich kitting labels
			if ib_check= true then
				//lsexpiryDate = String(w_do.idw_main.GetITemDateTime(1,'Complete_Date'),'DD-MMM-YYYY')
				lsexpiryDate = String(dw_label.GetITemDateTime(llRowPos,'workorder_putaway_expiration_date'),'DD-MMM-YYYY')
			else
				lsexpiryDate = String(dw_label.GetITemDateTime(llRowPos,'receive_putaway_expiration_date'),'DD-MMM-YYYY')
				
			end if		
			lsCurrentLabel = lu_labels.uf_replace(lsCurrentLabel,"~~EXPIRYDATE~~", lsexpiryDate)
			// End - Dinesh - 09/14/2021- S61867- Geistlich kitting labels
...
//			// Get the owner ID.
			...
			// Get the owner.
			//ls_owner = dw_label.GetItemstring ( llRowPos, "receive_master_user_field2" ) // Dinesh - 09092021
			// Begin - Dinesh - 09/09/2021-  S61867- Geistlich kitting labels 
			if ib_check= true then
				ls_owner = dw_label.GetItemstring ( llRowPos, "workorder_master_user_field2" )
			else
				ls_owner = dw_label.GetItemstring ( llRowPos, "receive_master_user_field2" )
			end if
			// End - Dinesh - 09/09/2021- S61867- Geistlich kitting labels
			...
			INTO :ls_userfield1 
			from customer
			where customer.project_id = 'GEISTLICH' 
	...
	// TAM 04/2017 - Change label to us a standard label for Print and Reprint
			...
			//if ib_check= true then
				lsCurrentLabel = lu_labels.uf_replace(lsCurrentLabel,"~~EXPIRYDATE~~", lsexpiryDate) //Dinesh - 09142021- S61867- Geistlich kitting labels
			//end if
...
event ue_select_marked();//Begin - Dinesh - 11092021- DE23654- GEISTLICH - WO lables all lables are getting printed. Not the selected ones only

Long	llRowPos,	&
		llRowCount
	
		
String ls_SKU, ls_SKU_Parent,ls_print
boolean lb_select
...
If llRowCount > 0 Then
	...
	For llRowPos = 1 to llRowCount
		 ...
		 is_print  				= dw_label.GetItemString( llRowPos , "c_print_ind"	) 
	 
		 If IsNull(  ls_SKU ) 			or Trim( ls_SKU ) = '' 			Then  ls_SKU = ''
		 If IsNull(   ls_SKU_Parent ) 	or Trim( ls_SKU_Parent ) = '' 	Then 	ls_SKU_Parent = ''
	 
		 If  ls_SKU <> '' and ls_SKU_Parent <> '' and   ls_SKU =  ls_SKU_Parent Then 
			if ib_select= true then
				dw_label.SetITem(llRowPos,'c_print_ind','Y')
			else
	 			dw_label.SetITem(llRowPos,'c_print_ind',is_print)
			end if
		end if
	Next
...
//End - Dinesh - 11092021- DE23654- GEISTLICH - WO lables all lables are getting printed. Not the selected ones only
...
on w_geistlich_part_label_print.create
int iCurrent
call super::create
...
this.Control[iCurrent+5]=this.cbx_2d_barcode
this.Control[iCurrent+6]=this.dw_label
this.Control[iCurrent+7]=this.cb_retrieve
end on

on w_geistlich_part_label_print.destroy
call super::destroy
...
IF not isvalid(w_ro) and not isvalid(w_tran) and not isvalid(w_owner_change) and not isvalid(w_workorder)Then // Dinesh - 09/09/2021-S61867- Geistlich kitting labels
	Messagebox("Labels","You must have an open Receiving Order, Transfer Order or SOC or Work Order to print part labels.")  // Dinesh - 09/09/2021-S61867- Geistlich kitting labels
End If
if isvalid(w_workorder) then
		dw_label.dataobject = 'd_geistlich_wo_part_labels_nolock' // dinesh - 09/09/2021-S61867- Geistlich kitting labels
		ib_check=true
end if
...
IF isvalid(w_ro) and not isvalid(w_owner_change) and not isvalid(w_tran)Then //If Inbound open its DW
	...
	if f_retrieve_parm("GEISTLICH", "FOOTPRINT", "FOOTPRINT_LABELS" ,"USER_UPDATEABLE_IND") = 'N'  then
		dw_label.dataobject = 'd_geistlich_part_labels_no_label_consolidation' // dinesh - 09/09/2021-S61867- Geistlich kitting labels
	else
		dw_label.dataobject = 'd_geistlich_part_labels_nolock' // dinesh - 09/09/2021-S61867- Geistlich kitting labels
...
ElseIF not isvalid(w_ro) and not isvalid(w_owner_change) and not isvalid(w_workorder) and isvalid(w_tran) Then //If Transfer open its DW
	dw_label.dataobject = 'd_geistlich_part_labels_transfer_nolock' //Dinesh - 09092021- S61867- Geistlich kitting labels
	dw_label.settransobject( sqlca ) 
ElseIF not isvalid(w_ro) and not isvalid(w_tran)  and not isvalid(w_workorder) and isvalid(w_owner_change) Then //If OC open  its DW
	dw_label.dataobject = 'd_geistlich_part_labels_owner_change_nolock' //Dinesh - 09092021- S61867- Geistlich kitting labels
	dw_label.settransobject( sqlca ) 
Else
...
If not isvalid(w_ro) and not isvalid(w_tran) and not isvalid(w_owner_change) and not isvalid(w_workorder)Then Return //Dinesh - 09092021- S61867- Geistlich kitting labels

If isvalid(w_ro) Then
	If w_ro.idw_main.RowCount() < 1 then return
	//messagebox('',w_ro.idw_main.GetITemString(1,'ro_no'))
	dw_label.Retrieve(w_ro.idw_main.GetITemString(1,'ro_no'))

end if

// Begin -Dinesh - 09092021- S61867- Geistlich kitting label 
If isvalid(w_workorder) Then
string ls_text,ls_wo
ls_text='GEISTLICH'
ls_wo=w_workorder. idw_main.GetItemString(1,'workorder_number')
ls_text=ls_text +""+ ls_wo
	If w_workorder.idw_main.RowCount() < 1 then return
	dw_label.settrans(sqlca)
	dw_label.Retrieve(ls_text)
    dw_label.SetFilter('SKU = SKU_Parent')	
	dw_label.Filter()
end if
...
type cb_cancel from w_main_ancestor`cb_cancel within w_geistlich_part_label_print
boolean visible = false
integer x = 1938
integer y = 1392
integer height = 80
...
type cb_ok from w_main_ancestor`cb_ok within w_geistlich_part_label_print
integer x = 1257
integer y = 32
integer height = 80
integer textsize = -9
...
event clicked; //Begin - Dinesh - 11092021- DE23654- GEISTLICH - WO lables all lables are getting printed. Not the selected ones only
if ib_select= true then
	Parent.Event ue_selectall()
else
	Parent.Event ue_select_marked()
		 ///Dinesh - 09092021- S61867- Geistlich kitting labels
end if

parent.Triggerevent('ue_print_part_label')
ib_select=false
//End - Dinesh - 11092021- DE23654- GEISTLICH - WO lables all lables are getting printed. Not the selected ones only
...
event clicked;ib_select=true //Begin - Dinesh - 11092021- DE23654- GEISTLICH - WO lables all lables are getting printed. Not the selected ones only
Parent.Event ue_selectall()
...
event clicked;//string ls_print
IF cbx_show_comp.Checked Then
	dw_label.SetFilter('')	
	dw_label.Filter()
Else
//	dw_label.SetFilter('ls_print='Y'')
	//dw_label.SetFilter('SKU = SKU_Parent')
	dw_label.SetFilter("c_print_ind='Y'")
	dw_label.Filter()
End If
...
integer height = 1612
string title = "Geistlich Part Labels"
string dataobject = "d_geistlich_part_labels_nolock"
