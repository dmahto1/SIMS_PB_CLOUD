$PBExportHeader$u_dw_import_skuserialhold.sru
...
/*dts - 2/25/14 - not inserting the records as they are in the File. Only as they are in inventory (8-char SKU, 12-char Serial)	  
	  ...
	  Else
		...
		//also do an update to serial_number_inventory setting hold_status = ‘Y’ based on Project, SKU and Serial_No (no need to check if it exists. It will just update nothing if it doesn’t)
		/*Update Serial_Number_Inventory
		Set Hold_status ='Y'
		Where Project_Id =:lsProject
		And SKU=:lsSku
