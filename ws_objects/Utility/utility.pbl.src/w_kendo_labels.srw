$PBExportHeader$w_kendo_labels.srw
...
public function string uf_get_store (string ascustcode)
public function long uf_get_casecnt (string assku)
public function string uf_replace (string asstring, string asrownbr, string assku, string asqty, string ascsku)
end prototypes
...
boolean	lbNewCarton, lbMultipleSKU, lbLTcaseCnt		//S49678 GailM 9/16/2020 Qty is less than case pack (CS - qty_2)
datastore ldsThisCarton
long llPrintCount

String lsMessage		//GailM 841
ll_failures = 0
Long llCaseCnt			//S49678
...
//	End If
	...
	If lbMultipleSKU Then
		...
		Lstrparms.String_arg[43] = "PARTIAL"	/* PARTIAL ON ULTA BEAUTY LABEL*/
		Lstrparms.String_arg[42] = "MIXED"		/* Customer SKU */
	Else
		//Lstrparms.String_arg[20] = dw_label.GetItemString(llRowPos,'sku') /* Sku*/
		//dts Lstrparms.String_arg[20] = dw_label.GetItemString(llRowPos,'customer_sku') /* Sku*/ //Dinesh - 12/03/2020- DE18797- Kendo label sku swapping with ulta sku
		//dts 12/11/2020 - DE18797 Cont'd.  Need to swap the Sku/Customer_Sku, only for ULTA labels
		If lsCustLabel = 'ULTA' or lsCustLabel =  'TARGET' Then    // 	Dhirendra-S59217- Kendo: Target New Label----Start      Added or Condition for Target    
			Lstrparms.String_arg[20] = dw_label.GetItemString(llRowPos,'customer_sku') /* Sku*/ //Dinesh - 12/03/2020- DE18797- Kendo label sku swapping with ulta sku
		else
			Lstrparms.String_arg[20] = dw_label.GetItemString(llRowPos,'sku') /* Sku*/
		end if
		...
		//Lstrparms.String_arg[42] = dw_label.GetItemString(llRowPos,'customer_sku') 	//GailM 11/2/2020 - S51062 F26207 I3222 Kendo - Ulta SKU label
		//dts Lstrparms.String_arg[42] = dw_label.GetItemString(llRowPos,'sku') //Dinesh - 12/03/2020- DE18797- Kendo label sku swapping with ulta sku
		//dts 12/11/2020 - DE18797 Cont'd.  Need to swap the Sku/Customer_Sku, only for ULTA labels
		If lsCustLabel = 'ULTA'  or lsCustLabel = 'TARGET'Then     // 	Dhirendra-S59217- Kendo: Target New Label----Start      Added or Condition for Target
			Lstrparms.String_arg[42] = dw_label.GetItemString(llRowPos,'sku') //Dinesh - 12/03/2020- DE18797- Kendo label sku swapping with ulta sku
		else
			Lstrparms.String_arg[42] = dw_label.GetItemString(llRowPos,'customer_sku') 	//GailM 11/2/2020 - S51062 F26207 I3222 Kendo - Ulta SKU label
		end if
		//Lstrparms.String_arg[43] = " "		/*NOT PARTIAL ON ULTA BEAUTY LABEL*/
	End if
	...
	If lsCustLabel = 'ULTA' Then
		Lstrparms.String_arg[26] = uf_get_store(lsCustCode)
		llCaseCnt = uf_get_casecnt(dw_label.GetItemString(llRowPos,'sku'))
		If llCaseCnt > 0 and dw_label.GetItemNumber(llRowPos,'Quantity') < llCaseCnt Then
			Lstrparms.String_arg[43] = "PARTIAL"
		End If
	// 	Dhirendra-S59217- Kendo: Target New Label----Start
	elseIf lsCustLabel = 'TARGET' Then
		Lstrparms.String_arg[26] = uf_get_store(lsCustCode)
		llCaseCnt = uf_get_casecnt(dw_label.GetItemString(llRowPos,'sku'))
		If llCaseCnt > 0 and dw_label.GetItemNumber(llRowPos,'Quantity') < llCaseCnt Then
			Lstrparms.String_arg[43] = "PARTIAL"
...
//	End If
	...
	//GailM 10/28/2020 DE18292 KDO Kendo - Stop original label printing for Ulta
	If lsCustLabel = 'ULTA'  or lsCustLabel = 'TARGET'    Then   // 	Dhirendra-S59217- Kendo: Target New Label----Start      Added or Condition for Target
	// DO Nothing 
	else 
      invo_labels.uf_kendo_ship(lsAny)  //  lsCustLabel <> 'TARGET'//Moved from ELSE to not print generic label if printing a customer one  GailM DE18223 10/20/2020
	End If
...
Long llQty
String lsSKU, lsPrevSKU, lsSort, lsCustSku
Str_parms	lStrparms
...
For liRow = 1 to liRows
	...
	lsCustSku = adsCarton.GetItemString(liRow, "customer_sku")	//GailM 11/2/2020 - S51062 F26207 I3222 Kendo - Ulta SKU label
	If IsNull(lsCustSku) Then lsCustSku = ""
	
	If lsSKU <> lsPrevSKU Then
		liNbrSkus ++
		liCtrlRow = liRow
		//lStrparms.String_arg[liRow] = lsSKU
		lStrparms.String_arg[liRow] = lsCustSku // Dinesh - 01/04/2021 - DE19100 - Kendo Ulta SKUs printing incorrectly
		lStrparms.Integer_arg[liRow] = llQty
		//lStrparms.String_arg_2[liRow] = lsCustSku
		lStrparms.String_arg_2[liRow] = lsSKU // Dinesh - 01/04/2021 - DE19100 - Kendo Ulta SKUs printing incorrectly
		lsPrevSKU = lsSKU
	Else
...
String lsLabelRow, lsLabelSku, lsLabelQty, lsLabelText, lsCust, lsCustSku

lstrSkuParms = astrSkuParms
lstrPrintParms = astrPrintParms

lsCust = lstrPrintParms.String_arg[38]		//GailM 11/2/2020 - S F26207 I3222 Kendo - Ulta SKU label - Add Customer SKU
llRowsPerLabel = 25
liThisRow = 0

If lsCust = 'ULTA' Then
	lsLabelHeader = "^XA^CI0^LL203^MD0^PMN^JZY^PR2^FO75,76^A0N,28,34^FDSKU Number             Quantity    Kendo SKU Number^FS"
	lsLabelBody = "^FO76,xrowx^A0N,25,30^FDxskux^FS^FO395,xrowx^A0N,25,30^FDxqtyx^FS^FO535,xrowx^A0N,25,30^FDxcskux^FS"
Else
...
For liRow = 1 to liNbrSKUs
	...
	lsCustSku = lstrSkuParms.String_arg_2[liRow]
	
	lsLabelText = uf_replace( lsLabelBody, lsLabelRow, lsLabelSku, lsLabelQty, lsCustSku )
...
public function string uf_get_store (string ascustcode);string lsReturn

	SELECT C.user_field3
	INTO :lsReturn 
	FROM customer c with ( NOLOCK )
	WHERE c.project_id = :gs_project and c.cust_code = :asCustCode
	USING SQLCA;

If IsNull(lsReturn) or lsReturn = "" Then
	lsReturn = '0000'
End If
...
public function long uf_get_casecnt (string assku);//GailM 9/11/2020 - S49678 F24970 I3021 KDO - Kendo: New Client-Specific Carton Label (Ulta Beauty)
Long llCaseCnt = 0
Long llQty_2, llQty_3, llQty_4


SELECT IsNull(qty_2,0), IsNull(qty_3,0), IsNull(qty_4,0) INTO :llQty_2, :llQty_3, :llQty_4
FROM item_master with (nolock)
WHERE project_id = :gs_project AND sku = :asSKU
AND (uom_2 = 'CS' OR uom_3 = 'CS' OR uom_4 = 'CS')
USING SQLCA;

llCaseCnt = llQty_2 + llQty_3 + llQty_4


return llCaseCnt
end function

public function string uf_replace (string asstring, string asrownbr, string assku, string asqty, string ascsku);String lsString, lsRow, lsSku, lsQty, lsCSku
Long llPos
...
llPos = Pos(lsString, lsCSku)
lsString = Replace( lsString, llPos, len(lsCSku), asCSku)
...
If isValid(W_DO) Then
	lsWhere = " and Delivery_Master.do_no = '" + w_do.idw_main.GetITemString(1,'do_no') + "' " // Dinesh -02/02/2021
