$PBExportHeader$u_nvo_proc_loreal.sru
forward
global type u_nvo_proc_loreal from nonvisualobject
end type
end forward

global type u_nvo_proc_loreal from nonvisualobject
end type
global u_nvo_proc_loreal u_nvo_proc_loreal
...
public function integer uf_otm_fields_modified ()
public function integer uf_process_itemmaster (string aspath, string asproject)
public function integer uf_process_boh (string asproject, string asinifile)
end prototypes
...
Choose Case Upper(Left(asFile,2))
	...
	Case 'IM'
		...
		//Starbucks_TH item master processing
...
public function integer uf_otm_fields_modified ();return 0
end function
...
for llFileRowPos = 1 to llFilerowCount
	...
	If NOT(IsNull(lsTemp) OR trim(lsTemp) = '') Then
		ldsItem.SetItem(1,'Part_UPC_Code', Long(lsTemp))
	End If			
	...
	If NOT(IsNull(lsTemp) OR trim(lsTemp) = '') Then
		ldsItem.SetItem(1,'age', Long(lsTemp))
	End If		
	...
	If NOT(IsNull(lsTemp) OR trim(lsTemp) = '') Then
		ldsItem.SetItem(1,'interpack_qty', Long(lsTemp))
	End If		
	...
	If NOT(IsNull(lsTemp) OR trim(lsTemp) = '') Then
		ldsItem.SetItem(1,'stackable_height', Long(lsTemp))
	End If		
...
on u_nvo_proc_loreal.create
call super::create
TriggerEvent( this, "constructor" )
end on

on u_nvo_proc_loreal.destroy
