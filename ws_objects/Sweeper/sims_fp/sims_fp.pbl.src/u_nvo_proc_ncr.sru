$PBExportHeader$u_nvo_proc_ncr.sru
...
public function integer uf_process_delivery_order ()
...
The OLDEST_RECEIPT_DATE is the first date the item was received in the HUB and is still on hand.  
Once the entire quantity of the First Batch has been shipped, then the Oldest Receipt Date becomes 
the Date Received of the Second Batch of that same item.  
This is using the FIFO (First In First Out) inventory concept.  
...
//P. Tibbits - change supplier_owned_flag to “Y” – everything in our(NCR) possession will be supplier owned
...
llHeaderCount = idsDOHeader.Retrieve()
...
For llRowPos = 1 to llRowCount
	...
	Choose Case lsRecType
		...
		Case 'DM' /* Header */
			...
			//	A DELJIT can be updated- the RFF ZZ codes “K”= Kanban (original), “A” = add, “C”= change.  
