$PBExportHeader$u_nvo_proc_comcast.sru
...
For llSerialPos = 1 to llSerialCount
		...
		lsOutString += ldsCartonSerial.GetItemString(llSerialPos, "Supp_invoice_no") +  '|'  // Reference Number – SIMS Order Number
		...
		If Not isnull(ldsCartonSerial.GetItemString(llSerialPos, "eis_model")) Then
			lsOutString += ldsCartonSerial.GetItemString(llSerialPos, "eis_model") + '|'  // ‘Model’ – Item_Master.User_field10
		Else
			lsOutString +=  '|'
		End If
