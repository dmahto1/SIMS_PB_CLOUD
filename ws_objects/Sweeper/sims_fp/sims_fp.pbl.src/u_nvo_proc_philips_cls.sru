$PBExportHeader$u_nvo_proc_philips_cls.sru
...
public function integer uf_process_dboh (string asproject)
end prototypes
...
//Open and read the File In
//dts 10/26/20 - S50899 - lsLogOut = '      - Opening File for PHILIPSCLS Purchase Order Processing: ' + asPath
lsLogOut = '      - Opening File for ' + asproject + ' Purchase Order Processing: ' + asPath
FileWrite(giLogFileNo,lsLogOut)
gu_nvo_process_files.uf_write_log(lsLogOut) /*write to Screen*/

liFileNo = FileOpen(asPath,LineMode!,Read!,LockReadWrite!)
If liFileNo < 0 Then
	//dts 10/26/20 - S50899 - lsLogOut = "-       ***Unable to Open File for PHILIPSCLS Processing: " + asPath
	lsLogOut = "-       ***Unable to Open File for " + asproject + " Processing: " + asPath
	FileWrite(giLogFileNo,lsLogOut)
	gu_nvo_process_files.uf_write_log(lsLogOut) /*write to Screen*/
	Return -99
End If
...
For llRowPos = 1 to llRowCount
	...
	Choose Case Upper(lsRecType)
		...
		Case 'PM' /*PO Header*/
			...
			//Project Identifier is always PHILIPSCLS (dts 10/26/20 - S50899: now could also be PHILIPS-DA)
			...
			//dts 10/26/20 - S50899 - not sure why this is implemented this way (only seeing PHILIPS and PHILIPS-MY in DM* files)
			// - need to modify to allow PHILIPS-DA to use this.
			//IF Pos(lsTemp, 'PHILIPS') > 0 Then
			IF Pos(lsTemp, 'PHILIPS') > 0 or asproject='PHILIPS-DA' Then // Dinesh - 06/30/2021- Philips-DA change
			...
			End IF
//			// Begin - Dinesh - 06/30/2021- Philips-DA change
//			IF asproject='PHILIPS-DA' Then
//				ldsPOHeader.setItem(llNewRow,  'Wh_Code', trim(lsTemp))
//			elseIf Pos(lsTemp, 'SG') > 0 Then
//				ldsPOHeader.setItem(llNewRow,  'Wh_Code', 'DA-SG')
//			elseIf Pos(lsTemp, 'MY') > 0 Then
//				ldsPOHeader.setItem(llNewRow,  'Wh_Code', 'DA-MY')
//			End IF
//			// End - Dinesh - 06/30/2021- Philips-DA change
			//Purchase Order No
			lsRecData = Right(lsRecData,(len(lsRecData) - (Len(lsTemp) + 1))) /*strip off to next Column */
			If Pos(lsRecData, '|') > 0 Then
				lsTemp =Left(lsRecData, (Pos(lsRecData, '|') -1))
			...
			If lsAction = 'U' or lb_treat_adds_as_updates Then  //IF the Action is "U"pdate We need to issue a delete and then an Add for the entire order
				...
				IF Len(ls_errors) > 0 THEN
					//dts lsLogOut = "        *** Unable to create datastore for PHILIPSCLS Inbound Process.~r~r" + ls_errors
					lsLogOut = "        *** Unable to create datastore for " + asproject + " Inbound Process.~r~r" + ls_errors
					FileWrite(gilogFileNo,lsLogOut)
					RETURN - 1
				END IF
...
//Open and read the File In
//dts S50897 - lsLogOut = '      - Opening File for PHILIPSCLS Delivery Order Processing: ' + asPath
lsLogOut = '      - Opening File for ' + asproject + ' Delivery Order Processing: ' + asPath
FileWrite(giLogFileNo,lsLogOut)
gu_nvo_process_files.uf_write_log(lsLogOut) /*write to Screen*/

liFileNo = FileOpen(asPath,LineMode!,Read!,LockReadWrite!)
If liFileNo < 0 Then
//dts - S50897 -	lsLogOut = "-       ***Unable to Open File for PHILIPSCLS Processing: " + asPath
	lsLogOut = "-       ***Unable to Open File for " + asproject + " Processing: " + asPath
	FileWrite(giLogFileNo,lsLogOut)
	gu_nvo_process_files.uf_write_log(lsLogOut) /*write to Screen*/
	Return -99
...
FOR llRowPos = 1 to llRowCount 
	...
	CHOOSE CASE Upper(lsRecType)
		...
		CASE 'DM' /* Header */
			...
			//dts 10/26/20 - S50897 - not sure why this is implemented this way (only seeing PHILIPS and PHILIPS-MY in DM* files)
			// - need to modify to allow PHILIPS-DA to use this.
			IF Pos(lsTemp, 'PHILIPS') > 0 or asproject='PHILIPS-DA' Then
			//IF Pos(lsTemp, 'PHILIPS') > 0 then // Dinesh -S50897 -  06/30/2021- Phiplips-DA
			...
			End IF
//			// Begin - S50897 - Dinesh - 06/30/2021- Phiplips-DA
//			IF asproject='PHILIPS-DA' Then
//				ldsDOHeader.setItem(llNewRow,  'WH_Code', trim(lsTemp))
//			elseIf Pos(lsTemp, 'SG') > 0 Then
//				ldsDOHeader.setItem(llNewRow,  'WH_Code', 'DA-SG')
//			elseIf Pos(lsTemp, 'MY') > 0 Then
//				ldsDOHeader.setItem(llNewRow,  'WH_Code', 'DA-MY')
//			End IF
			// End - S50897 - Dinesh - 06/30/2021- Phiplips-DA
			//Invoice No (DM005)
			lsRecData = Right(lsRecData,(len(lsRecData) - (Len(lsTemp) + 1))) /*strip off to next Column */
			If Pos(lsRecData,'|') > 0 Then
				lsTemp = Left(lsRecData,(pos(lsRecData,'|') - 1))
...
FOR llRow = llDetailCount to 1 Step -1
	...
	IF lbSoftBundle and upper(lsUF5) ='PIC' and lsPrevUF6 = lsUF6 THEN
		...
		if ldParentQty=0 then ldParentQty = 1 //dts - 11/29/2020 - Outbound Order files are coming with 0 quantities causing 'divide by 0' error.
...
//Open and read the File In
//dts S50901  lsLogOut = '      - Opening File for PHILIPSCLS Return Orders Processing: ' + asPath
lsLogOut = '      - Opening File for ' + asproject + ' Return Orders Processing: ' + asPath
FileWrite(giLogFileNo,lsLogOut)
gu_nvo_process_files.uf_write_log(lsLogOut) /*write to Screen*/

liFileNo = FileOpen(asPath,LineMode!,Read!,LockReadWrite!)
If liFileNo < 0 Then
	//dts S50901 replaced hard-coded PHILIPSCLS with asproject
	lsLogOut = "-       ***Unable to Open File for " + asproject + " CLS Processing: " + asPath
	FileWrite(giLogFileNo,lsLogOut)
	gu_nvo_process_files.uf_write_log(lsLogOut) /*write to Screen*/
	Return -99
End If
...
For llRowPos = 1 to llRowCount
	...
	Choose Case Upper(lsRecType)
		...
		Case 'RM' /*PO Header*/
			...
			//Project Identifier is always PHILIPSCLS (dts 10/29/20 - S50901: can now also be PHILIPS-DA) 
			lsRecData = Right(lsRecData,(len(lsRecData) - (Len(lsTemp) + 1))) /*strip off to next Column */
			If Pos(lsRecData,'|') > 0 Then
				lsTemp = Left(lsRecData,(pos(lsRecData,'|') - 1))
			Else 
			...
			//dts - 10/26/20 - S50901 - not sure why this is implemented this way (only seeing PHILIPS and PHILIPS-MY in DM* files)
			// - need to modify to allow PHILIPS-DA to use this.
			//IF Pos(lsTemp, 'PHILIPS') > 0 Then
			IF Pos(lsTemp, 'PHILIPS') > 0 or asproject='PHILIPS-DA' Then // Dinesh - 06/30/2021- Philips-DA
...
//				ldsROHeader.setItem(llNewRow,  'Wh_Code', trim(lsTemp))
//			elseIf Pos(lsTemp, 'SG') > 0 Then
//				ldsROHeader.setItem(llNewRow,  'Wh_Code', 'DA-SG')
//			elseIf Pos(lsTemp, 'MY') > 0 Then
//				ldsROHeader.setItem(llNewRow,  'Wh_Code', 'DA-MY')
//			End IF
			// End - Dinesh - 06/30/2021- Philips-DA change
			...
			If lsAction = 'U' or lb_treat_adds_as_updates Then  //IF the Action is "U"pdate We need to issue a delete and then an Add for the entire order
				...
				ldsRoNo.Create(SQLCA.SyntaxFromSQL(sql_syntax,"", ls_errors))
				IF Len(ls_errors) > 0 THEN
					//dts S50901 lsLogOut = "        *** Unable to create datastore for PHILIPSCLS Inbound Process.~r~r" + ls_errors
					lsLogOut = "        *** Unable to create datastore for " + asproject + " Inbound Process.~r~r" + ls_errors
					FileWrite(gilogFileNo,lsLogOut)
					RETURN - 1
				END IF
...
//Open and read the File In
//dts - 11/19/2020 - S51582 - for PHILIPS-DA effort, use asproject instead of 'PHILIPSCLS'
//lsLogOut = '      - Opening File for PHILIPSCLS Shipment Delivered Status Processing: ' + asPath
lsLogOut = '      - Opening File for ' + asproject + ' Shipment Delivered Status Processing: ' + asPath
FileWrite(giLogFileNo,lsLogOut)
gu_nvo_process_files.uf_write_log(lsLogOut) /*write to Screen*/

liFileNo = FileOpen(asPath,LineMode!,Read!,LockReadWrite!)
If liFileNo < 0 Then
	//dts - S51582 lsLogOut = "-       ***Unable to Open File for PHILIPSCLS Processing: " + asPath
	lsLogOut = "-       ***Unable to Open File for " + asproject + " Processing: " + asPath
	FileWrite(giLogFileNo,lsLogOut)
	gu_nvo_process_files.uf_write_log(lsLogOut) /*write to Screen*/
	Return -99
...
//dts - 10/29/2020 - S50979 - added asproject parameter as we're now using this for PHILIPS-DA as well
