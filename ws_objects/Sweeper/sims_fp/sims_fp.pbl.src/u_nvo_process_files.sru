$PBExportHeader$u_nvo_process_files.sru
...
//	gs_OTM_Flag = ProfileString(gsinifile,lsProject,"OTM_Flag","")
	...
	Choose Case Upper(lsProject) /*create proper UO for processing project specific files*/
		...
		Case 'PHILIPS-DA' //dts 10/26/2020 using Philips CLS object for Philips-DA
			lu_nvo_proc_philips_cls = Create u_nvo_proc_philips_cls
	End Choose
...
//	end if
	...
	//dts 11/03/2020 S50899 - added PHILIPS-DA
	IF UPPER(lsProject) = 'PHILIPS-SG' OR UPPER(lsProject) = 'PHILIPSCLS' OR UPPER(lsProject) = 'PHILIPS-TH' OR UPPER(lsProject) = 'PHILIPS-DA' THEN
	...
	//06-FEB-2019 :Madhu S28685 Added PHILIPSCLS
	if lsProject = "FLEX-SIN" OR (UPPER(lsProject) = 'PHILIPS-SG' AND ls_allow_dup_ro_numbers_ind = 'Y') OR (UPPER(lsProject) = 'PHILIPSCLS' AND ls_allow_dup_ro_numbers_ind = 'Y') OR (UPPER(lsProject) = 'PHILIPS-TH' AND ls_allow_dup_ro_numbers_ind = 'Y') OR (UPPER(lsProject) = 'PHILIPS-DA' AND ls_allow_dup_ro_numbers_ind = 'Y') then    /* MEA - 201009 - Do no need to check previous orders.  */
		llRmasterCount = 0
	else
...
ls_ConnectRetries =  ProfileString(gsinifile,"sims3FP","dbConnectRetries","")
ls_dbConnectWait = ProfileString(gsinifile,"sims3FP","dbConnectWait","")
...
//connect using sqlca;
 //   Added by Dhirendra : Loop until (10 times ) database connect sucessfully....Start Here
FOR i = 1 to ll_loopcount  
	IF 	ll_loopcount  > long(ls_ConnectRetries)   then
		exit 
	else
		connect using sqlca;
	end if 
	...
	END IF  
	ll_loopcount++
	Sleep (long(ls_dbConnectWait) )
next 

IF sqlca.sqlcode <> 0     THEN
	lsOutput = '** Unable to connect to Database after ' +String(ll_loopcount)+ ' attempts:' + sqlca.sqlerrtext 
	FileWrite(gilogFileNo,lsOutput)
	uf_write_Log(lsOutput) /*display msg to screen*/
	Halt close
	Return -1
elseif sqlca.sqlcode =  0  then
	lsOutput = '** Successfully connected to Database in ' +String(ll_loopcount)+ ' attempt(s).'
	FileWrite(gilogFileNo,lsOutput)
	...
	Return -1
END IF
 //   Added by Dhirendra : Loop until (10 times ) database connect sucessfully....END Here 
...
For llTranPos = 1 to llTranCount
	...
	gsFileName = '' //dts - 11/26/2020 - DE15099 - Bosch GI transactions are incorrectly being stamped by previous Transaction's file name.
	...
	lsTranType = trim(ldsTransFile.GetITemString(llTranPos,'trans_Type'))
	//lsProject = ldsTransFile.GetITemString(llTranPos,'project_ID')		// LTK 20111117	Project blocking sweeper change, now set above
	lsOrderID = trim(ldsTransFile.GetITemString(llTranPos,'trans_order_ID'))  //Jxlim 08/01/2013 ariens; trans_order_id is consolidation_no (shipment_ID) 
	lsTrans_parm = trim(ldsTransFile.GetITemString(llTranPos,'trans_parm')) 	   //Jxlim 08/01/2013 ariens; trans_parm is dono 
...
//	2.	For outbound orders, order types = “C” (Cross-dock; bakery items for stores within Bangkok) and “F” (Frozen; bakery items for stores outside Bangkok) are to be excluded from the SIMS indirects.
...
public function integer uf_send_email (string asproject, string asdistriblist, string assubject, string astext, string asattachments);String	lsDistribList,	lsTemp, 	lsOutPut, lsReturn, lsCommand, lsAttachments, lsSubject, lsmailhostServerName, lsFromAddress
...
For llHeaderPos = 1 to llHeaderCount
	...
	Else /* No errors */
	 ...
	 if lsProject <> 'PANDORA' then
		...
		//  Dhirendra-S55342-Default ESD to original RDD from 940-END 
		...
		//06-FEB-2019 :Madhu S28685 Added PHILIPSCLS
		//dts 11/03/2020 S50897 Added PHILIPS-DA (necessary?)
		if Upper(lsProject) = 'PHILIPS-SG' OR Upper(lsProject) = 'PHILIPSCLS' OR Upper(lsProject) = 'PHILIPS-DA' then
