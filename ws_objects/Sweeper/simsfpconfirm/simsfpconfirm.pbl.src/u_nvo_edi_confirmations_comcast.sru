$PBExportHeader$u_nvo_edi_confirmations_comcast.sru
...
//   Loop through the Putaway list and for each distinct Pallet_Id (Lot_No), retrieve all of the records from “carton_serial” for that Pallet_ID.

//For each carton serial record, create a record to send to ICC (layout attached) – pipe delimited 
...
//	if lsCurrentPalletID <> lsLastPalletID then
	...
	lsOutString += lsOrdNo + '|'  // Reference Number – SIMS Order Number (??)
	
	lsOutString += '' + '|'  // Container Temp ID – Blanks		
...
//	End Choose
	...
	lsOutString += lsserial + '|'  //Serial – Serial_No from Carton_Serial record 
	lsOutString += 'S'  + '|'   //Detail Type – ‘S’
	
	if IsNull(lsModel) THEN lsModel = ''
	lsOutString += lsModel + '|'  // ‘Model’ – Item_Master.User_field10
...
If llNewQty = 0 Then /*Pallet being Deleted - Send a Delete (TUP)*/
	...
	For llSerialPos = 1 to llSerialCount
		...
		lsOutString = 'TUP|'  //Tran type - ‘TUP’
		lsOutString += lsComcastwarehouse + '|' //FROM Site ID -  Warehouse.User_Field1
		lsOutString += lsComcastwarehouse + '|' //To Site ID -  Warehouse.User_Field1
		lsOutString += String(alAdjustID) + '|'  // Reference Number – Adjustment ID
		lsOutString += '' + '|'  // Container Temp ID – Blanks		
		lsOutString+= '' + '|'  //Start Date
...
//		End Choose
		...
		lsOutString += lsserial + '|'  //Serial – Serial_No from Carton_Serial record 
		lsOutString += 'S'  + '|'   //Detail Type – ‘S’
		lsOutString += lsEISModel + '|'  // ‘Model’ – Item_Master.User_field10
...
For llRowPos = 1 to llRowCount
	...
	//From Site ID 
	lsOutString += lsComcastwarehouse + "|"  // From Site ID – Ship From Warehouse User_Field1
...
//   Loop through the existing Delivery_Derial_detail records. Each “serial_no” may be a pallet_id (most likey), a “carton_id” or a serial_no (least likely) from the “carton_serial” table. Try retrieving the carton-serial records by Pallet first, then carton and then serial last.
...
//1
	...
	If lsWarehouseType =  'S' Then
		lsOutString = 'TSN|'  //Tran type - ‘TSN’
	Else
		lsOutString = 'TSU|'  //Tran type - ‘TSU’
	End If
...
//	End If
	...
	//Address1 -  CPE = Cable Card Unit Address Field 20 – M-Card Unit Address (UF3),   				DTA = MAC Address
	//Address2 - CPE = Host Embedded Serial Number Field 10 – Set-top Serial Number - (UF1?),		DTA = Unit Address
	//Address3 - CPE = Host Embedded STB MAC Field 15 – eSTB MAC - (mac_id)							DTA = Blank
	//Address4  - CPE =Host Embedded Cable Modem MAC Field 13 – eCM MAC (UF4)					DTA = Blank
	//Address5  - CPE =Host ID Field 12 – Host ID (UF2)
