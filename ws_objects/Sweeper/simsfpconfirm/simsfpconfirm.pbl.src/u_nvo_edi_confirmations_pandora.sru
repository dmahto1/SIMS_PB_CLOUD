$PBExportHeader$u_nvo_edi_confirmations_pandora.sru
...
//If the 4C1 is a Serial Change 4C1 then, If the Item Master flag <> “Y” then do not send a DAT file to ORacle.
...
string		ls_Pack_Po_No2 //dts - 03/30/2021
...
If llPackRowCount > 0 Then
	...
	For ll_Pack_Row = 1 to  idsDOPack.RowCount()
		...
		//dts 03/30/2021 - Need to grab Pallet from Pack_po_no2 if it's not footprints?
		ls_Pack_Po_No2 = idsDOpack.GetItemString( ll_Pack_Row, 'Pack_Po_No2')	
		...
		elseIf lbFootPrintSku and ls_prev_Po_No2 = ls_carton_No and ls_prev_Pack_container_Id <> ls_Pack_container_Id Then
			...
			//dts 03/30/2021 - using Pack_Po_no2 if present.
			If lbFootPrintSku = False and ls_Pack_Po_No2 <> '' and ls_Pack_Po_No2 <> '-' and ls_Pack_Po_No2 <> 'NA' and not isnull(ls_Pack_Po_No2) Then				
				lsFind = " REFCHAR3 = '"+ls_Pack_Po_No2+"'" /* rec will be built with REFCHAR3 = Pack_Po_No2*/
		...
		If llFindRow = 0 Then //If Record doesn't present, add to data store.
			...
			IF lbFootPrintSku THEN
				...
				// 03/20 - PCONKL - F21806/S43394 - Suppress Pallet record if Pallet ID = 'NA'
				If lbPallet  and ls_carton_No <> ls_Pack_container_Id and ls_carton_No <> 'NA' Then
					ll_attr_row =idsOMQWhDOAttr.insertrow(0)
				...
				If ls_Pack_container_Id <> '-' and ls_Pack_container_Id <> 'NA' then
					...
					//MEA- 2019/07/26 - DE11824 - If Carton_No = Pack_container_ID for that Pack Record, don’t do the SetItem on the REFCAHR2.
			...
			ELSE
				...
				If (ls_carton_Type = 'PALLET' or ls_carton_Type = 'OVERPACK') and ls_carton_No <> ls_Pack_container_Id THEN
					...
					//dts 06/30/2021 if ls_Pack_Po_No2<>'-' then
					//dts 06/30/2021 - DE22163 - OMA_WAREHOUSE_ORDERATTR and OMA_WH_ORDERDETAIL_ATTR records not created correctly
					If ls_Pack_Po_No2 <> '' and ls_Pack_Po_No2 <> '-' and ls_Pack_Po_No2 <> 'NA' and not isnull(ls_Pack_Po_No2) Then	 //do we need <lbFootPrintSku = False> as part of this?
						lsFind = " REFCHAR3 = '"+ls_Pack_Po_No2+"'"
					else
					...
					If llFindRow = 0 Then
						...
						//dts 06/30/2021 - DE22163 - OMA_WAREHOUSE_ORDERATTR and OMA_WH_ORDERDETAIL_ATTR records not created correctly
						If ls_Pack_Po_No2 <> '' and ls_Pack_Po_No2 <> '-' and ls_Pack_Po_No2 <> 'NA' and not isnull(ls_Pack_Po_No2) Then	 //do we need <lbFootPrintSku = False> as part of this?
							this.uf_process_gi_om_pallet_construct( ll_attr_row, aitransid, ls_client_id, asdono, lsWH, ld_pallet_qty, ls_Pack_Po_No2, ls_ship_track_Id, ll_Pack_Row)
						else
							this.uf_process_gi_om_pallet_construct( ll_attr_row, aitransid, ls_client_id, asdono, lsWH, ld_pallet_qty, ls_carton_No, ls_ship_track_Id, ll_Pack_Row)
						end if
					...
					If  ls_Pack_container_Id <> '' and ls_Pack_container_Id <> '-' and ls_Pack_container_Id <> 'NA' and not isnull(ls_Pack_container_Id) Then
						...
						llRefch1++
						idsOMQWhDOAttr.setitem(ll_attr_row, 'REFCHAR1', string(llRefch1)) //Increment count
						//dts 04/26/2021 (re dts change 0f 03/31/2021).  Now stamping REFCHAR2 for Container record (if Pack_PONO2 is present)
						//dts 06/30/2021 If ls_carton_No <> ls_Pack_container_Id and ls_Pack_Po_No2<>'-' then
						//dts 06/30/2021 - DE22163 - OMA_WAREHOUSE_ORDERATTR and OMA_WH_ORDERDETAIL_ATTR records not created correctly
						If ls_Pack_Po_No2 <> '' and ls_Pack_Po_No2 <> '-' and ls_Pack_Po_No2 <> 'NA' and not isnull(ls_Pack_Po_No2) Then	 //do we need <lbFootPrintSku = False> as part of this?
							idsOMQWhDOAttr.setitem(ll_attr_row, 'REFCHAR2', string(ll_Parent_RF1)) //store Pallet Id as Parent (Pallet -> Carton -> Item)
...
Datastore ldsroPutaway  // Dinesh - 03/22/2021- S54935- Google - SIMS – 947 change needed for Google SAP
...
//End.....Akash Baghel....09/25/2024... SIMS-546 Google SIMS - SIMS not adjusting out serials numbers when manual order is processed for SHORTAGE ISSUE //
    ...
    if ls_trans_parm='Inbound' then
		//Write to File and Screen
		lsLogOut = '      - OM Adjustment- Start Processing of uf_om_adjustment() for RONO: ' + ls_trans_order
		FileWrite(giLogFileNo,lsLogOut)
		gu_nvo_process_files.uf_write_log(lsLogOut)
	elseif ls_trans_parm='Outbound' then
		//Write to File and Screen
		lsLogOut = '      - OM Adjustment- Start Processing of uf_om_adjustment() for DONO: ' + ls_trans_order
		FileWrite(giLogFileNo,lsLogOut)
		gu_nvo_process_files.uf_write_log(lsLogOut)
	else
		lsLogOut = "        *** Unable to retrieve Record for RONO/DONO: " + ls_trans_order
		FileWrite(gilogFileNo,lsLogOut)
		Return -1
	end if
End if
...
if ls_trans_parm='Inbound' then
	If Not isvalid(ldsroMaster) Then
		ldsroMaster = Create Datastore
		ldsroMaster.Dataobject = 'd_ro_master'
		ldsroMaster.SetTransObject(SQLCA)
	End If
	
	If Not isvalid(ldsroPutaway) Then
		ldsroPutaway = Create Datastore
		//dts ldsroPutaway.Dataobject = 'd_ro_putaway'  //dts TODO - do we need to use d_ro_putaway_serial (if roll-up)?
		//dts ldsroPutaway.SetTransObject(SQLCA)
		//dts ldsOC = Create Datastore
	//dts - need to roll up putaway rows by line_item_no
		//dts - 4/21/21 - DE21081 - don't need COO so no longer using...
		//sql_syntax = "select ro_no, line_item_no, sku, rp.owner_id, owner_cd, po_no, country_of_origin, sum(quantity) quantity"
		sql_syntax = "select ro_no, line_item_no, sku, rp.owner_id, owner_cd, po_no, sum(quantity) quantity"
		sql_syntax += " from receive_putaway rp with(nolock) inner join owner o with(nolock) on rp.owner_id=o.owner_id"
		sql_syntax += " where rp.ro_no='"+ls_trans_order+"'"
		//dts - 4/21/21 - DE21081 sql_syntax += " group by ro_no, line_item_no, sku, rp.owner_id, owner_cd, po_no, country_of_origin;"
		sql_syntax += " group by ro_no, line_item_no, sku, rp.owner_id, owner_cd, po_no;"
		ldsroPutaway.Create(SQLCA.SyntaxFromSQL(sql_syntax,"", ERRORS))
		
		IF Len(ERRORS) > 0 THEN
			lsLogOut = "         OM Adj -  Unable to create Putaway datastore for Adjustment Transaction.~r~r" + Errors
			FileWrite(gilogFileNo,lsLogOut)
			RETURN -1
		END IF
		
		ldsroPutaway.SetTransObject(SQLCA)
		llRowCount = ldsroPutaway.retrieve( )
		if llRowCount<=0 then
					lsLogOut = "        *** Unable to retrieve Putaway Records for RONO: " + ls_trans_order
					FileWrite(gilogFileNo,lsLogOut)
					Return -1
		End If
	End If
end if //  if ls_trans_parm='Inbound' 

if ls_trans_parm='Outbound' then
	If Not isvalid(ldsdoMaster) Then
		ldsdoMaster = Create Datastore
		ldsdoMaster.Dataobject = 'd_do_master'
		ldsdoMaster.SetTransObject(SQLCA)
	End If
	
	 If Not isvalid(ldsdoPicking) Then
		ldsdoPicking = Create Datastore
		//dts 4/18/21 ldsdoPicking.Dataobject = 'd_do_picking'
		//dts 4/18/21 ldsdoPicking.SetTransObject(SQLCA)
		//dts 4/18/21 ldsroPutaway = Create Datastore
		//dts - need to roll up picking rows by line_item_no
		//dts - 4/21/21 - DE21081 - don't need COO so no longer using...
		//sql_syntax = "select do_no, line_item_no, sku, dp.owner_id, owner_cd, po_no, country_of_origin, sum(quantity) quantity"
		sql_syntax = "select do_no, line_item_no, sku, dp.owner_id, owner_cd, po_no, sum(quantity) quantity"
		sql_syntax += " from delivery_picking dp with(nolock) inner join owner o with(nolock) on dp.owner_id=o.owner_id"
		sql_syntax += " where dp.do_no='"+ls_trans_order+"'"
		//dts - 4/21/21 - DE21081 - sql_syntax += " group by do_no, line_item_no, sku, dp.owner_id, owner_cd, po_no, country_of_origin;"
		sql_syntax += " group by do_no, line_item_no, sku, dp.owner_id, owner_cd, po_no;"
		ldsdoPicking.Create(SQLCA.SyntaxFromSQL(sql_syntax,"", ERRORS))
		
		IF Len(ERRORS) > 0 THEN
			lsLogOut = "         OM Adj -  Unable to create Picking datastore for Adjustment Transaction.~r~r" + Errors
			FileWrite(gilogFileNo,lsLogOut)
			RETURN -1
		END IF
		
		ldsdoPicking.SetTransObject(SQLCA)
		llRowCount = ldsdoPicking.retrieve( )
		if llRowCount<=0 then
					lsLogOut = "        *** Unable to retrieve Picking Records for DONO: " + ls_trans_order
					FileWrite(gilogFileNo,lsLogOut)
					Return -1
		End If
	End If
end if //if ls_trans_parm='Outbound'
...
// Begin - Dinesh - 03/22/2021- S54935- Google - SIMS – 947 change needed for Google SAP
 ...
 if ls_trans_parm='Inbound' then
	 ...
	 //dts 4/18 - now being retrieved above.
//			If ldsroPutaway.Retrieve(ls_trans_order) <= 0 Then
//				lsLogOut = "        *** Unable to retreive Putaway Record for RONO: " + ls_trans_order
//				FileWrite(gilogFileNo,lsLogOut)
//				Return -1
//			End If

			If ldsroMaster.Retrieve(ls_trans_order) <= 0 Then
				lsLogOut = "        *** Unable to retrieve Receive Master Record for RONO: " + ls_trans_order
				FileWrite(gilogFileNo,lsLogOut)
				Return -1
			End If
	elseif  ls_trans_parm='Outbound' then
	//Retreive the Outbound record
	 //dts 4/18 - now being retrieved above.
//			If ldsdoPicking.Retrieve(ls_trans_order) <= 0 Then
//				lsLogOut = "        *** Unable to retrieve Picking Record for DONO: " + ls_trans_order
//				FileWrite(gilogFileNo,lsLogOut)
//				Return -1
//			End If

			If ldsdoMaster.Retrieve(ls_trans_order) <= 0 Then
				lsLogOut = "        *** Unable to retrieve Delivery Master Record for DONO: " + ls_trans_order
				FileWrite(gilogFileNo,lsLogOut)
				Return -1
			end if
		else
			...
			If ldsAdjustment.Retrieve(asProject, alAdjustID) <= 0 Then
				lsLogOut = "        *** Unable to retrieve Adjustment Record for AdjustID: " + String(alAdjustID)
				FileWrite(gilogFileNo,lsLogOut)
				Return -1
			End If
		end if

select OM_Client_Id into :ls_client_id from Project with(nolock) where Project_Id=:asproject using sqlca; //dts TODO - may need to move this up and also select receive_putaway_serial_rollup_ind into variable

 if ls_trans_parm='Inbound' then
	//dts Need to have ldsROPutaway rolled up by line_item_no (and then can call uf_om_sn_adjustment and grab all SN's for current line (in instance variable))
	//dts - Need to get next ITRNKEY inside the loop (and call uf_om_sn_adjustment in the loop)
	//dts ldOMQ_Inv_Tran = gu_nvo_process_files.uf_get_next_seq_no(asProject, 'OMQ_Inv_Tran', 'ITRNKey')
	//OMQ_Inventory_Transaction for Inbound
	For llRowPos =1 to  ldsROPutaway.RowCount()
		ldOMQ_Inv_Tran = gu_nvo_process_files.uf_get_next_seq_no(asProject, 'OMQ_Inv_Tran', 'ITRNKey')
		ll_Inv_Row = idsOMQInvTran.insertrow(0)
		il_LineNo=ldsroPutaway.GetItemNumber(llRowPos,'line_item_no')

		//dts 4/18/21 - changing GetItem calls: using llRowPos for Picking and '1' for Master
		// - Should clean this up; we don't need many of the variables being set.  In fact, we're doing GetItem where we should be using the variables that are set.
		//lsroNO = ldsroPutaway.GetITemString(ll_Inv_Row,'ro_no')
		lsSku = ldsroPutaway.GetITemString(llRowPos,'sku')
		lsWhcode =ldsroMaster.getItemString(1,'wh_code')
		ls_from_loc =ldsroMaster.getItemString(1,'User_Field6')
		select code_descript into :ls_reasoncd from lookup_table where code_type='ORDER_CONFIRM_ADJ' and code_id=:ls_from_loc using sqlca;
		//dts lsAdjType= 'Q'
		llOldOwnerId = ldsroPutaway.GetITemNumber(llRowPos,'owner_id')
		lsOldOwner_Cd = ldsroPutaway.getItemString(llRowPos,'owner_cd')
		lsOldPoNo = ldsroPutaway.getitemstring(llRowPos, 'po_no')
		//lsOldCoo = ldsroPutaway.getItemString(llRowPos,'Country_of_Origin')
		llOldQty = 0
		llNewOwnerId = ldsroPutaway.GetITemNumber(llRowPos,'owner_Id')
		lsNewOwner_Cd = ldsroPutaway.getItemString(llRowPos,'owner_cd')
		lsNewPoNo = ldsroPutaway.getitemstring(llRowPos, 'po_no')
		//lsNewCoo = ldsroPutaway.getItemString(llRowPos,'Country_of_Origin')
		llNewQty = ldsroPutaway.GetITemNumber(llRowPos,'quantity')
		//llAdjustID = ldsroPutaway.GetITemNumber(ll_Inv_Row,'adjust_no')
		...
		idsOMQInvTran.setitem( ll_Inv_Row, 'TRANTYPE', 'AJ') //Tran Type as AJ (Goods Receive)
		idsOMQInvTran.setitem( ll_Inv_Row, 'EFFECTIVEDATE', ldtToday) //Effective Date
		idsOMQInvTran.setitem( ll_Inv_Row, 'ITEM', ldsroPutaway.getitemstring(llRowPos ,'sku')) //RP.Sku
		idsOMQInvTran.setitem( ll_Inv_Row, 'LOTTABLE01', ldsroPutaway.getitemstring(llRowPos ,'owner_cd')) //RP.Owner_Cd
		idsOMQInvTran.setitem( ll_Inv_Row, 'LOTTABLE03', ldsroPutaway.getitemstring(llRowPos ,'po_no')) //RP.Po_No
		idsOMQInvTran.setitem( ll_Inv_Row, 'QTY', ldsroPutaway.GetItemNumber(llRowPos,'quantity')) //ITRNKey
		idsOMQInvTran.setitem( ll_Inv_Row, 'REASONCODE', ls_reasoncd) //Reason code
		//idsOMQInvTran.setitem( ll_Inv_Row, 'RECEIPTKEY', Right(trim(ldsroMaster.GetItemString(1, 'supp_invoice_no')),10)) //RM.Supp_Invoice_No
		//idsOMQInvTran.setitem( ll_Inv_Row, 'RECEIPTLINENUMBER', Right(string(ldsroPutaway.GetItemNumber(llRowPos,'line_item_no')),5)) //RP.Line_Item_No
		idsOMQInvTran.setitem( ll_Inv_Row, 'SOURCEKEY', ldsroPutaway.getitemstring(llRowPos ,'ro_no') + string (ldsROPutaway.GetItemNumber(llRowPos,'line_item_no') ,'0000')) //RP.Ro_No
		idsOMQInvTran.setitem( ll_Inv_Row, 'SOURCETYPE', 'ADJUSTMENT')
		
		//dts 4/18 - moved from below (to be done for each put-away line
		is_trans_parm=ls_trans_parm
		this.uf_om_sn_adjustment( asproject, altransid, ls_trans_order, string(ldOMQ_Inv_Tran)) //Insert SN Records.
	Next
...
elseif ls_trans_parm='Outbound' then
	...
	For llRowPos =1 to  ldsdoPicking.RowCount()
		ldOMQ_Inv_Tran = gu_nvo_process_files.uf_get_next_seq_no(asProject, 'OMQ_Inv_Tran', 'ITRNKey')
		ll_Inv_Row = idsOMQInvTran.insertrow(0)
		il_LineNo=ldsdoPicking.GetItemNumber(llRowPos,'line_item_no')
		
		//dts 4/18/21 - changing GetItem calls: using llRowPos for Picking and '1' for Master
		// - Should clean this up; we don't need many of the variables being set.  In fact, we're doing GetItem where we should be using the variables that are set.
		//lsroNO = ldsdoPicking.GetITemString(ll_Inv_Row,'do_no')
		lsSku = ldsdoPicking.GetITemString(llRowPos,'sku')
		lsWhcode =ldsdoMaster.getItemString(1,'wh_code')
		ls_custcode =ldsdoMaster.getItemString(1,'cust_code')
		select code_descript into :ls_reasoncd from lookup_table where code_type='ORDER_CONFIRM_ADJ' and code_id=:ls_custcode using sqlca;
		//dts lsAdjType= 'Q'
		llOldOwnerId = ldsdoPicking.GetITemNumber(llRowPos,'owner_id')
		//lsOldOwner_Cd = ldsdoPicking.getItemString(ll_Inv_Row,'owner_owner_cd')
		lsOldOwner_Cd = ldsdoPicking.getItemString(llRowPos,'owner_cd')
		lsOldPoNo = ldsdoPicking.getitemstring(llRowPos, 'po_no')
		//lsOldCoo = ldsdoPicking.getItemString(llRowPos,'Country_of_Origin')
		llOldQty = ldsdoPicking.GetITemNumber(llRowPos,'quantity')
		llNewOwnerId = ldsdoPicking.GetITemNumber(llRowPos,'owner_Id')
		//lsNewOwner_Cd = ldsdoPicking.getItemString(ll_Inv_Row,'owner_owner_cd')
		lsNewOwner_Cd = ldsdoPicking.getItemString(llRowPos,'owner_cd')
		lsNewPoNo = ldsdoPicking.getitemstring(llRowPos, 'po_no')
		//lsNewCoo = ldsdoPicking.getItemString(llRowPos,'Country_of_Origin')
		llNewQty =0
		//llAdjustID = ldsAdjustment.GetITemNumber(1,'do_no')
		idsOMQInvTran.setitem( ll_Inv_Row,'CLIENT_ID',ls_client_id)
		idsOMQInvTran.setitem( ll_Inv_Row, 'QACTION', 'I') //Action
		idsOMQInvTran.setitem( ll_Inv_Row, 'QADDWHO', 'SIMSUSER') //Add Who
		idsOMQInvTran.setitem( ll_Inv_Row, 'QSTATUS', 'NEW') //Status
		...
		idsOMQInvTran.setitem( ll_Inv_Row, 'ITRNKEY', string(ldOMQ_Inv_Tran)) //ITRNKey
		idsOMQInvTran.setitem( ll_Inv_Row, 'TRANTYPE', 'AJ') //Tran Type as AJ (Good Transit)
		idsOMQInvTran.setitem( ll_Inv_Row, 'EFFECTIVEDATE', ldtToday) //Effective Date
		idsOMQInvTran.setitem( ll_Inv_Row, 'ITEM', ldsdoPicking.getitemstring(llRowPos ,'sku')) //DP.Sku
		//idsOMQInvTran.setitem( ll_Inv_Row, 'LOTTABLE01', ldsdoPicking.getitemstring(llRowPos ,'owner_owner_cd')) //RP.Owner_Cd
		idsOMQInvTran.setitem( ll_Inv_Row, 'LOTTABLE01', ldsdoPicking.getitemstring(llRowPos ,'owner_cd')) //Owner_Cd
		idsOMQInvTran.setitem( ll_Inv_Row, 'LOTTABLE03', ldsdoPicking.getitemstring(llRowPos ,'po_no')) //DP.Po_No
		//dts 4/16 idsOMQInvTran.setitem( ll_Inv_Row, 'QTY', ldsdoPicking.GetItemNumber(llRowPos,'quantity')) //ITRNKey
		idsOMQInvTran.setitem( ll_Inv_Row, 'QTY', (0 - ldsdoPicking.GetItemNumber(llRowPos,'quantity'))) //ITRNKey
		idsOMQInvTran.setitem( ll_Inv_Row, 'REASONCODE', ls_reasoncd) //Reason code
		//idsOMQInvTran.setitem( ll_Inv_Row, 'RECEIPTKEY', Right(trim(ldsdoMaster.GetItemString(1, 'invoice_no')),10)) //DM.Invoice_No
		//idsOMQInvTran.setitem( ll_Inv_Row, 'RECEIPTLINENUMBER', Right(string(ldsdoPicking.GetItemNumber(llRowPos,'line_item_no')),5)) //DP.Line_Item_No
		idsOMQInvTran.setitem( ll_Inv_Row, 'SOURCEKEY', ldsdoPicking.getitemstring(llRowPos ,'do_no') + string (ldsdoPicking.GetItemNumber(llRowPos,'line_item_no') ,'0000')) //DP.DO_NO
		idsOMQInvTran.setitem( ll_Inv_Row, 'SOURCETYPE', 'ADJUSTMENT')
		
		//dts 4/18 - moved from below (to be done for each picking line)
		is_trans_parm=ls_trans_parm
		this.uf_om_sn_adjustment( asproject, altransid, ls_trans_order, string(ldOMQ_Inv_Tran)) //Insert SN Records.
	Next
	
else //ls_trans_parm not 'Inbound' or 'Outbound'
...
If upper(ls_om_enabled) <> 'Y' Then
	 if ls_trans_parm='Inbound' then
		//Write to File and Screen
			lsLogOut = '      - OM Adjustment- Processing of uf_om_adjustment() for RONO: ' + ls_trans_order + ' Warehouse: '+lsWhcode+' is not enabled for OM'
			FileWrite(giLogFileNo,lsLogOut)
			gu_nvo_process_files.uf_write_log(lsLogOut)
			Return -1
	  elseif ls_trans_parm='Outbound' then
			//Write to File and Screen
			lsLogOut = '      - OM Adjustment- Processing of uf_om_adjustment() for DONO: ' + ls_trans_order + ' Warehouse: '+lsWhcode+' is not enabled for OM'
			FileWrite(giLogFileNo,lsLogOut)
			gu_nvo_process_files.uf_write_log(lsLogOut)
			Return -1
	  else	
...
//dts 4/16 (adding condition on ls_trans_parm
if ls_trans_parm='Inbound' or ls_trans_parm='Outbound' then
	//leave transaction variables false
ElseIF  ((llNewQty <> lloldQty)  and upper(lsAdjType) = 'Q') Then //Qty changes
	lbQtyTrans =TRUE
...
If llRC =1 Then
	...
	if om_sqlca.sqlcode = 0 then
		...
		if ls_trans_parm='Inbound' then
			...
			//Write to File and Screen
			lsLogOut = '      - OM Adjustment - Processing of uf_om_adjustment() for RONO: ' + ls_trans_order +"  is failed to write/update OM Tables: " + om_sqlca.SQLErrText
			FileWrite(giLogFileNo,lsLogOut)
			gu_nvo_process_files.uf_write_log(lsLogOut)
			Return -1
		...
		elseif ls_trans_parm='Outbound' then
			//Write to File and Screen
			lsLogOut = '      - OM Adjustment - Processing of uf_om_adjustment() for DONO: ' + ls_trans_order +"  is failed to write/update OM Tables: " + om_sqlca.SQLErrText
			FileWrite(giLogFileNo,lsLogOut)
			gu_nvo_process_files.uf_write_log(lsLogOut)
			Return -1
		else
	...
	//Execute Immediate "ROLLBACK" using om_sqlca; MikeA - DE15499
	rollback using om_sqlca;
	 // Begin - Dinesh - 03/24/2021- S54935- Google - SIMS – 947 change needed for Google SAP
	if ls_trans_parm='Inbound' then
		
		//Write to File and Screen
		lsLogOut = '      - OM Adjustment - Processing of uf_om_adjustment() for RONO: ' + ls_trans_order +"  is failed to write/update OM Tables: " + om_sqlca.SQLErrText +  "System error, record save failed!"
		FileWrite(giLogFileNo,lsLogOut)
		gu_nvo_process_files.uf_write_log(lsLogOut)
		Return -1
	elseif ls_trans_parm='Outbound' then
		//Write to File and Screen
		lsLogOut = '      - OM Adjustment - Processing of uf_om_adjustment() for DONO: ' + ls_trans_order +"  is failed to write/update OM Tables: " + om_sqlca.SQLErrText +  "System error, record save failed!"
		FileWrite(giLogFileNo,lsLogOut)
		gu_nvo_process_files.uf_write_log(lsLogOut)
		Return -1
	else
	// End - Dinesh - 03/24/2021- S54935- Google - SIMS – 947 change needed for Google SAP
		//Write to File and Screen
...
//if ls_trans_parm='Inbound' then
	//dts 4/18 - this is now done above for Inbound/Outbound
	//is_trans_parm=ls_trans_parm
	//this.uf_om_sn_adjustment( asproject, altransid, ls_trans_order, string(ldOMQ_Inv_Tran)) //Insert SN Records.
//elseif ls_trans_parm='Outbound' then
	//is_trans_parm=ls_trans_parm
	//this.uf_om_sn_adjustment( asproject, altransid, ls_trans_order, string(ldOMQ_Inv_Tran)) //Insert SN Records.
if ls_trans_parm='Inbound' or ls_trans_parm='Outbound' then
	//the InvTxn commit happens in uf_om_sn_adjustment for non-Inbound/Outbound
	...
	If llRC =1 Then
		...
		if om_sqlca.sqlcode = 0 then
			...
			//Write to File and Screen
			lsLogOut = '      -OM Adj change- Processing of uf_om_adjustment failed to update OM Tables: ' + om_sqlca.SQLErrText
			FileWrite(giLogFileNo,lsLogOut)
			gu_nvo_process_files.uf_write_log(lsLogOut)
			Return -1
		end if
		...
		rollback using om_sqlca;
		//Write to File and Screen
		lsLogOut = "      -  OM Adj change- Processing of uf_om_adjustment failed to write/update OM Tables: " + om_sqlca.SQLErrText +  "System error, record save failed!"
		FileWrite(giLogFileNo,lsLogOut)
		gu_nvo_process_files.uf_write_log(lsLogOut)
		Return -1
	End If
...
// Begin - Dinesh - 03/30/2021- S54935- Google - SIMS – 947 change needed for Google SAP
if is_trans_parm='Inbound' then
	ldsOC = Create Datastore
	//dts 4/16	ldsOC.Dataobject = 'd_inbound_om_adjustment'
	//dts 4/16	ldsOC.SetTransObject(SQLCA)
		
	//	sql_syntax = "SELECT rm.Wh_Code,rm.Complete_Date,rm.ID_NO,rm.owner_id,rm.po_no,rm.sku,'',serial_no"  
	//	sql_syntax += " from receive_master rm with(nolock)"
	//	sql_syntax += " inner join receive_putaway rp with(nolock) on rm.ro_no=rp.ro_no"
	//	sql_syntax += " where rm.ro_no='"+as_trans_order_id+"'"
	//	sql_syntax += ";"
	...
	IF Len(ERRORS) > 0 THEN
		lsLogOut = "         OM Adj-SN -  Unable to create datastore for Putaway Serial Number Transaction.~r~r" + Errors
		FileWrite(gilogFileNo,lsLogOut)
		RETURN -1
	END IF
	
	//dts 4/16 If ldsOC.Retrieve(as_trans_order_id) <= 0 Then
	ldsOC.SetTransObject(SQLCA)
	llRowCount = ldsOC.retrieve( )
	//If ldsOC.Retrieve( ) <= 0 Then
	if llRowCount=0 then
				lsLogOut = "        *** No serial Records for RONO: " + as_trans_order_id + ", line: " + string(il_LineNo)
				FileWrite(gilogFileNo,lsLogOut)
				Return 0
	Elseif llRowCount<0 then
				lsLogOut = "        *** Unable to retrieve Putaway (serial) Records for RONO: " + as_trans_order_id + ", line: " + string(il_LineNo)
				FileWrite(gilogFileNo,lsLogOut)
				Return -1
	End If
	...
	For llRowPos = 1 to llRowCount
		ls_wh = ldsOC.GetItemString(llRowPos, 'Wh_Code') //wh code
		ls_sku= ldsOC.GetItemString(llRowPos, 'Sku') //sku
		//dts 4/16 ls_from_serialno =ldsOC.GetItemString(llRowPos, 'From_Serial_No')
		//dts 4/16 ls_to_serialno =ldsOC.GetItemString(llRowPos, 'To_Serial_No')
		ls_to_serialno =ldsOC.GetItemString(llRowPos, 'Serial_No')
			
		//dts 4/16 llid_no =ldsOC.GetItemNumber(llRowPos, 'Id_No')
		...
		//dts 4/16 IF (not IsNull(ls_from_serialno) and ls_from_serialno <> '-' ) Then
...
elseif is_trans_parm='Outbound' then
	
	ldsOC = Create Datastore
	//dts 4/16 	ldsOC.Dataobject = 'd_outbound_om_adjustment'
	//dts 4/16 	ldsOC.SetTransObject(SQLCA)
	
	ldsOC = Create Datastore
	sql_syntax = "SELECT Wh_Code,Complete_Date,owner_id,po_no,sku,dsd.serial_no"
	sql_syntax += " from delivery_master dm with(nolock)"
	sql_syntax += " inner join delivery_picking_detail dp with(nolock) on dm.do_no=dp.do_no"
	sql_syntax += " inner join Delivery_Serial_Detail dsd on dp.id_no=dsd.id_no"
	sql_syntax += " where dm.do_no='"+as_trans_order_id+"' and line_item_no=" + string(il_LineNo) +";"
	
	ldsOC.Create(SQLCA.SyntaxFromSQL(sql_syntax,"", ERRORS))
	
	IF Len(ERRORS) > 0 THEN
		lsLogOut = "         OM Adj-SN -  Unable to create datastore for Delivery Serial Number Transaction.~r~r" + Errors
		FileWrite(gilogFileNo,lsLogOut)
		RETURN -1
	END IF
	...
	//If ldsOC.Retrieve(as_trans_order_id) <= 0 Then
	if llRowCount = 0 then
				lsLogOut = "        *** No Serial Records for DONO: " + as_trans_order_id + ", line: " + string(il_LineNo)
				FileWrite(gilogFileNo,lsLogOut)
				Return 0
	elseif llRowCount <0 then
				lsLogOut = "        *** Unable to retreive Serial Records for DONO: " + as_trans_order_id + ", line: " + string(il_LineNo)
				FileWrite(gilogFileNo,lsLogOut)
				Return -1
	End If
	...
	For llRowPos = 1 to llRowCount
		...
		ls_sku= ldsOC.GetItemString(llRowPos, 'Sku') //sku
		//dts 4/16 ls_from_serialno =ldsOC.GetItemString(llRowPos, 'From_Serial_No')
		ls_from_serialno =ldsOC.GetItemString(llRowPos, 'Serial_No')
		//dts 4/16 ls_to_serialno =ldsOC.GetItemString(llRowPos, 'To_Serial_No')
		...
		//dts 4/16 IF (not IsNull(ls_from_serialno) and ls_from_serialno <> '-' ) Then
			idsOMQInvTxnSernum.setitem( ll_InvTxn_row, 'SERIALNUMBER', ls_from_serialno)
		//dts 4/16 else
		//dts 4/16 	idsOMQInvTxnSernum.setitem( ll_InvTxn_row, 'SERIALNUMBER', ls_to_serialno)
		//dts 4/16 End If
...
else //'normal' sn adjustment (not from Inbound/Outbound order)
	...
	If (len(ls_trans_parm) > 0 and ls_trans_parm <>' ' and not IsNull(ls_trans_parm)) Then
		...
		IF Len(ERRORS) > 0 THEN
			lsLogOut = "         OM Adj-SN - Unable to create datastore for Serial Number Adjustment Transaction.~r~r" + Errors
			FileWrite(gilogFileNo,lsLogOut)
			RETURN -1
		END IF
	...
	//destroy ldsOC
	destroy idsOMQInvTxnSernum  //dts - moved from below
End If

destroy ldsOC
//dts Need to not destroy idsOMQInvTxnSernum here for Inbound/Outbound (may be multiple putaway/picking lines)
//dts destroy 	idsOMQInvTxnSernum

//Write to File and Screen
lsLogOut = '      - OM Adj-SN - End Processing of uf_om_sn_adjustment() ' 
FileWrite(giLogFileNo,lsLogOut)
gu_nvo_process_files.uf_write_log(lsLogOut)
...
// 01/20 - PCONKL -  DE14057 - Need to keep Attr_ID unique which isn't always working with Pallet ID, now using al_attr_row which will be sequential/unique
//idsOMQWhDOAttr.setitem(al_attr_row, 'ATTR_ID', Right(as_carton_No,9)+'P')
idsOMQWhDOAttr.setitem(al_attr_row, 'ATTR_ID', String(al_attr_row) +'P')
...
//dts 03/25/2021 - S53782  idsOMQWhDOAttr.setitem(al_attr_row, 'REFCHAR5','LB')
idsOMQWhDOAttr.setitem(al_attr_row, 'REFCHAR5', lsUOM)
...
// 01/20 - PCONKL -  DE14057 - Need to keep Attr_ID unique which isn't always working with Carton ID, now using al_attr_row which will be sequential/unique
//idsOMQWhDOAttr.setitem(al_attr_row, 'ATTR_ID', Right(as_carton_No,9)+'C')
idsOMQWhDOAttr.setitem(al_attr_row, 'ATTR_ID', String(al_attr_row) +'C')
...
IF  ids_CC_UpCountZero_Result.rowcount( ) > 0 THEN
	...
	commit using sqlca; //MikeA - DE15499
...
//MikeA - 02/11/2020 DE14637
//Don't create Load Lock File if Load ID is 'na', null, or empty

IF Upper(asLoadID) = 'NA' OR ISNull(asLoadID) OR Trim(asLoadID) = '' THEN
	RETURN 0
END IF
...
ldsToHeader.SetItem(ll_header_row, 'User_Field3', ls_ToNo)			//GailM 6/23/2020 DE16258 Google - SOC Generating without order number
