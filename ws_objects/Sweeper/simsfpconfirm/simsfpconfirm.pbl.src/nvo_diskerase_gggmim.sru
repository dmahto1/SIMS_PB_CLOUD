$PBExportHeader$nvo_diskerase_gggmim.sru
forward
...
public function boolean f_insertupdate ();Boolean lbSQLCAauto

// If this record already existed,
...
Else
	...
	// Insert the new record.
	//Tima 01/13/12 	Added column names becasue there was an error when adding a column to the table but it was not specified.
	//TimA 05/14/14 Turn the AutoCommit to true so that we capture the insert. 
	//Sometimes these are Rollback for unknown reasons.
	 lbSQLCAauto = SQLCA.AutoCommit
	SQLCA.AutoCommit = true  

	insert into diskerase (
	Filename,
	Source,
	Boxno,
	Partno,
	...
	Creationdate,
	Importdate)
	values (:is_filename, :is_source, :is_boxnumber, :is_googlepartnumber, :is_manpartnumber, :is_driveserialnumber, :idt_boxcreationdate, :idt_importdate) using sqlca;
