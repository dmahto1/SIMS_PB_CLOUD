...
CASE "PANDORA"
	//For CASE Pandora, call a new function “uf_Item_modified_Pandora” and move the existing logic from  
	//w_Maintenance_Item_Master.wf_otm_fields_modified”. Pass the return code all the way back out of uf_check_itemmaster_modified.
...
CASE "PHILIPS-SG" ,"PHILIPSCLS","PHILIPS-DA"
	...
	//for Philips we will only send if the Supplier code from the Item Master record (idw_main passed in) is ‘SG00’, ‘SG03’, ‘SG10’ or ‘SG71’.
...
if adw_main.RowCount() > 0 then
	...
	//Philips we will only send if the Supplier code from the Item Master record (idw_main passed in) is ‘SG00’, ‘SG03’, ‘SG10’ or ‘SG71’.
