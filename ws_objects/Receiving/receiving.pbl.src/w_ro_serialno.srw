$PBExportHeader$w_ro_serialno.srw
$PBExportComments$*
forward
...
window iwCurrent
boolean ib_value, ibFromWMS	//GailM 7/6/2020 DE16627
...
String lsCustCode, lsCustType

ibFromWMS = FALSE	//GailM 7/6/2020 DE16627
...
If upper(gs_project) ='PANDORA' Then 
	ids_scan_serial = istrparms.datastore_arg[1] //16-Jan-2018 :Madhu S14839 Foot Print
	...
	IF g.ibSNchainofcustody THEN
		...
		If upper(ls_ord_type) = 'Z' or ls_Component_ind <> '*' Then
			...
			if ll_return <> 0 then  // Test return from serial validation function
				...
				If upper(ls_Ord_type) = 'Z' or ibFromWMS = True Then   // if a warehouse transfer
