	...
	ELSE
		...
		If  Date( idw_main.GetItemDateTime(1, "Arrival_Date" ) ) =  ldToday Then
			If  ltArrivalTime <  ltCutoffTime Then
				lsMsg = "The Arrival DateTime is before cut off Datetime.~r~n~r~nAre you sure you want to confirm the order? Y/N$$HEX1$$1d20$$ENDHEX$$"
				If MessageBox( is_title, lsMsg, Exclamation!, YesNo!,2 ) <> 1 Then
					tab_main.SelectTab(1 ) 
					f_setfocus(idw_main, i, "Arrival_Date")
					Return		
			...
			If  ltArrivalTime >  ltCutoffTime Then
				lsMsg = "The Arrival DateTime is after cut off Datetime.~r~n~r~nAre you sure you want to confirm the order? Y/N$$HEX1$$1d20$$ENDHEX$$"
				If MessageBox( is_title, lsMsg, Exclamation!, YesNo!,2 ) <> 1 Then
					tab_main.SelectTab(1 ) 
					f_setfocus(idw_main, i, "Arrival_Date")
					Return		
...
String	lsPalletSave, lsPallet, lsRoNo, lsInvoice, lsWH
...
If gs_project = 'COMCAST' Then
	...
	// cawikholm 07/05/11 Added call to track user 
	f_method_trace_special( gs_project,this.ClassName() + ' -ue_post_confirm','start ue_post_confirm COMCAST: ',lsRoNo,' ',' ',is_suppinvoiceno ) //08-Feb-2013 :Madhu added
	...
	For lLRowPOs = 1 to llRowCount
		...
		lsPallet = idw_Putaway.GetITemString(llROwPos,'lot_no')
		
		If lsPallet <> lsPalletSave Then
			
			Update Carton_Serial
			Set ro_no = :lsRoNo,
			      do_no = ""						//JXLIM 05/10/2010 as part of Comcast enhancement work
			Where Project_id = :gs_Project and pallet_id = :lsPallet;
	...
	f_method_trace_special( gs_project,this.ClassName() + ' -ue_post_confirm','End ue_post_confirm COMCAST: ',lsRoNo,' ',' ',is_suppinvoiceno ) //08-Feb-2013  :Madhu added
...
IF (Upper(gs_project) ='PHILIPS-SG' or Upper(gs_project) ='PHILIPSCLS' ) and idw_main.getitemstring(1,"ord_type") ='X' THEN
	For li_idx =1 to idw_putaway.rowcount( )
		idw_putaway.setitem( li_idx, "expiration_date",date("01/01/1999"))
	NEXT
END IF
...
IF upper(gs_project) ='PHILIPSCLS' THEN  wf_min_shelf_life_inbound_validation(TRUE)
...
public function integer wf_validation ();string ls_whcode, ls_code,ls_sku,ls_psku, ls_osku, ls_posku,lsInactivesku, ls_prev_sku
...
long	llskucount, llskufindrow, ll_find_row, ll_find_putaway_row
						...
						Else
							...
							elseIf llLocAvail1 = 0 and (llLocAlloc1 > 0  or llLocSit > 0 or llLocTfr_in > 0 or llLocTfr_out > 0 ) and isAllowMultiOwnerPerLocation = 'Y'  Then
								...
								Else	// Ask to allow this condition
									ll_wh_row = messagebox(is_title, 'This bin Location has allocated inventory of a different ownercode.  If you wish to use this location, press ~$$HEX1$$1c20$$ENDHEX$$OK~$$HEX1$$1d20$$ENDHEX$$. Otherwise, press ~$$HEX1$$1c20$$ENDHEX$$Cancel~$$HEX2$$1d202000$$ENDHEX$$and choose a new bin location.', Question!, OKCancel!)
									if ll_wh_row = 2 Then
...
//
//	20110316 LTK	The putaway validation is no longer needed per Dave S.
//
//	//3/10 JAyres Check to see if Customer is Active
//	If li_ret = 1 and tab_main.tabpage_putaway.dw_putaway.RowCount() > 0 then
//		FOR i_indx = 1 to tab_main.tabpage_putaway.dw_putaway.RowCount()
//			If li_ret = 0 Then Continue
//			llOwnerID 		=  tab_main.tabpage_putaway.dw_putaway.GetItemNumber(i_indx,"owner_id")
//			lsOwnername 	=  tab_main.tabpage_putaway.dw_putaway.GetItemString(i_indx,"c_owner_name")
...
//			End If
//		Next
//	// dts - moved below If li_ret = 1  Then li_ret = idw_putaway.Update()
//	End If // put-away rows...
//End If //Pandora
// End Joel validation block
...
long llFindRow, ll_return  // 02/21/2011 ujh: I-135:
...
// 20110316 LTK	Pandora "Joel validations" were performed here.  Now they are performed outside of
//						this transaction.  The return code is still set here, as before, based on the validation outcome.
if Len(ls_pandora_validation_error) > 0 then
	li_ret = 0
end if
...
//3-FEB-2019 :Madhu S28945 Added PHILIPSCLS
If gs_project = 'PHILIPS-SG' or upper(gs_project) ='PHILIPSCLS' or gs_project = 'PHILIPS-TH' or  gs_project = 'SG-MUSER' or gs_project = 'WARNER' or gs_project = 'TPV' or gs_project = 'FUNAI'   or gs_project = 'GIBSON' Then
...
//			idw_main.SetTransObject(SQLCA)
		...
		//		idw_main.object.user_field6.background.Color = idw_main.object.datawindow.color
		//	end if
		//end if
...
this.Control[]={this.dw_putaway_mobile,&
this.cb_emc,&
...
string sql_syntax, ls_cc_lock_ind, ls_old_containerId, ls_old_pono2
