HA$PBExportHeader$w_stryker_stock_expiry_report.srw
$PBExportComments$Stock Movement Report for All SKU's by Date range
forward
global type w_stryker_stock_expiry_report from w_std_report
end type
end forward
...
integer width = 5019
integer height = 3032
string title = "Stryker Stock Expiry Report"
end type
global w_stryker_stock_expiry_report w_stryker_stock_expiry_report
...
on w_stryker_stock_expiry_report.create
call super::create
end on

on w_stryker_stock_expiry_report.destroy
call super::destroy
...
lsWhere  += " and content_summary.Project_id = '" + gs_project + "'" //10-Sep-2013 :Madhu - added it for baseline
//

//SKU
ls_sku = dw_select.GetItemString(1,"sku")
if  not isnull(ls_sku) and trim(ls_sku) <> "" then
	lswhere += " and Content_Summary.SKU like '%" + dw_select.GetItemString(1,"sku") + "%'  "
	lb_where = TRUE
end if
...
if  not isnull(dw_select.GetItemString(1,"lot_no")) then
	lswhere += " and Content_Summary.lot_no =  '" + dw_select.GetItemString(1,"lot_no") + "'  "
	lb_where = TRUE
end if
...
if not isNull(ls_string) then
	lswhere += " and (Content_Summary.po_no = '" + ls_string + "')"
	lb_where = TRUE
end if
...
if not isnull(dw_select.GetItemString(1,"inventory_type")) then
	lswhere += " and Content_Summary.inventory_type = '" + dw_select.GetItemString(1,"inventory_type") + "'  "
	lb_where = TRUE
end if
...
If Date(dw_select.GetItemDateTime(1,"exp_date_from")) > date('01/01/1900 00:00') Then
		lsWhere = lsWhere + " and Content_Summary.expiration_date >= '" + string(dw_select.GetItemDateTime(1,"exp_date_from"),'mm/dd/yyyy hh:mm') + "'"	
		lb_where = True
End If

//To Order Date
If Date(dw_select.GetItemDateTime(1,"exp_date_to")) > date('01/01/1900 00:00') Then
	lsWhere = lsWhere + " and Content_Summary.expiration_date <= '" + string(dw_select.GetItemDateTime(1,"exp_date_to"),'mm/dd/yyyy hh:mm') + "'"
	lb_where = True
End If
...
type dw_select from w_std_report`dw_select within w_stryker_stock_expiry_report
...
type cb_clear from w_std_report`cb_clear within w_stryker_stock_expiry_report
...
type dw_report from w_std_report`dw_report within w_stryker_stock_expiry_report
integer x = 32
integer width = 4919
integer height = 2632
integer taborder = 30
string dataobject = "d_stryker_stock_expiry_report"
