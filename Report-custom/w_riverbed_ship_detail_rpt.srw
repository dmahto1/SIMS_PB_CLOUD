HA$PBExportHeader$w_riverbed_ship_detail_rpt.srw
$PBExportComments$Delivery Report
forward
global type w_riverbed_ship_detail_rpt from w_std_report
end type
end forward
...
global w_riverbed_ship_detail_rpt w_riverbed_ship_detail_rpt
...
on w_riverbed_ship_detail_rpt.create
call super::create
end on

on w_riverbed_ship_detail_rpt.destroy
call super::destroy
...
If Date(dw_select.GetItemDateTime(1,"complete_from_date")) > date('01-01-1900') Then
		lsWhere = lsWhere + " and  DM.Complete_Date >= '" + string(dw_select.GetItemDateTime(1,"complete_from_date"),'mm-dd-yyyy hh:mm') + "'"
...
If Date(dw_select.GetItemDateTime(1,"complete_to_date")) > date('01-01-1900') Then
	lsWhere = lsWhere + " and  DM.Complete_Date <= '" + string(dw_select.GetItemDateTime(1,"complete_to_date"),'mm-dd-yyyy hh:mm') + "'"
	lb_complete_to = TRUE
	lb_where = TRUE
End If
...
If  not isnull(dw_select.GetItemString(1,"inv_type")) then
	lswhere += " and DP.Inventory_type = '" + dw_select.GetItemString(1,"Inv_type") + "'"
	lb_where = TRUE
end if
End If
...
type dw_select from w_std_report`dw_select within w_riverbed_ship_detail_rpt
integer x = 18
integer y = 0
integer width = 3648
integer height = 292
...
type cb_clear from w_std_report`cb_clear within w_riverbed_ship_detail_rpt
end type

type dw_report from w_std_report`dw_report within w_riverbed_ship_detail_rpt
integer x = 0
integer y = 300
integer width = 3625
integer height = 1592
integer taborder = 30
string dataobject = "d_riverbed_ship_details_rpt"
