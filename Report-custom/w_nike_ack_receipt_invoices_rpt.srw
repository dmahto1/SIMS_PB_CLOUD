HA$PBExportHeader$w_nike_ack_receipt_invoices_rpt.srw
forward
global type w_nike_ack_receipt_invoices_rpt from window
end type
type cbx_all from checkbox within w_nike_ack_receipt_invoices_rpt
end type
type dw_dispatch from datawindow within w_nike_ack_receipt_invoices_rpt
end type
type dw_query from datawindow within w_nike_ack_receipt_invoices_rpt
end type
end forward
...
string title = "Acknowledgement Receipt of Invoice Report"
...
global w_nike_ack_receipt_invoices_rpt w_nike_ack_receipt_invoices_rpt
...
filename = gs_syspath + "Reports\Acknowledgement_Receipt_Invoices.xls"	
...
For i = 1 to ll_cnt
	...
	lineout[1] =dw_dispatch.GetItemString(i, "invoice_no")
	lineout[2] = String(dw_dispatch.GetItemDateTime(i, "GID"), "mm/dd/yy")  
...
//	lineout[7] = String(dw_dispatch.GetItemNumber(i, "ship_cost"),"####") 	
//	lineout[8] = dw_dispatch.GetItemString(i, "Transporter") 

//	ls_do_no = dw_dispatch.GetItemString(i, "do_no") 
//	
//	integer li_idx
//	string ls_delivery_no
//	
//	lds_delivery_no_list.Retrieve(ls_do_no)
//	
//	ls_delivery_no = ""
//	
//	for li_idx = 1 to lds_delivery_no_list.RowCount()
//		if li_idx > 1 then ls_delivery_no = ls_delivery_no + ", "
//		ls_delivery_no =  ls_delivery_no + lds_delivery_no_list.GetItemString( li_idx, "delivery_no")
//	next	
//	


	lineout[4] =  dw_dispatch.GetItemString(i, "Delivery_no") 
	
	xs.range("a" + String(pos) + ":d" +  String(pos)).Value = lineout
	
	If lb_write_flag then 
		xs.cells(pos,5).value= dw_dispatch.GetItemNumber(i, "ctn_cnt")
   End if  
...
//	xs.cells(pos,13).value=String(dw_dispatch.GetItemNumber(i, "req_qty"))
//	xs.cells(pos,14).value=String(dw_dispatch.GetItemNumber(i, "qty"))
...
on w_nike_ack_receipt_invoices_rpt.create
...
on w_nike_ack_receipt_invoices_rpt.destroy
...
integer height = 364
integer taborder = 10
string dataobject = "d_nike_ack_invoice_rpt"
