HA$PBExportHeader$w_nycsp_kit_detail_sims.srw
$PBExportComments$AMS Multi User CMR Report
forward
global type w_nycsp_kit_detail_sims from w_std_report
end type
end forward
...
integer width = 3653
integer height = 2356
string title = "NYCSP Kit Detail In SIMS"
end type
global w_nycsp_kit_detail_sims w_nycsp_kit_detail_sims
...
on w_nycsp_kit_detail_sims.create
call super::create
end on

on w_nycsp_kit_detail_sims.destroy
call super::destroy
...
For k = 1 to ll_skus
	if k < lds_kit_skus.rowcount()	 then
		lsNewSQL += "Select '" + lds_kit_skus.GetItemString(k,'SKU') + "' as parent_Sku,  RTRIM(SKU) as SKU, RTRIM(Supp_Code) as Supp_Code, RTRIM(WH_Code) as WH_Code,  Avail_Qty, Component_Qty, Component_No,  RTRIM(Container_ID) as  Container_ID, Expiration_Date,  RTRIM(L_Code) as L_Code,  RTRIM(Lot_No) as Lot_No from content where project_id = 'NYCSP' and component_no in (select component_no from content where project_id = 'NYCSP' and SKU = '" + lds_kit_skus.GetItemString(k,'SKU') + "') UNION ALL "
Else 
		lsNewSQL += "Select '" + lds_kit_skus.GetItemString(k,'SKU') + "' as parent_Sku,  RTRIM(SKU) as SKU, RTRIM(Supp_Code) as Supp_Code, RTRIM(WH_Code) as WH_Code,  Avail_Qty, Component_Qty, Component_No,  RTRIM(Container_ID) as  Container_ID, Expiration_Date,  RTRIM(L_Code) as L_Code,  RTRIM(Lot_No) as Lot_No  from content where project_id = 'NYCSP' and component_no in (select component_no from content where project_id = 'NYCSP' and SKU = '" + lds_kit_skus.GetItemString(k,'SKU') + "') Order By WH_Code, parent_Sku, Component_No, Avail_Qty desc"
	End If		
Next
...
type dw_select from w_std_report`dw_select within w_nycsp_kit_detail_sims
...
type cb_clear from w_std_report`cb_clear within w_nycsp_kit_detail_sims
integer taborder = 20
end type

type dw_report from w_std_report`dw_report within w_nycsp_kit_detail_sims
integer x = 32
integer y = 12
integer width = 3483
integer height = 2056
integer taborder = 30
string dataobject = "d_nycsp_kit_details_sims"
