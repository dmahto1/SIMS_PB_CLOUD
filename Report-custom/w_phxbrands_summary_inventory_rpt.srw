HA$PBExportHeader$w_phxbrands_summary_inventory_rpt.srw
$PBExportComments$Window used for displaying summary inventory information
forward
global type w_phxbrands_summary_inventory_rpt from w_std_report
end type
end forward
...
integer width = 3639
integer height = 2072
string title = "Phoenix Brands Summary Inventory Report"
end type
global w_phxbrands_summary_inventory_rpt w_phxbrands_summary_inventory_rpt
...
on w_phxbrands_summary_inventory_rpt.create
call super::create
end on

on w_phxbrands_summary_inventory_rpt.destroy
call super::destroy
...
event ue_retrieve;/* dts 05/16/06  
	...
	For PHXBRANDS Summary Inv. Rpt, user may select Canada, US or All (in warehouse drop-down)
...
/* (from w_summary_inventory_rpt - changed for PHXBRANDS custom report)
...
//dts 05/13/08 - now using this report for SIKA, so doing this conditionally...
if gs_project = 'PHXBRANDS' then
	//dts 05/16/06 - for PHX report, add Canada, US and All.  
	//For Canada and US choices, we'll add " and wh.country like 'C%' or 'U%' " to where clause
	//07-Mar-2016 Madhu- Replaced "warehouse_wh_name" by "wh_name"
	insertrow = ldwc_warehouse.insertrow(0)
	ldwc_warehouse.setitem( insertrow , 'wh_name', '- All (Roll Up) -' )
	ldwc_warehouse.setitem( insertrow , 'wh_code', 'All Rollup' )

	insertrow = ldwc_warehouse.insertrow(0)
	ldwc_warehouse.setitem( insertrow , 'wh_name', '- All -' )
	ldwc_warehouse.setitem( insertrow , 'wh_code', 'AllWH' )	
		
	insertrow = ldwc_warehouse.insertrow(0)
	ldwc_warehouse.setitem( insertrow , 'wh_name', '- Canada -' )
	ldwc_warehouse.setitem( insertrow , 'wh_code', 'C' )
	
	insertrow = ldwc_warehouse.insertrow(0)
	ldwc_warehouse.setitem( insertrow ,'wh_name', '- US -' )
	...
	dw_report.modify("t_Title.text = 'SIKA Summary Inventory Report'")
	dw_select.SetItem(1, "warehouse" , 'SIKA-CRANB')
end if
end event

type dw_select from w_std_report`dw_select within w_phxbrands_summary_inventory_rpt
integer x = 23
integer y = 24
integer width = 3534
integer height = 188
...
type cb_clear from w_std_report`cb_clear within w_phxbrands_summary_inventory_rpt
integer x = 3104
integer y = 12
end type

type dw_report from w_std_report`dw_report within w_phxbrands_summary_inventory_rpt
integer x = 23
integer y = 212
integer width = 3543
integer height = 1560
string dataobject = "d_phxbrands_summary_inventory_rpt"
