HA$PBExportHeader$w_diebold_shipment_report.srw
$PBExportComments$Diebold Shipment Report
forward
global type w_diebold_shipment_report from w_std_query_window
end type
end forward

global type w_diebold_shipment_report from w_std_query_window
string title = "Diebold Inventory Report"
end type
global w_diebold_shipment_report w_diebold_shipment_report
...
public subroutine setcriteriadw ();long insertrow, llFindRow
...
on w_diebold_shipment_report.create
call super::create
end on

on w_diebold_shipment_report.destroy
call super::destroy
...
//	see w_gm_last_on_hand_rpt for an example of this.
	...
	lstrparms.string_arg[1] = 'd_diebold_shipment_report_search'
	lstrparms.string_arg[2] = 'd_diebold_shipment_report'
...
// open the status multi select object
	...
	//THis is an inventory report - not selecting based on Order Status
	StatusSelect.Hide()
...
type dw_select from w_std_query_window`dw_select within w_diebold_shipment_report
end type

type cb_clear from w_std_query_window`cb_clear within w_diebold_shipment_report
end type

type dw_report from w_std_query_window`dw_report within w_diebold_shipment_report
end type
