HA$PBExportHeader$w_nyxau_outbound_order_value_report.srw
$PBExportComments$Putaway Report
forward
global type w_nyxau_outbound_order_value_report from w_std_report
end type
end forward
...
global w_nyxau_outbound_order_value_report w_nyxau_outbound_order_value_report
...
on w_nyxau_outbound_order_value_report.create
call super::create
end on

on w_nyxau_outbound_order_value_report.destroy
call super::destroy
...
boolean lb_selection
	...
	If Date(dw_select.GetItemDateTime(1,"receive_from_date")) > date('01-01-1900') Then
			lsWhere = lsWhere + " and  Delivery_Master.complete_date >= '" + string(dw_select.GetItemDateTime(1,"receive_from_date"),'mm-dd-yyyy hh:mm') + "'"
	...
	If Date(dw_select.GetItemDateTime(1,"receive_to_date")) > date('01-01-1900') Then
			...
			lsWhere = lsWhere + " and  Delivery_Master.complete_date <= '" + string(dw_select.GetItemDateTime(1,"receive_to_date"),'mm-dd-yyyy hh:mm') + "'"
	...
	If ll_cnt > 0 Then
		...
		for llCurRow = 1 to dw_report.RowCount()
			dw_report.SetItem(llCurRow,'currency','$')
...
this.Title="NYX-AU Outbound Order Value Report"
end event

type dw_select from w_std_report`dw_select within w_nyxau_outbound_order_value_report
integer x = 14
integer y = 0
integer width = 3840
integer height = 264
string dataobject = "d_nyxau_outbound_order_value_report_select"
...
type cb_clear from w_std_report`cb_clear within w_nyxau_outbound_order_value_report
boolean visible = true
integer x = 3881
integer y = 56
end type

type dw_report from w_std_report`dw_report within w_nyxau_outbound_order_value_report
integer x = 0
integer y = 260
integer width = 4416
integer height = 1560
