HA$PBExportHeader$w_marcgt_shipment_reconcil_rpt.srw
$PBExportComments$Marc GT Shipment Reconciliation Report
forward
global type w_marcgt_shipment_reconcil_rpt from w_std_report
end type
end forward
...
global w_marcgt_shipment_reconcil_rpt w_marcgt_shipment_reconcil_rpt
...
on w_marcgt_shipment_reconcil_rpt.create
call super::create
end on

on w_marcgt_shipment_reconcil_rpt.destroy
call super::destroy
...
if  not isnull(dw_select.GetItemDateTime(1,"complete_date_from")) then
	ldt_s = dw_select.GetItemDateTime(1, "complete_date_from")
else
...
if  not isnull(dw_select.GetItemDateTime(1,"complete_date_to")) then
	ldt_e = dw_select.GetItemDateTime(1, "complete_date_to")
else
...
type dw_select from w_std_report`dw_select within w_marcgt_shipment_reconcil_rpt
integer x = 18
integer width = 3131
string dataobject = "d_marcgt_shipment_reconcil_rpt_search"
boolean border = false
borderstyle borderstyle = stylebox!
end type

type cb_clear from w_std_report`cb_clear within w_marcgt_shipment_reconcil_rpt
integer x = 3154
integer y = 12
integer width = 270
end type
...
type dw_report from w_std_report`dw_report within w_marcgt_shipment_reconcil_rpt
integer y = 188
integer width = 3401
integer height = 1704
string dataobject = "d_marcgt_shipment_reconcil_rpt"
