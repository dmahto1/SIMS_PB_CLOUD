HA$PBExportHeader$w_logitech_invoice_rpt.srw
$PBExportComments$Window used for Genrad invoice information
forward
global type w_logitech_invoice_rpt from w_std_report
end type
end forward
...
integer width = 3653
integer height = 2356
string title = "Linksys Commercial Invoice"
end type
global w_logitech_invoice_rpt w_logitech_invoice_rpt
...
on w_logitech_invoice_rpt.create
call super::create
end on

on w_logitech_invoice_rpt.destroy
call super::destroy
...
//
////01/01 PCONKL - dropdowns will be retrieved for the current customer (not all customers for project) when order number entered
//
////Default vat percentage to 0
//dw_Select.SetItem(1,'vat_percentage','0')
//
//dw_select.GetChild('genrad_name', ldwc)
//ldwc.SetTransObject(sqlca)
//ldwc.Retrieve(gs_project,'Genrad') /*show addresses for Genrad*/
//
//If ldwc.RowCount() > 0 Then
//	llFindRow = ldwc.Find("upper(address_code) = 'PRIMARY'",1,ldwc.RowCount())
//	If llFindRow > 0 Then
//		dw_select.SetItem(1,'genrad_name','Primary')
//	Else
//		dw_select.SetItem(1,'genrad_name',ldwc.GetItemString(1,'address_code'))
//	End If
//End If
//
////Default to Primary Address for Genrad
...
//
//
//dw_select.GetChild('bill_to_name', ldwc)
//ldwc.SetTransObject(sqlca)
//
//dw_select.GetChild('remit_to_name', ldwc)
//ldwc.SetTransObject(sqlca)
//ldwc.Retrieve(gs_project,'Genrad') /*show addresses for Genrad*/
//
//If ldwc.RowCount() > 0 Then
//	llFindRow = ldwc.Find("upper(address_code) = 'REMIT'",1,ldwc.RowCount())
//	If llFindRow > 0 Then
//		
//		dw_select.SetItem(1,'remit_to_name','Remit')
//		
...
//		Where Cust_code = 'Genrad' and project_id = 'Genrad' and address_code = 'remit';
//		
//	Else
//		
//		dw_select.SetItem(1,'remit_to_name',ldwc.GetItemString(1,'address_code'))
//		
//		Select Cust_name, address_1, address_2, address_3, address_4, city, state, zip, country
//		Into	:isRemit_Name, :isRemit_addr1, :isRemit_addr2, :isRemit_addr3, :isRemit_addr4, :isRemit_City, :isRemit_State, :isRemit_Zip, :isRemit_Country
//		From Customer
...
event ue_retrieve;DatawindowChild  	ldwc_genrad_name
DataWindowChild	ldwc_bill_to_name
DataWindowChild	ldwc_remit_to_name

string ls_order
string ls_do_no
string ls_genrad_cust_code
string ls_bill_to_cust_code
string ls_remit_to_cust_code
string ls_vat_percentage
decimal ld_vat_percentage
...
Integer	li_number_of_boxes
Integer	li_total_weight
...
If isdono > '' Then
	ll_cnt = dw_report.Retrieve(gs_project,isdono)
End If
...
//	Messagebox ("order", ls_order)
//	//Make sure can still find number if leading zeros are missing
//	ll_number = long(ls_order)
//	ll_number_of_digits = len(string(ll_number))
//	if ll_number_of_digits < 6 THEN
//		ls_order = fill('0',7 - ll_number_of_digits) + string(ll_number)
//	END IF
//		
//	SELECT do_no into :ls_do_no 
//	FROM delivery_master
//	WHERE delivery_master.invoice_no = :ls_order using SQLCA;
//	
//Else /*null*/
//	
//	MessageBox(is_title, "Please enter in an order number",stopsign!)
//	return
//		
//END IF

////Process vat percentage 
//IF NOT isnull(dw_select.GetItemstring(1,"vat_percentage")) THEN
//	ls_vat_percentage = dw_select.GetItemString(1,"vat_percentage")
//	IF IsNumber(ls_vat_percentage) = FALSE THEN
//		MessageBox(is_title, "Vat Percentage must be numeric",stopsign!)
//		RETURN
//	ELSE
//		ld_vat_percentage = dec(ls_vat_percentage)
//	END IF
//Else /*Null*/
//	MessageBox(is_title, "Please enter in a Vat Percentage",stopsign!)
//	return
//END IF	
//

// 01/01 PCONKL - Remit info is passed to the nested report from paramaters passed from here to main report!
...
type dw_select from w_std_report`dw_select within w_logitech_invoice_rpt
...
type cb_clear from w_std_report`cb_clear within w_logitech_invoice_rpt
integer taborder = 20
end type

type dw_report from w_std_report`dw_report within w_logitech_invoice_rpt
integer x = 32
integer y = 20
integer width = 3483
integer height = 2056
integer taborder = 30
string dataobject = "d_logitech_invoice_rpt"
boolean hscrollbar = true
end type

event dw_report::retrieverow;
//messagebox("row", row)
end event

event dw_report::rowfocuschanging;//messagebox("currentrow",currentrow)
//messagebox("newrow",newrow)
end event
