HA$PBExportHeader$w_logitech_iqc_inspection_rpt.srw
$PBExportComments$IAE Shipping Transaction Report
forward
global type w_logitech_iqc_inspection_rpt from w_std_report
end type
end forward
...
integer width = 3474
integer height = 2312
string title = "IQC Inspection Report"
end type
global w_logitech_iqc_inspection_rpt w_logitech_iqc_inspection_rpt
...
on w_logitech_iqc_inspection_rpt.create
call super::create
end on

on w_logitech_iqc_inspection_rpt.destroy
call super::destroy
...
If Not isnull(dw_select.GetItemString(1,'sku')) Then
	ls_Where += " and Receive_Detail.sku = '" + dw_select.GetItemString(1,'sku') + "'"
End If

// If present, tackon Warehouse
If Not isnull(dw_select.GetItemString(1,'wh_code')) Then
	ls_Where += " and Receive_Master.wh_code = '" + dw_select.GetItemString(1,'wh_code') + "'"
End If
...
If date(dw_select.GetItemDateTime(1, "complete_date_from")) > date('01-01-1900') Then
	ls_Where = ls_Where + " and receive_iqc.IQC_Date >= '" + string(dw_select.GetItemDateTime(1, "complete_date_from"),'mm-dd-yyyy hh:mm') + "'"
End If

//To
If date(dw_select.GetItemDateTime(1, "complete_date_To")) > date('01-01-1900') Then
	ls_Where = ls_Where + " and receive_iqc.IQC_Date <= '" + string(dw_select.GetItemDateTime(1, "complete_date_To"),'mm-dd-yyyy hh:mm') + "'"
End If
...
type dw_select from w_std_report`dw_select within w_logitech_iqc_inspection_rpt
integer x = 5
integer y = 16
integer width = 2985
integer height = 168
string dataobject = "d_logitech_iqc_inspection_rpt_select"
...
type cb_clear from w_std_report`cb_clear within w_logitech_iqc_inspection_rpt
integer width = 293
integer height = 96
end type

type dw_report from w_std_report`dw_report within w_logitech_iqc_inspection_rpt
integer x = 14
integer y = 196
integer width = 3392
integer height = 1900
string dataobject = "d_logitech_iqc_inspection_rpt"
