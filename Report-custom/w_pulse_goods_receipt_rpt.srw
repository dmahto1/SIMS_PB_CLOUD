HA$PBExportHeader$w_pulse_goods_receipt_rpt.srw
$PBExportComments$OutBound Order Report (GAP - 8/02)
forward
global type w_pulse_goods_receipt_rpt from w_std_report
end type
end forward
...
integer width = 3502
integer height = 2100
string title = "Pulse Goods Receipt"
end type
global w_pulse_goods_receipt_rpt w_pulse_goods_receipt_rpt
...
on w_pulse_goods_receipt_rpt.create
call super::create
end on

on w_pulse_goods_receipt_rpt.destroy
call super::destroy
...
//ELSE
 	ls_Where =" and receive_master.wh_code = 'PULSE-LIA'"
	ls_Where = ls_Where +  " and (receive_master.ord_status = 'C' OR receive_master.ord_status = 'D')"
	// 7/02 GAP - Always tackon  Project id and warehouse
	ls_Where = ls_Where + " and receive_master.project_id = '" + gs_project + "'"
	
	// 7/02 GAP - tackon  Order Type
	If dw_select.GetItemString(1, "ord_type") <> "" Then
		ls_Where = ls_Where + " and receive_master.Ord_Type = '" + Left(dw_select.GetItemString(1, "ord_type"),1) + "'"
		lb_where = True
	end if
	//For Skus
	If dw_select.object.sku[1] <> "" Then
		ls_Where = ls_Where + " and receive_putaway.sku Like '%" + dw_select.object.sku[1] + "%'"
		lb_where = True
	end if
	//Supplier code
	If dw_select.object.supplier[1] <> "" Then
		ls_Where = ls_Where + " and receive_putaway.supp_code Like '%" + dw_select.object.supplier[1] + "%'"
		lb_where = True
	end if
	//For Loc 
	If dw_select.object.plant[1] <> "" Then
			ls_Where = ls_Where + " and receive_master.user_field1 Like '%" + dw_select.object.plant[1] + "%'"
			lb_where = True
		end if
...
type dw_select from w_std_report`dw_select within w_pulse_goods_receipt_rpt
integer x = 18
integer y = 16
integer width = 3534
integer height = 244
...
type cb_clear from w_std_report`cb_clear within w_pulse_goods_receipt_rpt
integer taborder = 20
end type

type dw_report from w_std_report`dw_report within w_pulse_goods_receipt_rpt
integer x = 5
integer y = 276
integer width = 3429
integer height = 1532
integer taborder = 30
string dataobject = "d_pulse_receiving_rpt"
