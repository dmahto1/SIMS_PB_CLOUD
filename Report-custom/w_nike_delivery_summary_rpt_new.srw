HA$PBExportHeader$w_nike_delivery_summary_rpt_new.srw
forward
global type w_nike_delivery_summary_rpt_new from window
end type
type cbx_all from checkbox within w_nike_delivery_summary_rpt_new
end type
type dw_dispatch from datawindow within w_nike_delivery_summary_rpt_new
end type
type dw_query from datawindow within w_nike_delivery_summary_rpt_new
end type
end forward
...
string title = "Dispatch Summary with Category Report"
...
global w_nike_delivery_summary_rpt_new w_nike_delivery_summary_rpt_new
...
event ue_print();String ls_whcode,ls_customer,ls_invoice_no,ls_prev_invoice_no
...
String dummy,ls_region, ls_ordstatus
...
filename = gs_syspath + "Reports\DispRptnew.xls"	
...
ls_invoice_no = dw_dispatch.GetItemString(1, "invoice_no")
        
ls_prev_invoice_no = ls_invoice_no
...
For i = 1 to ll_cnt
	...
	ls_invoice_no = dw_dispatch.GetItemString(i, "invoice_no")
	
	if i=1 then lb_write_flag = True
	If i<>1 Then
		if ls_invoice_no = ls_prev_invoice_no Then
			lb_write_flag = FALSE
		elseif ls_invoice_no<> ls_prev_invoice_no then
			lb_write_flag = TRUE
			ls_prev_invoice_no = ls_invoice_no
		END IF
	END IF	
  
	lineout[1] = ls_invoice_no
	...
	//lineout[7] = String(dw_dispatch.GetItemDateTime(i, "eta_date"), "mm/dd/yy hh:mm") sarun
	lineout[7] = dw_dispatch.GetItemString(i, "ship_cost") // ,'######')
	lineout[8] = dw_dispatch.GetItemString(i, "Transporter") 
	lineout[9] = dw_dispatch.GetItemString(i, "Delivery_no")   //ls_invoice_no//
	lineout[10] = dw_dispatch.GetItemString(i, "salesorder") 
   ...
   End if  
	...
	xs.cells(pos,15).value=String(dw_dispatch.GetitemNumber(i,"division_10_qty"))
	xs.cells(pos,16).value=String(dw_dispatch.GetitemNumber(i,"division_20_qty"))
	xs.cells(pos,17).value=String(dw_dispatch.GetitemNumber(i,"division_30_qty"))
	xs.cells(pos,18).value=String(dw_dispatch.GetitemString(i,"category_type"))
	xs.cells(pos,19).value=String(dw_dispatch.GetitemString(i,"remark"))
	...
	xs.cells(pos,20).value=ls_ordstatus	
...
on w_nike_delivery_summary_rpt_new.create
...
on w_nike_delivery_summary_rpt_new.destroy
...
integer height = 364
integer taborder = 10
string dataobject = "d_nike_dispatch_delivery_ppl_new"
