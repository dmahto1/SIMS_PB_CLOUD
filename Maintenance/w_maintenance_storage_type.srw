HA$PBExportHeader$w_maintenance_storage_type.srw
$PBExportComments$Storage type maintenance
forward
global type w_maintenance_storage_type from w_std_simple_list
end type
end forward
...
integer width = 2661
integer height = 1660
string title = "StorageType List"
end type
global w_maintenance_storage_type w_maintenance_storage_type

type variables
w_maintenance_storage_type iw_window
end variables

on w_maintenance_storage_type.create
call super::create
end on

on w_maintenance_storage_type.destroy
call super::destroy
...
FOR ll_row = ll_rowcnt to 1 Step -1
	ls_code = dw_list.GetItemString(ll_row, "storage_type_cd")
...
For ll_row = ll_rowcnt To 1 Step -1
	...
	IF ls_code = ls_prev_code THEN
		...
		f_setfocus(dw_list, ll_row,"storage_type_cd")
		Return -1
	ELSE
...
dw_list.SetColumn("storage_type_cd")
end event
...
type dw_list from w_std_simple_list`dw_list within w_maintenance_storage_type
integer width = 2533
string dataobject = "d_maintenance_storage_type"
end type

event dw_list::process_enter;// If last row & Column then insert new row
IF This.GetColumnName() = "storage_type_desc" THEN
