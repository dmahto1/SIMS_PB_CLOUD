HA$PBExportHeader$w_maintenance_stryker_warehouse.srw
$PBExportComments$BCR - UOM Maintenance window
forward
global type w_maintenance_stryker_warehouse from w_std_simple_list
end type
end forward
...
integer width = 3643
integer height = 1648
string title = "Stryker Warehouse"
end type
global w_maintenance_stryker_warehouse w_maintenance_stryker_warehouse
...
on w_maintenance_stryker_warehouse.create
call super::create
end on

on w_maintenance_stryker_warehouse.destroy
call super::destroy
...
FOR ll_row = ll_rowcnt to 1 Step -1
	ls_code = dw_list.GetItemString(ll_row, "user_field1")
	IF IsNull(ls_code) THEN		
		
		dw_list.ScrollToRow(ll_row)
		dw_list.SetColumn("user_field1")
		MessageBox ("Error", "Row: " + string(ll_row) + " - Inventory Type required.")
		Return -1
...
type dw_list from w_std_simple_list`dw_list within w_maintenance_stryker_warehouse
integer x = 37
integer y = 32
integer width = 3534
string dataobject = "d_maintenance_stryker_warehouse"
