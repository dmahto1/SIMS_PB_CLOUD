HA$PBExportHeader$w_maintenance_location_type.srw
$PBExportComments$- Location type maintenance
forward
global type w_maintenance_location_type from w_std_simple_list
end type
end forward

global type w_maintenance_location_type from w_std_simple_list
integer width = 2482
string title = "Location Type List"
end type
global w_maintenance_location_type w_maintenance_location_type

type variables
w_maintenance_location_type iw_window
end variables

on w_maintenance_location_type.create
call super::create
end on

on w_maintenance_location_type.destroy
call super::destroy
...
FOR ll_row = ll_rowcnt to 1 Step -1
	ls_code = dw_list.GetItemString(ll_row, "l_type")
...
For ll_row = ll_rowcnt To 1 Step -1
	...
	IF ls_code = ls_prev_code THEN
		...
		f_setfocus(dw_list, ll_row,"l_type")
		Return -1
	ELSE
...
event ue_new;call super::ue_new;dw_list.SetColumn("l_type")
end event
...
type dw_list from w_std_simple_list`dw_list within w_maintenance_location_type
integer width = 2345
string dataobject = "d_maintenance_location_type"
end type

event dw_list::process_enter;// If last row & Column then insert new row
IF This.GetColumnName() = "l_type_desc" THEN
