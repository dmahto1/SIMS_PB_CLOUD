HA$PBExportHeader$w_maintenance_inventory_type.srw
$PBExportComments$-
forward
global type w_maintenance_inventory_type from w_std_simple_list
end type
end forward
...
integer width = 2793
integer height = 1648
string title = "Inventory Type List"
end type
global w_maintenance_inventory_type w_maintenance_inventory_type

type variables
w_maintenance_inventory_type iw_window

end variables

on w_maintenance_inventory_type.create
call super::create
end on

on w_maintenance_inventory_type.destroy
call super::destroy
...
FOR ll_row = ll_rowcnt to 1 Step -1
	ls_code = dw_list.GetItemString(ll_row, "inv_type")
...
event ue_new;call super::ue_new;dw_list.SetColumn("inv_type")
...
type dw_list from w_std_simple_list`dw_list within w_maintenance_inventory_type
integer x = 37
integer y = 32
integer width = 2688
string dataobject = "d_maintenance_inventory_type"
end type

event dw_list::process_enter;// If last row & Column then insert new row
IF This.GetColumnName() = "pick_sort_order" THEN
...
Choose Case upper(dwo.name)
	...
	//set pick sort order to 1 if setting shippable Ind to 'Y'
	Case 'INVENTORY_SHIPPABLE_IND'
		
		if data = 'Y' Then
			This.setItem(row,'pick_sort_order',1)
		Else
			This.setItem(row,'pick_sort_order',0)
		End If
