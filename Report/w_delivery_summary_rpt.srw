HA$PBExportHeader$w_delivery_summary_rpt.srw
$PBExportComments$delivery summary
forward
global type w_delivery_summary_rpt from Window
end type
type dw_query from datawindow within w_delivery_summary_rpt
end type
type dw_report_d from datawindow within w_delivery_summary_rpt
end type
end forward
...
global w_delivery_summary_rpt w_delivery_summary_rpt
...
ll_cnt = dw_report_d.Retrieve(ls_whcode, ld_sdate, ld_edate)
...
For i = 1 to ll_cnt
	...
	lineout[1] = dw_report_d.GetItemString(i, "do_no")
	lineout[2] = String(dw_report_d.GetItemDateTime(i, "ord_date"), "mm/dd/yyyy hh:mm")
	lineout[3] = String(dw_report_d.GetItemDateTime(i, "complete_date"), "mm/dd/yyyy hh:mm")
	ls_itype = dw_report_d.getitemstring(i,"inventory_type")
	select inv_type_desc into :ls_type from inventory_type
		where inv_type = :ls_itype;
	lineout[4] = ls_type
//	lineout[4] = dw_report_d.GetItemString(i,"inventory_type")
	lineout[5] = string(dw_report_d.GetItemnumber(i, "groupcbm"))
	lineout[6] = string(dw_report_d.GetItemnumber(i, "groupweight"))	
	lineout[7] = string(dw_report_d.GetItemnumber(i, "groupqty"))	
////	lineout[6] = String(dw_report_d.GetItemNumber(i, "ctn_cnt"))
////	lineout[7] = String(dw_report_d.GetItemNumber(i, "qty"))
////	lineout[8] = String(0)0
	if (lineout[1] = lineout1[1]) then continue
	pos1 += 1
	xs.range("a" + String(pos1) + ":h" +  String(pos1)).Value = lineout
	
//	xs.cells(pos,5).value=dw_report_d.GetItemNumber(i, "cbm")
//	xs.cells(pos,6).value=dw_report_d.GetItemNumber(i, "weight")
//	xs.cells(pos,7).value=dw_report_d.GetItemNumber(i, "quantity")
//	xs.cells(pos,8).value=0
Next
	xs.rows(pos1 + 1).Delete
	xs.cells(pos1 + 1,5).value = string(dw_report_d.GetItemNumber(1,"sumcbm"))
	xs.cells(pos1 + 1,6).value = string(dw_report_d.GetItemNumber(1,"sumweight"))
	xs.cells(pos1 + 1,7).value = string(dw_report_d.GetItemNumber(1,"sumqty"))	
...
on w_delivery_summary_rpt.create
...
on w_delivery_summary_rpt.destroy
