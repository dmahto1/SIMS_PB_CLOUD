HA$PBExportHeader$w_workorder_report.srw
$PBExportComments$Workorder Report
forward
global type w_workorder_report from w_std_report
end type
end forward
...
integer width = 3552
integer height = 2044
string title = "Workorder Report"
end type
global w_workorder_report w_workorder_report

type variables
String	isOrigSQL
end variables

on w_workorder_report.create
call super::create
end on

on w_workorder_report.destroy
call super::destroy
...
If Not isnull(dw_Select.GetITemString(1,'ord_Type')) Then
	lsWhere += " and workorder_master.ord_type = '" + dw_Select.GetITemString(1,'Ord_type') + "'"
End If
...
If date(dw_select.GetItemDateTime(1,"comp_date_From")) > date('01-01-1900') Then
	lsWhere = lsWhere + " and WorkOrder_Master.Complete_Date >= '" + string(dw_select.GetItemDateTime(1,"Comp_date_From"),'mm-dd-yyyy hh:mm') + "'"
End If

//Tackon To Complete Date
If date(dw_select.GetItemDateTime(1,"comp_date_to")) > date('01-01-1900') Then
	lsWhere = lsWhere + " and WorkOrder_Master.Complete_Date <= '" + string(dw_select.GetItemDateTime(1,"comp_date_to"),'mm-dd-yyyy hh:mm') + "'"
End If
...
For llRowPos = 1 to llRowCount
	...
	lswoNo = dw_report.GetITemString(llRowPos,'workorder_master_wo_no')
	lsSKu = dw_report.GetITemString(llRowPos,'workorder_detail_SKU')
	...
	dw_report.SetITem(llRowPos,'c_fg_qty',ldQty)
...
type dw_select from w_std_report`dw_select within w_workorder_report
integer x = 23
integer width = 3415
integer height = 272
...
type cb_clear from w_std_report`cb_clear within w_workorder_report
integer x = 3269
integer y = 0
end type

type dw_report from w_std_report`dw_report within w_workorder_report
integer x = 0
integer y = 304
integer width = 3474
integer height = 1512
