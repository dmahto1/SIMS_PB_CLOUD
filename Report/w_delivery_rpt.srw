HA$PBExportHeader$w_delivery_rpt.srw
$PBExportComments$Delivery Report
forward
global type w_delivery_rpt from w_std_report
end type
end forward
...
global w_delivery_rpt w_delivery_rpt
...
on w_delivery_rpt.create
call super::create
end on

on w_delivery_rpt.destroy
call super::destroy
if IsValid(MenuID) then destroy(MenuID)
end on

event open;call super::open;
if upper(gs_project) = 'GIBSON' then
	dw_report.dataobject =  'd_delivery_gibson_rpt'
	dw_report.SetTransObject(SQLCA)
end if 

//TAM 09/2016 Adding report for Pandora.
if upper(gs_project) = 'PANDORA' then
	dw_report.dataobject =  'd_pandora_delivery_rpt'
	dw_report.SetTransObject(SQLCA)
end if 
...
type dw_select from w_std_report`dw_select within w_delivery_rpt
integer x = 18
integer y = 0
integer width = 3648
integer height = 292
...
type cb_clear from w_std_report`cb_clear within w_delivery_rpt
end type

type dw_report from w_std_report`dw_report within w_delivery_rpt
integer x = 0
integer y = 300
integer width = 3625
integer height = 1592
integer taborder = 30
string dataobject = "d_delivery_rpt"
