HA$PBExportHeader$w_stock_movement_rpt_by_date.srw
$PBExportComments$Stock Movement Report for All SKU's by Date range
forward
global type w_stock_movement_rpt_by_date from w_std_report
end type
end forward
...
global w_stock_movement_rpt_by_date w_stock_movement_rpt_by_date
...
on w_stock_movement_rpt_by_date.create
call super::create
end on

on w_stock_movement_rpt_by_date.destroy
call super::destroy
...
// 08/19 - MikeA - F17372 - I2514 - KNY - NYSCP - Update Report 12 Stock Movement by Date
//Allow to select all warehouses - Need to remove warehouse in select statement so all warehouses will be retrieved. 

string ls_SQL_Select, ls_Warehouse_Table
integer li_Pos, li_Pos_Start, li_Pos_End, ll_length

if Upper(ls_whcode) =  'ALL'  then
		
	ls_SQL_Select = isOrigRptSQL
	
	do 
	
		li_Pos = Pos(ls_SQL_Select, ":as_whcode")
		
		if li_Pos > 0 then
		
			li_Pos_End =  Pos(Upper(ls_SQL_Select), "AND", li_Pos)
			
			ls_Warehouse_Table = ".WH_CODE"
			
			li_Pos_Start =Pos(Upper(ls_SQL_Select), Upper(ls_Warehouse_Table), li_Pos - 25)
			
			if li_Pos_Start = 0 OR li_Pos_Start > li_Pos_End then
				
				ls_Warehouse_Table = "b.d_warehouse"
			...
			if li_Pos_Start = 0 OR li_Pos_Start > li_Pos_End then
				...
				ls_Warehouse_Table = "b.s_warehouse"
			...
			li_Pos_Start = li_Pos_Start - 1
			
			ll_length = li_Pos_End - li_Pos_Start + 3
			
	
			ls_SQL_Select = Replace ( ls_SQL_Select, li_Pos_Start, ll_length, ' ' )
	
		end if
	
	loop until li_Pos = 0
	
	dw_report.Modify("datawindow.table.select=~"" + ls_SQL_Select + "~"")

else 
	
	dw_report.Modify("datawindow.table.select=~"" + isOrigRptSQL + "~"")
...
// 08/19 - MikeA - F17372 - I2514 - KNY - NYSCP - Update Report 12 Stock Movement by Date
//Allow to select all warehouses
//Allowed for all Projects. Modeled after Summary Inventory Report  as requested in BRD. 

integer li_row


li_row = idwc_warehouse.InsertRow(1)


idwc_warehouse.setitem( li_row , 'wh_name', '- All, by Warehouse -' )
idwc_warehouse.setitem( li_row , 'wh_code', 'All' )
...
if gs_project = 'KLONELAB' then
	...
	dw_report.dataobject = "d_stock_movement_rpt_by_date_klonelab"
	dw_report.SetTransObject(SQLCA)
	
	dw_report.GetChild('inv_type', ldw_child)
...
//TAM 09/2016 Adding Stock Movement report by date for Pandora.
If gs_project = 'PANDORA' Then
	dw_report.dataobject = 'd_stock_movement_rpt_by_date_pandora'
	dw_report.SetTransObject(SQLCA)
End If
...
type dw_select from w_std_report`dw_select within w_stock_movement_rpt_by_date
integer x = 18
integer width = 3131
...
type cb_clear from w_std_report`cb_clear within w_stock_movement_rpt_by_date
integer x = 3152
integer y = 13
integer width = 271
end type
...
type dw_report from w_std_report`dw_report within w_stock_movement_rpt_by_date
integer y = 189
integer width = 3401
integer height = 1706
string dataobject = "d_stock_movement_rpt_by_date"
