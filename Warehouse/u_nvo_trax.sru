HA$PBExportHeader$u_nvo_trax.sru
...
lsDONOHold = ""
	...
	If lsPrinter > '' Then
		lsXML = lu_websphere.uf_request_header("TraxShipShipmentRequest", "Printer='" + lsPrinter + "'")
	Else
...
If llength > 0 then	
	If Messagebox("TRAX call Successful" ,"Would you like to print Connote document?",Question!,YesNo!,1) = 1 then
		...
		//From their desktop version or citrix, the printer format looks like this :$$HEX1$$1d20$$ENDHEX$$\\copfp0472\2A3B368P006$$HEX2$$1d202000$$ENDHEX$$and works fine in the copyfile() command used to print the connote.
 		//From RDS, it looks like this: $$HEX1$$1c20$$ENDHEX$$2A3B368P006 on copfp0472 (redirected 30)$$HEX2$$1d202000$$ENDHEX$$and does not work in the copyfile() function.
 		//We need to put in logic to conditionally reformat the printer name if we are coming from RDS. I would just check for pos($$HEX1$$1c20$$ENDHEX$$redirect$$HEX1$$1d20$$ENDHEX$$) > 0 or something like that. 
		//If RDS, we need to parse out the server and  printer names from $$HEX1$$1c20$$ENDHEX$$lstempprinter$$HEX2$$1d202000$$ENDHEX$$and format it as $$HEX1$$1c20$$ENDHEX$$\\$$HEX2$$1d202000$$ENDHEX$$+ <server> + $$HEX1$$1c20$$ENDHEX$$\$$HEX2$$1d202000$$ENDHEX$$+ <printer> to match what it looks like from the desktop/citrix. I have tested by hardcoding the printer name and running it from RDS. It works fine.
...
//add the Header segment
lsXML = lu_websphere.uf_request_header("TraxVoidShipmentRequest")
...
Choose Case lsReturnCode
	...
	Case Else
		...
		If lsReturnDesc > '' Then
			Messagebox("",lsReturnDesc)
		Else
			Messagebox("", "TRAX VOID call Successful")
		End If
