HA$PBExportHeader$w_epson_uccs_labels.srw
$PBExportComments$Print Generic UCCS Shipping labels
forward
global type w_epson_uccs_labels from w_main_ancestor
end type
type cb_print from commandbutton within w_epson_uccs_labels
end type
type dw_label from u_dw_ancestor within w_epson_uccs_labels
end type
type cb_selectall from commandbutton within w_epson_uccs_labels
end type
type cb_clear from commandbutton within w_epson_uccs_labels
end type
end forward
...
global w_epson_uccs_labels w_epson_uccs_labels
...
string ls_shipper_tracking_id,  ls_awb_bol_no

ls_shipper_tracking_id =  dw_label.GetItemString(1, "ship_ref")
ls_awb_bol_no =  dw_label.GetItemString(1, "awb_bol_no")

IF IsNull(ls_shipper_tracking_id) OR Trim(ls_shipper_tracking_id) = '' OR &
	  IsNull(ls_awb_bol_no) OR Trim(ls_awb_bol_no) = '' THEN
...
boolean lb_is_pallet_order = false
boolean lb_is_package = false

//Check to see if there is a pallet contained on the order. Is so, it is a pallet order.
llRowCount = dw_label.RowCount()
For llRowPos = 1 to llRowCount /*each detail row */

	if dw_label.GetItemNumber(llRowPos,'quantity') >=  dw_label.GetItemNumber(llRowPos,'qty_3') then
		
		lb_is_pallet_order = true
...
//	Lstrparms.String_arg[32] = dw_label.GetItemString(llRowPos,'delivery_master_address_4')
	...
	Lstrparms.String_arg[45] = dw_label.GetItemString(llRowPos,'ship_ref')
	...
	string ls_carton_partial_pallet	
	...
	Lstrparms.String_arg[48] = f_epson_create_ucc( lb_is_pallet_order, lb_is_package, dw_label.GetItemString(llRowPos,'upc'), dw_label.GetItemString(llRowPos,'delivery_packing_carton_no'),ls_carton_partial_pallet)
	
	if lb_is_package then
		dw_label.SetItem( llRowPos, "delivery_packing_carton_no", ls_carton_partial_pallet)		
	end if
	
//	Lstrparms.String_arg[48] = dw_label.GetItemString(llRowPos,'upc')
	
	dw_label.SetItem( llRowPos, "user_field2", Lstrparms.String_arg[48] )
	...
	ls_sku = dw_label.object.sku[ llRowPos ] 
	ls_supp_code = dw_label.object.sku[ llRowPos ] 

integer 	ll_qty2

	ll_qty2 = dw_label.GetItemnumber(llRowPos,'qty_2')/*12/04 - PCONKL - Target CasePack */
		

	ll_alloc_qty = dw_label.GetItemnumber(llRowPos,'quantity')/*12/04 - PCONKL - Target CasePack */
		
	if ll_qty2 > 0 then

		lstrparms.string_arg[57] = string( integer( ll_alloc_qty /  ll_qty2) )
		
	end if
	
	
//	MessageBox ("ok", lstrparms.string_arg[57])
	
	
	lstrparms.string_arg[58] = dw_label.object.native_description[ llRowPos ] 
	lstrparms.string_arg[59] = dw_label.object.User_Field3[ llRowPos ] // Cust #



	lstrparms.Long_arg[7] = dw_label.GetItemnumber(llRowPos,'qty_2')
	...
	labelprinter.uf_epson_zebra_ship()
...
If isvalid(w_do) Then
	SetPointer(Hourglass!)
	 w_Do.idw_pack.Retrieve(w_do.is_dono) 
	 SetPointer(Arrow!)
End If
...
on w_epson_uccs_labels.destroy
call super::destroy
...
type cb_cancel from w_main_ancestor`cb_cancel within w_epson_uccs_labels
...
type cb_ok from w_main_ancestor`cb_ok within w_epson_uccs_labels
integer x = 1769
integer y = 24
integer height = 80
...
integer height = 1456
boolean bringtotop = true
string dataobject = "d_epson_uccs_ship"
