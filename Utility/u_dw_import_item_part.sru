HA$PBExportHeader$u_dw_import_item_part.sru
$PBExportComments$Import ItemMaster - Part information
forward
global type u_dw_import_item_part from u_dw_import
end type
end forward
...
integer width = 3077
integer height = 1572
string dataobject = "d_import_itemaster_part"
boolean border = false
borderstyle borderstyle = stylebox!
end type
global u_dw_import_item_part u_dw_import_item_part
...
luom:
//Validate uom1
If len(trim(This.getItemString(al_row,"uom_1"))) > 04 Then
	This.Setfocus()
...
// 11/01 - PCONKL - Must validate Supplier for GM if present. If null, it's OK
...
ccgroup:
// validate the cycle count group code
lsGroup = This.getItemString(al_row,"ccgroup")
If lsGroup > ' ' Then /* Only validate if present*/
	Select Count(*)  into :llCount
	from cc_group
	Where project_id = :gs_project and group_id = :lsGroup
	Using SQLCA;

	If llCount <= 0 Then
		This.Setfocus()
		This.SetColumn("ccgroup")
		iscurrvalcolumn = "ccgroup"
		Return "Cycle Count Group is not valid!"
	End If
end if

ccClass:
string ccClass
// validate the cycle count class
ccClass = This.getItemString(al_row,"ccClass")
If ccClass > ' ' Then /* Only validate if present*/
	Select Count(*)  into :llCount
	from cc_group_class_code
	Where project_id = :gs_project and group_code = :lsGroup and class_code = :ccClass
	Using SQLCA;

	If llCount <= 0 Then
		This.Setfocus()
		This.SetColumn("ccClass")
		iscurrvalcolumn = "ccClass"
		Return "Cycle Count Class is not valid!"
	End If
end if
...
For llRowPos = 1 to llRowCOunt
	...
	lsUOM = left(trim(This.GetItemString(llRowPos,"uom_1")),4)
	...
	// pvh - 06/30/06 - ccmods
	ccGroup = this.object.ccGroup[ llRowPos ]
	ccClass = this.object.ccClass[ llRowPos ]
	//
...
// so we dont clear out previously entered data if missing from layout
	...
	If lsUOM > ' ' Then lsSQL += "uom_1 = '" + lsUOM + "', "
	...
	// pvh - 06/30/06 - ccmods
	If len( ccClass ) > 0 Then lsSQL += "cc_class_code = '" + ccClass + "', "
	If len( ccGroup ) > 0 Then lsSQL += "cc_group_code = '" + ccGroup + "', "
	// 
	...
	If Sqlca.sqlnrows <> 1 Then /*Insert*/
		...
		// 10/02 - Pconkl - take owner for supplier if preent, otherwise, XX
		If lsSupplier > ' ' Then
			lsOwner = lsSupplier
		Else
			lsOWner = 'XX'
		...
		From Owner
		Where project_id = :gs_project and owner_type = 'S' and owner_cd = :lsOwner
		Using SQLCA;
		
		Insert Into item_master (project_id,sku,owner_id,country_of_origin_default,description,uom_1,length_1, width_1, height_1,supp_code,grp,last_user,last_update, interface_upd_req_ind) 
		values (:gs_project,:lsSku,:llowner,'XXX',:lsDesc,:lsUOM,:llLength1, :llWidth1, :llheight1,:lsSupplier,:lsGroup,:gs_userid,:ldToday, 'Y')
		Using SQLCA;
...
on u_dw_import_item_part.create
call super::create
end on

on u_dw_import_item_part.destroy
call super::destroy
end on
