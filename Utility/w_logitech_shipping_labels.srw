HA$PBExportHeader$w_logitech_shipping_labels.srw
$PBExportComments$Print Generic UCCS Shipping labels
forward
global type w_logitech_shipping_labels from w_main_ancestor
end type
type cb_print from commandbutton within w_logitech_shipping_labels
end type
type dw_label from u_dw_ancestor within w_logitech_shipping_labels
end type
type cb_selectall from commandbutton within w_logitech_shipping_labels
end type
type cb_clear from commandbutton within w_logitech_shipping_labels
end type
type cb_carton from commandbutton within w_logitech_shipping_labels
end type
type cb_partial from commandbutton within w_logitech_shipping_labels
end type
end forward
...
global w_logitech_shipping_labels w_logitech_shipping_labels
...
For llRowPos = 1 to llRowCount /*each detail row */
	...
	if arg_type = 'FULL' then
		...
		SELECT USER_FIELD1 INTO :ls_label
			FROM Customer
	...
	Lstrparms.String_arg[2] = dw_label.GetItemString(llRowPos,'client_name')
	...
	Lstrparms.String_arg[60] = dw_label.GetItemString(llRowPos,'user_field10') 
	Lstrparms.String_arg[61] = dw_label.GetItemString(llRowPos,'shipper_tracking_id') 
	...
	Lstrparms.String_arg[62] = dw_label.GetItemString(llRowPos,'awb_bol_no')

	Lstrparms.String_arg[80] = dw_label.GetItemString(llRowPos,'user_field9')
	...
	Lstrparms.String_arg[70] = String(Round( dw_label.GetItemnumber(llRowPos,'delivery_packing_weight_gross'), 2 ))
	...
	IF dw_label.object.delivery_packing_standard_of_measure[llRowPos] = 'M' THEN			
		...
		Lstrparms.String_arg[70] = String( round(i_nwarehouse.of_convert(dw_label.GetItemnumber(llRowPos,'delivery_packing_weight_gross'),"KG","PO"),2))
		
		Lstrparms.String_arg[71] = String(Round( dw_label.GetItemnumber(llRowPos,'delivery_packing_weight_gross'), 2 ))


	ELSE	
		lstrparms.Long_arg[4]= round(i_nwarehouse.of_convert(dw_label.GetItemnumber(llRowPos,'delivery_packing_weight_gross'),"PO","KG"),2)		

		Lstrparms.String_arg[71] = String( round(i_nwarehouse.of_convert(dw_label.GetItemnumber(llRowPos,'delivery_packing_weight_gross'),"PO","KG"),2))
	...
	*/
	...
	//ll_Alloc_Qty_Partial = dw_label.GetItemNumber(llRowPos,'alloc_qty')			
	/* 7/19/04 - dts - now using Packing quantity to determine need for Partial Label
			There may be an issue if the same sku is in the SAME Carton more than once
			 - */
...
/* 7/19/04 - dts - What's the intent here? Per Bob and Lisa, Weights on labels
                   should be the Packing Line weight (summed by Sku/Carton)
	...
	For llRowPos1 = llRowPos to llRowCount /*each detail row */
		...
		/* dts 7/19/04 - why are we doing this twice for Ingram?
		IF upper(ls_label) = "INGRAM" then
	 ...
	 Next
*/	 
	//dts - now grabbing Packing Weight to pass to ucc 128 label...
	lstrparms.Long_arg[5] = lstrparms.Long_arg[3]
	lstrparms.Long_arg[6] = lstrparms.Long_arg[4]	
	...
	ELSE
		lsAny=lstrparms		
		invo_labels.uf_logitech_zebra_ship(lsAny, ls_label)
	END IF
...
on w_logitech_shipping_labels.create
int iCurrent
call super::create
...
on w_logitech_shipping_labels.destroy
call super::destroy
...
type cb_cancel from w_main_ancestor`cb_cancel within w_logitech_shipping_labels
...
type cb_ok from w_main_ancestor`cb_ok within w_logitech_shipping_labels
integer x = 2779
integer y = 24
integer height = 80
...
integer height = 1456
boolean bringtotop = true
string dataobject = "d_logitech_uccs_ship"
