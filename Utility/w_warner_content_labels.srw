HA$PBExportHeader$w_warner_content_labels.srw
$PBExportComments$Powerwave Shipping labels
forward
global type w_warner_content_labels from w_main_ancestor
end type
type cb_print from commandbutton within w_warner_content_labels
end type
type dw_label from u_dw_ancestor within w_warner_content_labels
end type
type cb_selectall from commandbutton within w_warner_content_labels
end type
type cb_clear from commandbutton within w_warner_content_labels
end type
type cbx_ship from checkbox within w_warner_content_labels
end type
type cbx_carton_content from checkbox within w_warner_content_labels
end type
type cbx_oem from checkbox within w_warner_content_labels
end type
end forward
...
integer width = 3086
integer height = 1836
string title = "Warner Carton Labels"
string menuname = ""
event ue_print ( )
...
global w_warner_content_labels w_warner_content_labels
...
public function integer uf_content_label (integer airow, integer alprintjob)
...
i_dw_content_label = CREATE datastore;
 ...
 i_dw_content_label.dataobject = "d_warner_content_label"
...
lsPrintText = 'Warner Content Labels '
...
public function integer uf_content_label (integer airow, integer alprintjob);
String	lsWarehouse, lsAddr, lsCityState, lsShipDate, lsUCCPRefix, lsUCCCarton, lsCarrier, lsCarrierName
Long		lLWarehouseRow, llAddressPos
Integer	liCheck
long 		llPrintJob
...
Int	liCurPos, liRowDisplayNumber

i_dw_content_label.Reset()

integer li_cur_row, liRowNumber
...
For llRowPos = alBeginRow to alEndRow
	...
	li_cur_row = i_dw_content_label.InsertRow(0)	
	
	liRowDisplayNumber++
	
	
	i_dw_content_label.SetItem( li_cur_row, "row_number", liRowDisplayNumber)

	
	i_dw_content_label.SetItem( li_cur_row, "order_number", w_do.idw_main.GetITemString(1,'Invoice_no'))
	i_dw_content_label.SetItem( li_cur_row, "customer_name", w_do.idw_main.GetITemString(1,'cust_name'))
	i_dw_content_label.SetItem( li_cur_row, "city", w_do.idw_main.GetITemString(1,'city'))  
	i_dw_content_label.SetItem( li_cur_row, "carton_number", dw_label.GetITemString(llRowPos,'carton_no'))	

	i_dw_content_label.SetItem( li_cur_row, "quantity_sku", dw_label.GetITemNumber(llRowPos,'quantity'))	
	

	i_dw_content_label.SetItem( li_cur_row, "chinese_description", dw_label.GetITemString(llRowPos,'item_master_native_description'))	
	i_dw_content_label.SetItem( li_cur_row, "upc", string(dw_label.GetITemDecimal(llRowPos,'item_master_part_upc_code'))	)

	i_dw_content_label.SetItem( li_cur_row, "total_qty", string(ilCartonCount))


	if liCurPos >= 19 and  llRowPos < alEndRow then
		
		IF i_dw_content_label.RowCount() > 0 THEN
...
on w_warner_content_labels.create
int iCurrent
call super::create
...
on w_warner_content_labels.destroy
call super::destroy
...
IF not isvalid(w_do) Then
	Messagebox("Labels","You must have an open Delivery Order to print Carton Labels.")
	Return
End If
...
//	dw_label.SetItem(lLRowPos,'box_of',String(llCartonNo) + " of " + String(llCartonCount))
	...
	li_quantity = dw_label.GEtITEmNumber(llRowPOs, 'quantity')
	
	If IsNull(li_quantity) then li_quantity = 0
	
	ii_total_qty_cartons = ii_total_qty_cartons + li_quantity
	
Next

ilCartonCount = llCartonNo
...
type cb_cancel from w_main_ancestor`cb_cancel within w_warner_content_labels
boolean visible = false
integer x = 1938
integer y = 1596
integer height = 80
...
type cb_ok from w_main_ancestor`cb_ok within w_warner_content_labels
boolean visible = false
integer x = 2693
integer y = 52
integer height = 80
...
integer height = 1340
boolean bringtotop = true
string dataobject = "d_warner_label_Grid"
