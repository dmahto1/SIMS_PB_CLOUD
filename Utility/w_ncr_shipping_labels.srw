HA$PBExportHeader$w_ncr_shipping_labels.srw
$PBExportComments$Diebold Shipping labels
forward
global type w_ncr_shipping_labels from w_main_ancestor
end type
type cb_print from commandbutton within w_ncr_shipping_labels
end type
type dw_label from u_dw_ancestor within w_ncr_shipping_labels
end type
type cb_selectall from commandbutton within w_ncr_shipping_labels
end type
type cb_clear from commandbutton within w_ncr_shipping_labels
end type
end forward
...
integer width = 3200
integer height = 1392
string title = "NCR Shipping Labels"
string menuname = ""
event ue_print ( )
...
global w_ncr_shipping_labels w_ncr_shipping_labels
...
public function string getalternatesku (long therow)
end prototypes
...
lsFormatSave = lu_labels.uf_read_label_Format("ncr_shipping_Zebra.txt")
...
For llRowPos = 1 to llRowCount /*each detail row */
	...
	For j = 1 to llQty 
		...
		//		DeliveryPoint = dw_label.object.country_of_origin[llRowPos]
		//		If DeliveryPoint > '' Then
		//			lsFormat = invo_labels.uf_replace(lsFormat,"~~deliverypoint~~",DeliveryPoint)
		//		End If	
		//
		// NCR PO ( delivery_master.po_no
		//
		string NCR_PO
		NCR_PO = Left(Trim(dw_label.object.po_no[llRowPos]),50)
		If NCR_PO > '' Then
			lsFormat = invo_labels.uf_replace(lsFormat,"~~custpo_no~~",NCR_PO)
			lsFormat = invo_labels.uf_replace(lsFormat,"~~custpo_no_bc~~","1K" + NCR_PO)
		End If
		//
		// NCR Order Number ( delivery_master.invoice_no
		//
		string NCROrderNo
		NCROrderNo = Left(Trim(dw_label.object.invoice_no[llRowPos]),50)
		If NCROrderNo > '' Then
			lsFormat = invo_labels.uf_replace(lsFormat,"~~invoiceno~~",NCROrderNo)
			lsFormat = invo_labels.uf_replace(lsFormat,"~~invoiceno_bc~~","K" + NCROrderNo)
		End If
		//
		// NCR's Prod ID = delivery_detail_alternate_sku
		//
		//		string custsku
		//		custsku = getAlternateSku(llRowPos)
		//		If custsku > '' Then
		//			lsFormat = invo_labels.uf_replace(lsFormat,"~~cust_item~~",custsku)
		//			lsFormat = invo_labels.uf_replace(lsFormat,"~~cust_item_bc~~","P" + custsku)
		//		End If
		//
		// Sku
		//
		string Sku
		sku = Left(Trim(dw_label.object.sku[llRowPos]),40)
				If sku > '' Then
					lsFormat = invo_labels.uf_replace(lsFormat,"~~item~~",sku)
					lsFormat = invo_labels.uf_replace(lsFormat,"~~item_bc~~","P" + sku)
				End If
		//
		// Quantity
		//
		string qty
		qty = string(dw_label.object.quantity[llRowPos])
		If qty > '' Then
			lsFormat = invo_labels.uf_replace(lsFormat,"~~qty~~",qty)
			lsFormat = invo_labels.uf_replace(lsFormat,"~~qty_bc~~","Q" + qty)
		End If
		
		//isSerialNo = getSerialNumber(dw_label.object.sku[llRowPos])
		//Serial Number
		//		If isSerialNo > '' Then
		//			lsFormat = invo_labels.uf_replace(lsFormat,"~~serial_no~~",isSerialNo)
		//			lsFormat = invo_labels.uf_replace(lsFormat,"~~serial_no_bc~~","" + isSerialNo) /*include data identifier of "S" */
		//		End If
		//
		// COO
		//
		//		string COO
		//		COO = dw_label.object.country_of_origin[llRowPos]
		//		If COO > '' Then
		//			lsFormat = invo_labels.uf_replace(lsFormat,"~~coo~~",COO)
		//		End If		
		//
		// Weights
		//
		...
		decimal gWgt=0
		decimal kWgt=0
		grossWeight = string(dw_label.object.weight_gross[llRowPos])
		If grossWeight > '' Then
			if isNumber( grossWeight ) then
				gWgt = dec( grossWeight )
				KWgt = (gWgt * 0.4536)
			end if
			lsFormat = invo_labels.uf_replace(lsFormat,"~~lbweight~~",string(gWgt))
			lsFormat = invo_labels.uf_replace(lsFormat,"~~kgweight~~",string(KWgt))			
		End If			
		//
		// Pack Date
		//
		string PackDate
		PackDate = string(today(),"mm/dd/yyyy")
		If PackDate > '' Then
			lsFormat = invo_labels.uf_replace(lsFormat,"~~packdate~~",PackDate)
		End If
...
If upperBound(isLabels) > 0  Then
	...
	lsPrintText = 'NCR Shipping Labels '
...
public function string getalternatesku (long therow);// string = getAlternateSku()

string rValue
string do_no
string sku

do_no = dw_label.object.do_no[theRow]
sku = dw_label.object.sku[theRow]

select alternate_sku into :rValue
...
on w_ncr_shipping_labels.create
int iCurrent
call super::create
...
on w_ncr_shipping_labels.destroy
call super::destroy
...
type cb_cancel from w_main_ancestor`cb_cancel within w_ncr_shipping_labels
boolean visible = false
integer x = 1938
integer y = 1392
integer height = 80
...
type cb_ok from w_main_ancestor`cb_ok within w_ncr_shipping_labels
boolean visible = false
integer x = 1577
integer y = 28
integer height = 80
...
integer taborder = 20
boolean bringtotop = true
string dataobject = "d_ncr_shipping_label"
