HA$PBExportHeader$w_powerwave_shipping_labels.srw
$PBExportComments$Powerwave Shipping labels
forward
global type w_powerwave_shipping_labels from w_main_ancestor
end type
type cb_print from commandbutton within w_powerwave_shipping_labels
end type
type dw_label from u_dw_ancestor within w_powerwave_shipping_labels
end type
type cb_selectall from commandbutton within w_powerwave_shipping_labels
end type
type cb_clear from commandbutton within w_powerwave_shipping_labels
end type
type cbx_ship from checkbox within w_powerwave_shipping_labels
end type
type cbx_carton_content from checkbox within w_powerwave_shipping_labels
end type
type cbx_oem from checkbox within w_powerwave_shipping_labels
end type
end forward
...
integer width = 3749
integer height = 1836
string title = "Powerwave Shipping Labels"
string menuname = ""
event ue_print ( )
...
global w_powerwave_shipping_labels w_powerwave_shipping_labels
...
public function integer uf_huawei_part (string asformat, long albeginrow, long alendrow)
public function integer uf_nokia_part (string asformat, long albeginrow, long alendrow)
...
public function integer uf_nokia_part_dw (long albeginrow, long alendrow, long alprintjob)
public function integer uf_huawei_part_dw (long albeginrow, long alendrow, long alprintjob)
end prototypes
...
IF lsWarehouse =  "PWAVE-SUZ" OR  lsWarehouse = "PWAVE-SBLC" THEN
	...
	i_dw_part_label = CREATE datastore;
	 ...
	 i_dw_ship_label.dataobject = "d_powerwave_shipping_label"
	 i_dw_content_label.dataobject = "d_powerwave_carton_content_label"
	...
	lsPrintText = 'Powerwave Shipping Labels '
...
//	lsShipFormat = lu_labels.uf_read_label_Format("Powerwave_shipping_label.txt")
//	lsSKUFormat = lu_labels.uf_read_label_Format("Powerwave_carton_content.txt")
	
	
//	//Retreive the OEM Format if it exists..
//	If Upper(isOEMLabel) = 'NOKIA' Then
//		lsOEMFormat = lu_labels.uf_read_label_Format("Powerwave_Nokia_part_label.txt")
//	ElseIf Upper(isOEMLAbel) = "HUAWEI" Then
//		lsOEMFormat = lu_labels.uf_read_label_Format("Powerwave-huawei-carton.txt")
//	End If
	...
	For llRowPos = 1 to llRowCount /*each detail row */
		...
		If lsCarton <> lsCartonSave Then
			...
			If llEndRow > 0 Then /*carton content label for previous carton*/
				...
				//Huawei label is at item level within carton
				If Upper(isOEMLAbel) = "HUAWEI" Then
					uf_huawei_part_dw( llBeginRow, llEndRow,llPrintJob)
				ElseIf Upper(isOEMLabel) = 'NOKIA' Then
					uf_nokia_part_dw(llBeginRow, llEndRow,llPrintJob)
				End If
	...
	If llRowCount > 0 Then
		...
		//Huawei label is at item level within carton
		If Upper(isOEMLAbel) = "HUAWEI" Then
			uf_huawei_part_dw( llBeginRow, llRowCount, llPrintJob)
		ElseIf Upper(isOEMLabel) = 'NOKIA' Then
			uf_nokia_part_dw( llBeginRow, llRowCount,llPrintJob)
		End If
...
ELSE
	...
	For llRowPos = 1 to llRowCount /*each detail row */
		...
		If lsCarton <> lsCartonSave Then
			...
			If llEndRow > 0 Then /*carton content label for previous carton*/
				...
				//Huawei label is at item level within carton
				If Upper(isOEMLAbel) = "HUAWEI" Then
					uf_huawei_part(lsOEMFormat, llBeginRow, llEndRow)
				ElseIf Upper(isOEMLabel) = 'NOKIA' Then
					uf_nokia_part(lsOEMFormat, llBeginRow, llEndRow)
				End If
	...
	If llRowCount > 0 Then
		...
		//Huawei label is at item level within carton
		If Upper(isOEMLAbel) = "HUAWEI" Then
			uf_huawei_part(lsOEMFormat, llBeginRow, llRowCount)
		ElseIf Upper(isOEMLabel) = 'NOKIA' Then
			uf_nokia_part(lsOEMFormat, llBeginRow, llRowCount)
		End If
...
If dw_label.Find("C_print_Ind = 'Y'",1, dw_label.RowCount()) > 0 and (cbx_Ship.checked or cbx_carton_Content.checked or cbx_oem.checked) Then
...
public function integer uf_shipping_label (integer airow, string asformat);String	lsWarehouse, lsFormat, lsAddr, lsCityState, lsShipDate, lsUCCPRefix, lsUCCCarton, lsCarrier, lsCarrierName, lsStdOfMeasure
...
////////////////////////////////////////////////////////////////// BCR 10-MAY-2011: JIRA #208: Show standard of measure for SIZE
lsStdOfMeasure = dw_label.getITemString(aiRow,'standard_of_measure')
/////////////////////////////////////////////////////////////////

////////////////////////////////////////////////////////////////// BCR 20-APR-2011: JIRA #208
//Size
IF lsStdOfMeasure = 'M' THEN //Metric
	lsFormat = invo_labels.uf_replace(lsFormat,"~~Size~~",String(String(dw_label.getITemDecimal(aiRow,'length')) + 'cm x ' + String(dw_label.getITemDecimal(aiRow,'width')) + 'cm x ' + String(dw_label.getITemDecimal(aiRow,'height')) + 'cm'))
ELSEIF lsStdOfMeasure = 'E' THEN //English
	lsFormat = invo_labels.uf_replace(lsFormat,"~~Size~~",String(String(dw_label.getITemDecimal(aiRow,'length')) + 'in x ' + String(dw_label.getITemDecimal(aiRow,'width')) + 'in x ' + String(dw_label.getITemDecimal(aiRow,'height')) + 'in'))
END IF
//////////////////////////////////////////////////////////////////
...
public function integer uf_huawei_part (string asformat, long albeginrow, long alendrow);
String	lsFormat,  lsShipDate, lsDesc, lsSKU, lsSupplier, lsCode, lsCustomer
Long	llRowPos, llLabelPos, llFindRow, llpos, llBeginPos, llEndPos

If Not cbx_OEM.checked Then Return 0 /* Don't print if checkbox for OEM label not checked*/

//We need the Supplier Code from Customer UF1
lsCustomer = w_do.idw_main.GetITemString(1,'cust_code')

Select User_Field1 into :lsCode
From Customer
Where Project_id = :gs_Project and Cust_Code = :lsCustomer;
...
For llRowPos = alBeginRow to alEndRow
	...
	//SKU (Model) - Take from "Legacy SKU" (UF4) if present, otherwise from SKU
	llFindRow = w_do.idw_Detail.Find("upper(SKU) = '" + dw_label.GetITemString(llRowPos,'sku') + "'",1,w_do.idw_Detail.RowCount())
	If llFindRow > 0 Then
		If w_do.idw_detail.GetITemString(llFindRow,'User_field4') > "" Then
			lsSKU = w_do.idw_detail.GetITemString(llFindRow,'User_field4')
	...
	//Description - retrieve from Item MAster
	lsSKU = dw_label.GetITemString(llRowPos,'Sku')
	lsSupplier = dw_label.GetITemString(llRowPos,'supp_code')
	...
	If lsDesc > "" Then
		lsFormat = invo_labels.uf_replace(lsFormat,"~~description~~",lsDesc)
	End If
		
	
	//Customer SKU (ITEM) from Detail tab UF 1
	llFindRow = w_do.idw_Detail.Find("upper(SKU) = '" + dw_label.GetITemString(llRowPos,'sku') + "'",1,w_do.idw_Detail.RowCount())
	If llFindRow > 0 Then
		If w_do.idw_detail.GetITemString(llFindRow,'User_Field1') > "" Then
			lsFormat = invo_labels.uf_replace(lsFormat,"~~alternate_sku~~",w_do.idw_detail.GetITemString(llFindRow,'User_Field1'))
		Else
			lsFormat = invo_labels.uf_replace(lsFormat,"~~alternate_sku~~",w_do.idw_detail.GetITemString(llFindRow,'sku'))
		End If
	End IF
	
	//Qty
	If dw_label.GetITemNumber(llRowPos,'quantity') > 0 Then
		lsFormat = invo_labels.uf_replace(lsFormat,"~~qty~~",String(dw_label.GetITemNumber(llRowPos,'quantity')))
	End If
	
	//Customer PO (cust_order_No)
	If not isnull(w_do.idw_main.GetITemString(1,'cust_order_No')) Then
		lsFormat = invo_labels.uf_replace(lsFormat,"~~cust_order_no~~",w_do.idw_main.GetITemString(1,'cust_order_No'))
	End If
	...
	Else
		lsShipDate = String(Today(),'DD-MMM-YYYY')
	End If
	lsFormat = invo_labels.uf_replace(lsFormat,"~~date~~",lsShipDate)
	
	//Notes (CartonCount)
	If dw_label.GetITemString(llRowPos,'box_of') > "" Then
		lsFormat = invo_labels.uf_replace(lsFormat,"~~notes~~",dw_label.GetITemString(llRowPos,'box_of'))
	End If
		
	//Code (Supplier) From Customer UF1
	If lsCode > "" Then
		lsFormat = invo_labels.uf_replace(lsFormat,"~~code~~",lsCode)
	End If
	
	//Lot no entered by user on screen
	If dw_label.GetITemString(llRowPos,'c_huawei_code') > "" Then
		lsFormat = invo_labels.uf_replace(lsFormat,"~~lot_no~~",dw_label.GetITemString(llRowPos,'c_huawei_code'))
	End If
...
public function integer uf_nokia_part (string asformat, long albeginrow, long alendrow);
String	lsFormat,  lsShipDate, lsCountryName, lsSupplier, lsCustomer
Long	llRowPos, llLabelPos, llFindRow, llpos, llBeginPos, llEndPos
...
Select User_Field1 into :lsSupplier
From Customer
Where Project_id = :gs_Project and Cust_Code = :lsCustomer;
...
For llRowPos = alBeginRow to alEndRow
	...
	//LPN (Carton Nbo)
	If dw_label.GetITemString(llRowPos,'carton_no') > "" Then
		lsFormat = invo_labels.uf_replace(lsFormat,"~~carton_nbr~~",dw_label.GetITemString(llRowPos,'carton_No'))
		lsFormat = invo_labels.uf_replace(lsFormat,"~~carton_nbr_barcode~~","1J" + dw_label.GetITemString(llRowPos,'carton_No'))
	End If


	//Supplier - from Customer UF1
	If lsSupplier > "" Then
		lsFormat = invo_labels.uf_replace(lsFormat,"~~supplier~~",lsSupplier)
		lsFormat = invo_labels.uf_replace(lsFormat,"~~supplier_barcode~~","V" + lsSupplier)
	End If

	//Customer PO (cust_order_No)
	If not isnull(w_do.idw_main.GetITemString(1,'cust_order_No')) Then
		lsFormat = invo_labels.uf_replace(lsFormat,"~~Cust_order_nbr~~",w_do.idw_main.GetITemString(1,'cust_order_No'))
		lsFormat = invo_labels.uf_replace(lsFormat,"~~cust_order_nbr_barcode~~","K" + w_do.idw_main.GetITemString(1,'cust_order_No'))
	End If

	//Customer Part Nbr from Detail tab (UF1)
	llFindRow = w_do.idw_Detail.Find("upper(SKU) = '" + Upper(dw_label.GetITemString(llRowPos,'sku')) + "'",1,w_do.idw_Detail.RowCount())
	If llFindRow > 0 Then
		If not isnull(w_do.idw_detail.GetITemString(llFindRow,'User_field1')) Then
			lsFormat = invo_labels.uf_replace(lsFormat,"~~cust_sku~~",w_do.idw_detail.GetITemString(llFindRow,'User_field1'))
			lsFormat = invo_labels.uf_replace(lsFormat,"~~cust_sku_barcode~~", "P" + w_do.idw_detail.GetITemString(llFindRow,'User_field1'))
		End If
	End IF
	...
	If dw_label.GetITemNumber(llRowPos,'quantity') > 0 Then
		lsFormat = invo_labels.uf_replace(lsFormat,"~~qty~~",String(dw_label.GetITemNumber(llRowPos,'quantity')))
		lsFormat = invo_labels.uf_replace(lsFormat,"~~qty_barcode~~","Q" + String(dw_label.GetITemNumber(llRowPos,'quantity')))
	End If
	
	//Traceability No (DO_NO?????)
	If not isnull(w_do.idw_main.GetITemString(1,'do_no')) Then
		lsFormat = invo_labels.uf_replace(lsFormat,"~~trace_nbr~~",w_do.idw_main.GetITemString(1,'do_no'))
		lsFormat = invo_labels.uf_replace(lsFormat,"~~trace_nbr_barcode~~","1T" + w_do.idw_main.GetITemString(1,'do_no'))
	End If


	//Ship Date
	If not isnull(w_do.idw_main.GetITemDateTime(1,'Complete_Date')) Then
		lsShipDate = String(w_do.idw_main.GetITemDateTime(1,'Complete_Date'),'DDMMYY')
	Else
		lsShipDate = String(Today(),'DDMMYY')
	End If
	
	lsFormat = invo_labels.uf_replace(lsFormat,"~~Ship_Date~~",lsShipDate)
	lsFormat = invo_labels.uf_replace(lsFormat,"~~Ship_Date_Barcode~~","1D" + lsShipDate)
	
	//COO
	If dw_label.GetITemString(llRowPos,'country_of_Origin') > "" Then
		
		//We need the Country Name as well as the code
		lsCountryName = f_get_country_Name( dw_label.GetITemString(llRowPos,'country_of_Origin'))
		If lsCountryName > "" Then
			lsCountryName =  dw_label.GetITemString(llRowPos,'country_of_Origin') + " (" + lsCountryName + ")"
		Else
			lsCountryName =  dw_label.GetITemString(llRowPos,'country_of_Origin') 
		End If
		
		lsFormat = invo_labels.uf_replace(lsFormat,"~~COO~~",lsCountryName)
		lsFormat = invo_labels.uf_replace(lsFormat,"~~COO_Barcode~~",dw_label.GetITemString(llRowPos,'country_of_Origin'))
	...
	//CartonCount
	If dw_label.GetITemString(llRowPos,'box_of') > "" Then
		lsFormat = invo_labels.uf_replace(lsFormat,"~~Box~~",dw_label.GetITemString(llRowPos,'box_of'))
	End If
	
	//Weight
	If dw_label.GetITemNumber(llRowPos,'weight_gross') > 0 Then
		lsFormat = invo_labels.uf_replace(lsFormat,"~~Weight~~",String(dw_label.GetITemNumber(llRowPos,'weight_gross')))
	End If
...
For lLRowPos = 1 to lLRowCount
	...
	//For Huawei, Mfg Date must be filled in
	If cbx_oem.Checked and  dw_label.GetITemString(llRowPOs,'c_oem_label') = 'HUAWEI' and &
		(dw_label.GetITemString(llRowPos,'c_huawei_Code') = "" or isnull(dw_label.GetITemString(llRowPos,'c_huawei_Code'))) Then
	
			Messagebox('Labels','Mfg Date must be entered if printing Huawei OEM Labels')
			dw_label.SetFocus()
			dw_label.SetRow(llRowPOs)
			dw_label.SetColumn('c_huawei_code')
			Return -1
	End If
...
public function integer uf_shipping_label_dw (integer airow, long alprintjob);String	lsWarehouse, lsAddr, lsCityState, lsShipDate, lsUCCPRefix, lsUCCCarton, lsCarrier, lsCarrierName, lsStdOfMeasure
Long		lLWarehouseRow, llAddressPos
Integer	liCheck
long 		llPrintJob
...
//Size
IF lsStdOfMeasure = 'M' THEN //Metric
	i_dw_ship_label.SetItem(1,"Size",String(String(dw_label.getITemDecimal(aiRow,'length')) + 'cm x ' + String(dw_label.getITemDecimal(aiRow,'width')) + 'cm x ' + String(dw_label.getITemDecimal(aiRow,'height')) + 'cm'))
ELSEIF lsStdOfMeasure = 'E' THEN //English
	i_dw_ship_label.SetItem(1,"Size",String(String(dw_label.getITemDecimal(aiRow,'length')) + 'in x ' + String(dw_label.getITemDecimal(aiRow,'width')) + 'in x ' + String(dw_label.getITemDecimal(aiRow,'height')) + 'in'))
END IF
//////////////////////////////////////////////////////////////////
...
public function integer uf_nokia_part_dw (long albeginrow, long alendrow, long alprintjob);
String	 lsShipDate, lsCountryName, lsSupplier, lsCustomer
Long	llRowPos, llLabelPos, llFindRow, llpos, llBeginPos, llEndPos

If Not cbx_OEM.checked Then Return 0 /* Don't print if checkbox for OEM label not checked*/


i_dw_part_label.dataobject = "d_powerwave_nokia_part_label"

i_dw_part_label.Reset()
i_dw_part_label.InsertRow(0)
...
For llRowPos = alBeginRow to alEndRow
	...
	//From Address
	uf_set_From_address_dw(i_dw_part_label)
	
	//to Address
	uf_set_to_address_dw(i_dw_part_label)
	
	
	//LPN (Carton Nbo)
	If dw_label.GetITemString(llRowPos,'carton_no') > "" Then
		i_dw_part_label.SetItem( 1, "carton_nbr",dw_label.GetITemString(llRowPos,'carton_No'))
		i_dw_part_label.SetItem( 1, "carton_nbr_barcode","1J" + dw_label.GetITemString(llRowPos,'carton_No'))
	End If


	//Supplier - from Customer UF1
	If lsSupplier > "" Then
		i_dw_part_label.SetItem( 1, "supplier",lsSupplier)
		i_dw_part_label.SetItem( 1, "supplier_barcode","V" + lsSupplier)
	End If

	//Customer PO (cust_order_No)
	If not isnull(w_do.idw_main.GetITemString(1,'cust_order_No')) Then
		i_dw_part_label.SetItem( 1, "Cust_order_nbr",w_do.idw_main.GetITemString(1,'cust_order_No'))
		i_dw_part_label.SetItem( 1, "cust_order_nbr_barcode","K" + w_do.idw_main.GetITemString(1,'cust_order_No'))
	End If
	...
	If llFindRow > 0 Then
		If not isnull(w_do.idw_detail.GetITemString(llFindRow,'User_field1')) Then
			i_dw_part_label.SetItem( 1, "cust_sku",w_do.idw_detail.GetITemString(llFindRow,'User_field1'))
			i_dw_part_label.SetItem( 1, "cust_sku_barcode", "P" + w_do.idw_detail.GetITemString(llFindRow,'User_field1'))
		End If
	End IF
	
	//Qty
	If dw_label.GetITemNumber(llRowPos,'quantity') > 0 Then
		i_dw_part_label.SetItem( 1, "qty",String(dw_label.GetITemNumber(llRowPos,'quantity')))
		i_dw_part_label.SetItem( 1, "qty_barcode","Q" + String(dw_label.GetITemNumber(llRowPos,'quantity')))
	End If
	
	//Traceability No (DO_NO?????)
	If not isnull(w_do.idw_main.GetITemString(1,'do_no')) Then
		i_dw_part_label.SetItem( 1, "trace_nbr",w_do.idw_main.GetITemString(1,'do_no'))
		i_dw_part_label.SetItem( 1, "trace_nbr_barcode","1T" + w_do.idw_main.GetITemString(1,'do_no'))
	End If
	...
	i_dw_part_label.SetItem( 1, "Ship_Date",lsShipDate)
	i_dw_part_label.SetItem( 1, "Ship_Date_Barcode","1D" + lsShipDate)
	...
	If dw_label.GetITemString(llRowPos,'country_of_Origin') > "" Then
		...
		i_dw_part_label.SetItem( 1, "COO",lsCountryName)
		i_dw_part_label.SetItem( 1, "COO_Barcode",dw_label.GetITemString(llRowPos,'country_of_Origin'))
	...
	//CartonCount
	If dw_label.GetITemString(llRowPos,'box_of') > "" Then
		i_dw_part_label.SetItem( 1,"Box",dw_label.GetITemString(llRowPos,'box_of'))
	End If
	
	//Weight
	If dw_label.GetITemNumber(llRowPos,'weight_gross') > 0 Then
		i_dw_part_label.SetItem( 1, "Weight",String(dw_label.GetITemNumber(llRowPos,'weight_gross')))
	End If
			
	PrintDataWindow ( alPrintJob, i_dw_part_label )	
...
public function integer uf_huawei_part_dw (long albeginrow, long alendrow, long alprintjob);
String	 lsShipDate, lsDesc, lsSKU, lsSupplier, lsCode, lsCustomer, ls_User_Field17
Long	llRowPos, llLabelPos, llFindRow, llpos, llBeginPos, llEndPos

If Not cbx_OEM.checked Then Return 0 /* Don't print if checkbox for OEM label not checked*/

i_dw_part_label.dataobject = "d_powerwave-huawei-carton_label"
...
For llRowPos = alBeginRow to alEndRow
	...
	If lsSKU > "" Then
		i_dw_part_label.SetItem( 1,"sku",lsSKU)
	End If

	i_dw_part_label.SetItem( 1,"wh_code", w_do.idw_main.GetITemString(1,'wh_code'))

	//Description - retrieve from Item MAster
	lsSKU = dw_label.GetITemString(llRowPos,'Sku')
	lsSupplier = dw_label.GetITemString(llRowPos,'supp_code')
	
	Select Description, User_Field17 into :lsDesc, :ls_User_Field17
	From Item_Master
	Where Project_id = :gs_project and sku = :lsSKU and supp_Code = :lsSupplier USING SQLCA;
	
//	MessageBox ("sql", oledb.sqlcode)
	
//	MessageBox (lsSKU, lsSupplier)
	
	If lsDesc > "" Then
		i_dw_part_label.SetItem( 1,"description",lsDesc)
	End If


	i_dw_part_label.SetItem( 1,"chinese_description",ls_User_Field17)
	...
	If llFindRow > 0 Then
		If w_do.idw_detail.GetITemString(llFindRow,'User_Field1') > "" Then
			i_dw_part_label.SetItem( 1,"alternate_sku",w_do.idw_detail.GetITemString(llFindRow,'User_Field1'))
		Else
			i_dw_part_label.SetItem( 1,"alternate_sku",w_do.idw_detail.GetITemString(llFindRow,'sku'))
		End If
	End IF
	...
	//Customer PO (cust_order_No)
	If not isnull(w_do.idw_main.GetITemString(1,'cust_order_No')) Then
		i_dw_part_label.SetItem( 1,"cust_order_no",w_do.idw_main.GetITemString(1,'cust_order_No'))
	End If
	...
	Else
		lsShipDate = String(Today(),'DD-MMM-YYYY')
	End If
	i_dw_part_label.SetItem( 1,"date",lsShipDate)
	
	//Notes (CartonCount)
	If dw_label.GetITemString(llRowPos,'box_of') > "" Then
		i_dw_part_label.SetItem( 1,"notes",dw_label.GetITemString(llRowPos,'box_of'))
	End If
		
	//Code (Supplier) From Customer UF1
	If lsCode > "" Then
		i_dw_part_label.SetItem( 1,"code",lsCode)
	End If
	
	//Lot no entered by user on screen
	If dw_label.GetITemString(llRowPos,'c_huawei_code') > "" Then
		i_dw_part_label.SetItem( 1,"lot_no",dw_label.GetITemString(llRowPos,'c_huawei_code'))
	End If
...
on w_powerwave_shipping_labels.create
int iCurrent
call super::create
...
this.Control[iCurrent+7]=this.cbx_oem
end on

on w_powerwave_shipping_labels.destroy
call super::destroy
...
Select User_Field2 into :isOEMLabel
From Customer
Where Project_id = :gs_Project and Cust_Code = :lsCustomer;

If isOemLabel > "" Then
	cbx_oem.Checked = True
	cbx_oem.text = "OEM (" + isOEMLabel + ")"
Else
	cbx_oem.Visible = False
End IF
...
For llRowPos = 1 to llRowCount
	...
	//Show OEM Label...
	dw_label.SetItem(llRowPos,'c_oem_label',isOEMLabel)
...
For llRowPos = 1 to llRowCount
	...
	//BCR 12-APR-11: Uncommented the following line to fix the box_of problem...JIRA # 202
...
type cb_cancel from w_main_ancestor`cb_cancel within w_powerwave_shipping_labels
boolean visible = false
integer x = 1938
integer y = 1596
integer height = 80
...
type cb_ok from w_main_ancestor`cb_ok within w_powerwave_shipping_labels
boolean visible = false
integer x = 3397
integer y = 52
integer height = 80
...
integer height = 1340
boolean bringtotop = true
string dataobject = "d_powerwave_label_Grid"
