HA$PBExportHeader$w_garmin_labels.srw
$PBExportComments$Print H2O Shipping labels
forward
global type w_garmin_labels from w_main_ancestor
end type
type cb_label_print from commandbutton within w_garmin_labels
end type
type dw_label from u_dw_ancestor within w_garmin_labels
end type
type cb_label_selectall from commandbutton within w_garmin_labels
end type
type cb_label_clear from commandbutton within w_garmin_labels
end type
type cbx_part_labels from checkbox within w_garmin_labels
end type
end forward
...
global w_garmin_labels w_garmin_labels
...
Any	lsAny
String	lsCityStateZip, lsSKU, ls_format, ls_carton_no,	lsDONO,  ls_CustName,ls_find,lsCartonNo, lsConsolNo, lsPrinter, lsDONOPrev, lsStore, lsInvoiceNo
...
//  If we have a default printer for GARMIN Invoice, Load now
lsPrinter = ProfileString(gs_iniFile,'PRINTERS','GARMINLABELS','')
...
For llRowPos = 1 to llRowCount /*each detail row */
	...
	lsInvoiceNo = dw_label.GetITemString(llRowPos,'invoice_no')
...
// Get the Store# from the order
	lsStore = mid(lsInvoiceNo,pos(lsInvoiceNo,'-',1)+1)
	Lstrparms.String_arg[23] = lsStore /* Store  */
	...
	invo_labels.uf_garmin_ucc_labels(lsAny)
...
// We want to store the last printer used for Printing the GARMIN label for defaulting later
lsPrinter = PrintGetPrinter()
SetProfileString(gs_inifile,'PRINTERS','GARMINLABELS',lsPrinter)
...
//	dw_label.getItemString(1,'delivery_Master_cust_code') = '05062' Then /*Customer name contains MCX or ULTA*/
...
//If Pos(upper(dw_label.getItemString(1,'delivery_Master_cust_name')),'SEARS') > 0 Then /*Customer name contains KOHL*/
...
type cb_cancel from w_main_ancestor`cb_cancel within w_garmin_labels
...
type cb_ok from w_main_ancestor`cb_ok within w_garmin_labels
integer x = 1874
integer y = 28
integer height = 80
...
integer height = 1456
boolean bringtotop = true
string dataobject = "d_garmin_uccs_ship"
