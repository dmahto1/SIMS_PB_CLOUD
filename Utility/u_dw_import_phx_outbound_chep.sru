HA$PBExportHeader$u_dw_import_phx_outbound_chep.sru
forward
global type u_dw_import_phx_outbound_chep from u_dw_import
end type
end forward

global type u_dw_import_phx_outbound_chep from u_dw_import
string title = "Import Phx Outbound Chep"
string dataobject = "d_import_phx_outbound_chep"
end type
global u_dw_import_phx_outbound_chep u_dw_import_phx_outbound_chep
...
public function string wf_validate (long al_row);//Jxlim 04/19/10 New Phoenix Brand Import Outbound.
...
//Validate Customer Order Number must be Exist and valid
lssuppordnbr:
lsSuplOrdNo = This.getItemString(al_row,"order_nbr")
// Supplier Order Number must be Exist
IF isNull( lsSuplOrdNo ) or len(lsSuplOrdNo ) = 0 then 
	lsMsg= "Order Number' can not be null!"
	Return lsMsg
End IF
...
IF (Not isnull(lsSuplOrdNo)) and lsSuplOrdNo > ' ' Then
		...
		And invoice_no = :lsSuplOrdNo
		And ord_status in ('C', 'D')
		Using SQLCA;	
		
		If llCount <= 0 Then
			lsMsg="Order Number' Does Not Exist!"
			Return  lsMsg
			This.Setfocus()
			This.SetColumn("order_nbr")
			iscurrvalcolumn = "order_nbr"			
		End If		
...
For llRowPos = 1 to llRowCount	
	...
	lsSuppOrdNbr = This.GetItemString(llRowPos, "order_nbr")
	...
	Set User_field15 = :lsChepProcDate, 
	User_Field17 = :lsChepRefNbr, 
	User_Field10 =  :lsChepReconQty
	Where project_id = :gs_project
	And invoice_no = :lsSuppOrdNbr
	Using SQLCA;
...
on u_dw_import_phx_outbound_chep.create
call super::create
end on

on u_dw_import_phx_outbound_chep.destroy
call super::destroy
end on
