HA$PBExportHeader$w_sika_shipping_labels.srw
$PBExportComments$Print Generic UCCS Shipping labels
forward
global type w_sika_shipping_labels from w_main_ancestor
end type
type cb_print from commandbutton within w_sika_shipping_labels
end type
type dw_label from u_dw_ancestor within w_sika_shipping_labels
end type
type cb_selectall from commandbutton within w_sika_shipping_labels
end type
type cb_clear from commandbutton within w_sika_shipping_labels
end type
end forward
...
global w_sika_shipping_labels w_sika_shipping_labels
...
integer li_Max_Count = 0
integer li_box_number

FOR llRowPos = 1 to llRowCount
	
	li_box_number = dw_label.GetItemNumber(llRowPos,'box_number')
	
	if li_box_number > li_Max_Count then li_Max_Count = li_box_number
...
//	
	...
	lstrparms.String_Arg[28] = String(li_box_number) +" of " + String(li_Max_Count)
	...
	lsAny=lstrparms		
	invo_labels.uf_sika_shipping_label_zebra(lsAny)
...
on w_sika_shipping_labels.create
int iCurrent
call super::create
...
on w_sika_shipping_labels.destroy
call super::destroy
...
type cb_cancel from w_main_ancestor`cb_cancel within w_sika_shipping_labels
...
type cb_ok from w_main_ancestor`cb_ok within w_sika_shipping_labels
integer x = 1769
integer y = 24
integer height = 80
...
integer height = 1456
boolean bringtotop = true
string dataobject = "d_sika_shipping"
