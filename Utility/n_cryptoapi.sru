...
// 11/27/2012	ET3			Various Modifications 
// -----------------------------------------------------------------------------
...
If Not CryptGetDefaultProvider(iProviderType, 0, CRYPT_MACHINE_DEFAULT, ls_provider, lul_datalen) Then
...
lul_dwIndex = 0
do while CryptEnumProviders(lul_dwIndex, 0, 0, lul_dwType, 0, lul_cbName)
	ls_provider = Space(lul_cbName)
	If CryptEnumProviders(lul_dwIndex, 0, 0, lul_dwType, ls_provider, lul_cbName) Then
	...
	Else
		of_GetLastError(lul_error, ls_msgtext)
		SignalError(lul_error, "CryptEnumProviders:~r~n~r~n" + ls_msgtext)
	End If
	lul_dwIndex++
loop
...
//					Use STRING(<blob>, EncodingANSI!) on returned value to 
...
//             create the blob using this as an example: ablob_data = blob(as_string,EncodingANSI!)
//
...
lb_rtn = CryptBinaryToString(ablob_data, lul_len, CRYPT_STRING_BASE64, ls_encoded, lul_buflen)
...
// decrypt the data
lblob_decrypted = this.of_EncryptDecrypt( ablob_data, as_password, False)
...
// encrypt the data
lblob_encrypted = this.of_EncryptDecrypt( ablob_data, as_password, True)
...
// encrypt the data
lblob_encrypted = this.of_EncryptDecrypt( lblob_data, as_password, True)
...
// encrypt the data
lblob_decrypted = this.of_EncryptDecrypt( lblob_data, as_password, False)
