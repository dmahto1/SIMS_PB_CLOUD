HA$PBExportHeader$u_dw_import_pandora_serial_no.sru
$PBExportComments$Process complex import logic for Pandora min/max values
forward
global type u_dw_import_pandora_serial_no from u_dw_import
end type
end forward
...
integer width = 5225
integer height = 2072
string title = "Import Serial Number"
string dataobject = "d_import_serial_number"
end type
global u_dw_import_pandora_serial_no u_dw_import_pandora_serial_no
...
ELSEIF lrtn = 1 AND bSaveErrorsFound THEN
	MessageBox('Info', 'Some Changes successfully saved. Please update tracked by serial number in Item Master.')
...
ELSEIF lrtn = 1 AND NOT bSaveErrorsFound THEN
	MessageBox('Info', 'Changes successfully saved.  Please update tracked by serial number in Item Master. ')
	this.reset( )
	If IsValid(w_import) Then
		w_import.cb_import.Enabled = TRUE
		w_import.cb_ok.Enabled = TRUE
...
on u_dw_import_pandora_serial_no.create
call super::create
end on

on u_dw_import_pandora_serial_no.destroy
call super::destroy
end on
...
ids_codes.DataObject = "d_import_serial_number"
...
//				lrst = ids_codes.SetItem( ldr, 'import_col_num', lCol_Num )
//				lrst = ids_codes.SetItem( ldr, 'import_cust_code', sCust_Code )
				lrst = ids_codes.SetItem( ldr, 'owner_cd', sCust_Code )
				lrst = ids_codes.SetItem( ldr, 'wh_code', sWH_Code )
				lrst = ids_codes.SetItem( ldr, 'owner_id', lOwner_Id )
	...
	DO UNTIL (i >= llen)
		...
		If cnt = 1 then
			...
			if SQLCA.sqlcode <> 0 then
				...
				sWH_Code = 'INVALID'
			End if
		ELSEIF cnt = 2 THEN //SKU
			sSKU = TRIM(ss)
			
			SELECT Sku, po_no2_controlled_ind ,container_tracking_ind 
			INTO :lsSkuFound, :lsPno2ControlledInd , :lsContainerTrackingInd
			FROM ITEM_MASTER
			...
			If SQLCA.sqlcode <> 0 or lsSkuFound = '' then
				bInvalidSKU = True
				sSKU = 'INVALID'
			End if
			If lsPno2ControlledInd = 'Y' and lsContainerTrackingInd = 'Y' then
				//LPN SKUS don't allow importing serial numers
				sMsg = "These SKU's are licensed Plated - ~r~n" &
	      		+ " importing serial numbers are not allowed at this time. ~r~n" &
				+ " Please check the file and try again."
				MessageBox('Import Error', sMsg )
				RETURN
			End if
		ELSEIF cnt = 3 THEN  // now process the PONO - project code
			sSerialNo = TRIM(ss)

			lSetCnt = 0
			
//			If IsNull(ss) OR (TRIM(ss) = '') Then
//				sSerialNo = sDefProjCode
//			Else
//				sSerialNo = TRIM(ss)
//			End If
...
//					sWH_Code = ids_codes.GetItemString( lSetCnt, 'wh_code')
//					sCust_Code = ids_codes.GetItemString( lSetCnt, 'cust_code')
//					lOwner_Id = ids_codes.GetItemNumber( lSetCnt, 'owner_id')
					...
					this.setitem( ldr, 'serial_no', sSerialNo ) 
					...
					this.setitem( ldr, 'project_id', gs_project )
					this.setitem( ldr, 'Update_User', gs_userid )
					this.setitem( ldr, 'Update_Date', ldttoday )
