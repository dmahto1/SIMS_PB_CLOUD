HA$PBExportHeader$w_batch_pick.srw
...
If Upper(gs_project) = 'PHILIPSCLS' and lsGsinType = "2" Then
...
//				lsCarton = Right(Mid(idw_pick.GetItemString(llPickPos,"do_no"),(len(gs_project) + 1),7),6) /* right 6 of everything after project*/
		...
		If Upper(gs_project) = 'PHILIPSCLS'  and idw_pick.GetItemString(llPickPos,"ord_status") <> 'C' and idw_pick.GetItemString(llPickPos,"ord_status") <> 'R' and idw_pick.GetItemString(llPickPos,"ord_status") <> 'V' Then
...
IF Upper(gs_project) <>  'AMS-MUSER' AND Upper(gs_project) <>  'MAQUET' AND  Upper(gs_project) <> 'STBTH'  THEN
	...
	Else
		If lStrparms.String_Arg[1] = 'O' Then /*sort by order, load standard pick list*/
			...
			If Upper(gs_project) = 'PHILIPS-SG' or Upper(gs_project) = 'PHILIPSCLS' or  Upper(gs_project) = 'TPV'  or  Upper(gs_project) = 'FUNAI' or  Upper(gs_project) = 'GIBSON'   Then /* 09/09 - PCONKL, 12/12 - added TPV */ /* 6/13 - MEA - added FUNAI */ //TAM - 2015-01-12 added GIBSON
...
For i = 1 to ll_cnt
	...
	If (ls_sku <> lsSKUHold) or (lsSupplier <> lsSupplierHold) Then
		...
		If llFindRow > 0 and J > 1 Then
			...
			// 04/29/11 cawikholm
			//1-FEB-2019 :Madhu S28945 Added PHILIPSCLS
			IF ((Upper(gs_project) = 'PHILIPS-SG' or Upper(gs_project) = 'PHILIPSCLS') AND lsSupplier = 'SG03') THEN
